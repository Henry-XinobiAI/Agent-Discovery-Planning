...
>
> **다음 순서 (2026-07-09 합의):** **Phase 9(eval→CI gate) → Phase 10(real edge/maturity) → Phase 8B
> 잔여 튜닝.** CI 게이트로 "깨지면 바로 보임"을 Phase 10의 integration surface 확대 전에 잠근다. 8B
...
> Phase 8A) · `fallback_used` teeth(Phase 8A) · 재정의된 8B 그라운딩 폴백 사다리 전체(expansion ③ +
...
- **소비 위치 결정 (B-vs-C, Phase 10에서 확정):**
  - **(B)** discovery의 rerank가 `topic_context`를 프롬프트 맥락으로 소비.
  - **(C)** memory-api가 ingest용 `Grounder.ground(mention, *, context=)`를 read 엔드포인트로 노출 →
    discovery가 `ground(topic_text, context=topic_context)` 호출. **일관성 이점:** `/personal/groundings`
    edge가 바로 그 grounder로 만들어졌으니, 쿼리도 같은 grounder로 grounding하면 QID join이 어긋날 위험이
    적음(Phase 10 real edge와 직결).
  - `topic_context` 필드는 B/C에 무관 — 맥락을 안쪽으로 나르기만 함. 결정은 real edge 붙일 때.
...
- **추가:** [`eval gate`](10-eval-metrics-and-gates.md)를 CI 파이프라인의 게이트 스텝으로 배선. 모든 PR에서
  ratchet 강제, baseline 갱신은 PR 리뷰로.
- **기존 변경:**
  - 코드 변경은 최소 — `eval gate`가 이미 exit code 계약을 갖고 있음 (0/non-zero). CI yaml/workflow 추가가
    주 작업.
  - baseline.json 갱신이 리뷰 대상 diff가 됨 (이미 diff-stable JSON).
  - real anchor backfill(`build-anchors`)이 CI 이전에 선행돼야 "teeth" 있는 게이트가 됨 (`manual-seed` →
    실제 memory-api 캡처). 현 ratchet은 전부 천장(top1/top3=1.0, fallback=0.0)이라 real anchor + ambiguous
    층이 들어와야 게이트에 변별력이 생김.
- **선행 전제(2026-07-08):** real 앵커는 **동음이의 exact-label 동점**을 드러냄(오프라인에서 7/25만 ground —
  상세 [findings](findings-real-anchor-grounding-ties.md)). teeth 있는 재측정의 실질 선행은 **memory-api
  search relevance 개선**(alias는 이미 검색되나 흔한 이름은 label^3에 밀림)이며, 이건 memory-api 팀 소유의
  외부 dependency다(discovery 8B가 지는 작업 아님). **2026-07-10 실측**으로 이 gap 확인(예: `q=JavaScript`
  → 정본 top-50 밖) — 상세 [findings](findings-real-anchor-grounding-ties.md) "Memory-api public search
  relevance", 요청은 `memory_api_agent_recommendation_requirements*.md`. 이 검색 개선(맥락-무관 recall/
  ranking)과 동음이의 sense 선택(맥락 필요, §8-7)은 **별개**다.
...
- **8B 잔여 튜닝·8-4와의 순서:** 8B 사다리 _구현_은 끝났다(→ [03](03-normalize-and-linker.md)). 남은 8B
  **튜닝**(expansion threshold + expansion stratum)과 judge 분모(8-4)는 후보가 mock인 상태에선
  ...
  의존. 그리고 §8-7(`topic_context` 소비 B-vs-C)도 real edge 붙일 때 함께 확정한다.

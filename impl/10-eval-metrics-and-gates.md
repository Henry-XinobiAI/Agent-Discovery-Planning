...
[06. decision log](06-serving-and-decision-log.md) · [11. Phase 8/9 로드맵](11-phase-8-9-roadmap.md)
...
- `ambiguous_fallback_rate`: decision B — `fallback_used`만 (grounding_failed 에러는 errored-count
  hard fail로 이미 잡히니 double-book 안 함). Alpha에선 `fallback_used`가 항상 False라 **0.0**,
  거기서 freeze. Phase 8 fallback 착지 시 teeth.
...
CI 판정. 전부 결정적 gold. ratchet 슬롯(특히 fallback)은 지금 잠잠하지만 Phase 8이 teeth를 붙입니다

...
[06. decision log](06-serving-and-decision-log.md) · [11. Phase 8–10 로드맵](11-phase-8-9-roadmap.md)
...
- `ambiguous_fallback_rate`: decision B — `fallback_used`만 (grounding_failed 에러는 errored-count
  ...
  자체는 serving에서 live (Phase 8A); rerank·substitution report-only strata가 가짜 rung을 주입해 eval에서도 fallback 경로를 밟지만 committed baseline은 불변(가짜는 그 strata에만 주입·committed gate는 `reranker=None`/`substituter=None` → baseline byte-identical); real-anchor 코퍼스로 eval을 reranker와 함께 돌릴 때 teeth.

### (C) report-only
- `by_need` / `by_stratum` breakdown (grounding_top1) — **절대 gate 조건 아님** (decision A).
...
**★ baseline-in-sync = ratchet의 teeth:**
```python
...
(0.0), fallback 자체는 serving에서 이미 live (Phase 8A)이고 rerank·substitution report-only strata(주입 가짜)가 eval에서 fallback 경로를 밟아도 committed baseline은 그대로다 — [11. Phase 8–10 로드맵](11-phase-8-9-roadmap.md).

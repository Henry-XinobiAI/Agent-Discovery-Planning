...
[agentic stance retrieval 스펙](../docs/superpowers/specs/2026-07-20-agentic-stance-retrieval-design.md) (Phase 10 stance 슬라이스)
...
- **선행(진짜 크리티컬 패스) — cross-team 계약:** (1) **memory-api edge = Competence vector 투영 계약**
  ...
  **query-time judge 산출**이므로 여기선 claims/evidence 공급만; ↓ 스펙) ② competence→edge signal
  ...
  축이라 계약 예약. 계약 협의는 리드타임이 기니 코드 작업과 무관하게 즉시 시작. (3) **neutral statement-search
  엔드포인트(신규) — stance judge Tier 1.5 전용 lead-time ask**: `POST /personal/statements/search`(`subject_qid` +
  `owner_ids` + `queries` + `active_only`=True + `per_owner_limit`; owner별 top-K·global limit 없음)로 QID-bound BM25 statement 검색. 기존 statement
  인덱스(필드 이미 존재) 위 신규 라우트라 rebuild 불필요. Tier 1 후보 조회·provenance 원문 조회(`conversations/messages/lookup`)는
  **이미 충족** → **필수 신규 memory-api 변경은 이 statement-search 1건**(기준 repo `bourbon-memory-api` main@`f12e8b2`). 이 엔드포인트는
  dense/insufficient 후보의 on-demand evidence recall(§D3)용. 상세 =
  ↑ [stance 스펙](../docs/superpowers/specs/2026-07-20-agentic-stance-retrieval-design.md) §D6·§D3.

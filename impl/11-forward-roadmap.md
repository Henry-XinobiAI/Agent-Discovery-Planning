...
[stance retrieval 스펙](../docs/superpowers/specs/2026-07-16-stance-for-against-retrieval-design.md) (Phase 10 stance 슬라이스)
...
- **선행(진짜 크리티컬 패스) — cross-team 계약:** (1) **memory-api edge = Competence vector 투영 계약**
  ...
  **query-time judge 산출**이므로 여기선 claims/evidence 공급만; ↓ 스펙) ② competence→edge signal
  ...
  - **★ stance(①의 일부)·evidence-ref(③) 하위 슬라이스 설계 = [for/against stance retrieval 스펙](../docs/superpowers/specs/2026-07-16-stance-for-against-retrieval-design.md)** (PR #12, `009917b`). 요지: for/against **verdict는 Discovery 소유 Tier 2 LLM claim-judge**가 만들고 memory-api엔 stance verdict/`stance_dir`를 요청하지 않음(query-무관 evidence/prior만) · 2-tier retrieval(Tier 1 stance-무관 shortlist / Tier 2 judge) · judge OFF/degraded → **명시 silence**(topic-discussers fallback 금지). **이 작업은 독립 phase가 아니라 Phase 10의 stance 슬라이스로 얹힌다** — 단 memory-api claim-inline 계약 협의는 lead-time상 먼저 킥오프하고, verdict 품질 검증은 **8-4 B2 silver judge** stratum에 의존한다. (aspect 세밀도는 파편화 내재 비용으로 **Open Beta 이월**.) (2) **agent moderator의
  ...
  축이라 계약 예약. 계약 협의는 리드타임이 기니 코드 작업과 무관하게 즉시 시작. (3) **claim-inline
  payload(옵트인) — stance judge 전용 lead-time ask**: list/search 응답에 stance-bearing claim을 옵트인
  inline(`include_claims`, 엔티티당 top-M) + `text`·`statement_kind`·`epistemic`·`confidence`·
  `provenance_message_ids`(+가능하면 `support_snippet`). Tier 2 judge의 N+1 round-trip을 없앤다. 상세 =
  ↑ [stance 스펙](../docs/superpowers/specs/2026-07-16-stance-for-against-retrieval-design.md) §4·§D5.

...
| `rerank-injection` | rerank는 이제 live (8A) ⇒ fallback ladder의 report-only strata(rerank/substitution)가 **가짜 rung을 주입**해 rung 복구 + provenance를 assert. 결정적 gate는 `reranker=None` 유지(baseline 불변). |

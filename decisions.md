...
| O2 | 요청자 자신의 topic 목록을 **어디서** 읽나: (a) 요청 시 topic-api 내부 route(권고) vs (b) 소유자 키 아래에만 미러. 읽는 tier 범위(public·friends·private, hidden 제외)는 R22로 닫혔다 | 오너 |
...
| O9 | 타입 ② 섹션 크기·순서·페이지(섹션당 몇 명, 한 화면에 몇 섹션, 섹션 순서가 요청자 preference 점수인지 — 지금은 preference) | 오너 |
...
| O18 | **content 유사도의 정의.** 지금은 topic id 정확 일치의 가중 합이다. 제안: (a) IDF 가중 `log(전체 소유자 수 / 그 topic을 가진 소유자 수)` — 흔한 topic 겹침의 과대평가를 막는다; (b) 계층 감쇠 — topic-api 검색 설정(`SEARCH_DEFAULT_DECAY` 0.6, `SEARCH_MAX_DEPTH` 3)의 상수 **0.6/hop, 최대 3 hop**을 빌린다. 빌리는 것은 상수다: topic-api는 `/search/users`에서 질의 topic의 자손(하향) 방향에 `decay^distance`를 쓰고 단일 topic route(`/topics/{id}/users`)는 감쇠가 없는데, 우리는 요청자 topic과 소유자 topic 사이의 양방향 최단 hop에 쓴다(요청자 `hand_drip` ↔ 소유자 `coffee`처럼 부모만 가진 소유자도 0.6). 카탈로그는 DAG라 최단 hop, 경로당 가장 구체적인 일치 하나만 세고(하위 정확 일치 1.0 > 부모만 일치 0.6), drawer(`selectable: false`)는 일치 노드가 아니라 통과 노드; (c) 임베딩 kNN(pgvector/OpenSearch)은 넣지 않는다 — 카탈로그 자체가 의미 계층이고 grounding이 `related`까지 표시한다. 제안이 정하는 다시 볼 조건: topic-api의 "grounding 실패 preference 비율"이 나쁠 때. **제안에서 제외한 것**: "요청자가 대화한 agent들의 topic을 요청자 프로필에 더한다" — bourbon-agent가 요청자의 발화에서 이미 persona를 추출해 topic으로 흘러오고, agent의 발화는 evidence가 될 수 없다는 추출기 규칙과 정면으로 어긋난다(행동 신호는 CF의 몫). 필요한 것: 카탈로그 그래프 내부 route(이벤트 정의서 §2-8). 측정: 비교표에 "content만" 행 | 비교 설계 시 · route는 topic-api |
| O19 | **요청자가 이미 대화를 시작한 agent를 for-you(타입 ③)에서 제외하는가.** 계약 §2-3과 합성 정답(스펙 §4 ③ CF)은 제외를 가정하고 있고, walkthrough §5-3·§5-4가 A·H를 그 가정으로 뺀다. 제품이 다르게 정하면 셋을 같이 바꾼다 | 오너 |

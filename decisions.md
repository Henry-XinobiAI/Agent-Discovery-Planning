...
topic별 item으로 — `√wsum(d)`에서 다른 주제가 빠짐, 정확 계산 recall 99/100, 후보 3은 쓰지 않음)이고 **`GOR-X6` 통과**(2026-09-04: Gorse recall 94/100, 정확 99/100; 상위 100의 66명이 최상위 2개 이상 — 폭 중립; 비용 item ×1.88). **채택(`Dnn`)은 오너 확인 대기.** 그것과 `N*`가 끝나면 C11을 닫는다. 실행 순서는
`gorse.md` §12-6.

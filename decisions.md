...
> 갖고 있던 것) · `분석`(이번 조사가 근거를 만들어 올린 것, **오너 확인 전**) · `실측`(실험이 근거를 만들고
> 오너가 그 결과를 보고 확정한 것 — `gorse.md` §12-5의 결과 소절이 근거다). 규칙 ③을 지키려면
> 넷이 같은 무게로 보여서는 안 된다.
>
> **마지막 갱신**: 2026-09-07
...
| **D03** | `UserId = requester_user_id`, `ItemId = personal_agent_id`(owner에서 결정론적 유도). **surface 1 전용 인스턴스(D22)의 item은 `D24`가 따로 정한다** | `recsys_opensource/README.md` §3 | 승계 | 2026-09-01 |
...
**2026-09-03: 아래 본문 정리를 모두 마쳤다(그 뒤 추가된 행은 각 행에 날짜).** 마지막 열은 그때 무엇을 고쳤는지의 기록이고,
현재 그 위치에 옛 서술은 없다. 각 문서의 변경 이력이 상세를 갖는다.

참조에 **줄번호를 쓰지 않는다** — 이전 판이 줄번호로 적었던 참조 다섯 개가 적은 그날 이미 어긋나 있었다(2026-09-03 확인).
...
| `degraded`를 wire에 실을지 | **필드 삭제 — 도달 불가** → **D20이 다시 도달 가능하게 만듦. 되살린다** | rev 5.5가 지운 이유가 "전부 503이라 발생할 수 없는 값"이었고 그 전제가 사라졌다. 2026-09-03 | `recommendation_pipeline_design.md` §9 ⑧ (rev 5.13에서 해소 표시 — D20·D21) |
...
### C7 — catalogue projection 지연의 성격 `해소 경로 확인 — N*는 베타 뒤(오너 2026-09-07)`
...
**→ `GOR-X5` 통과(2026-09-04)**: 새 세대 쿼리 item에는 새 agent가 **1 사이클 안에** 보이고, 옛 세대는 `cache_expire` 전엔
영구히 안 갱신된다는 것이 실측됐다. 삭제 세대의 문자열 키 3개는 우리가 지운다(절차는 §12-5 X5 결과). 이제 가시성
지연 = **사이클 길이 ≈ 0.4 ms × item 수**(`GOR-X3` 실측, 8 jobs). **목표 가시성 지연은 오너가 정한다**(이 항목의 값) — 그것이 `N*`의 사이클 항이 되고, `N*`가 확정되면 C11과 함께 닫는다.

**2026-09-07 오너**: `N*`는 Closed·Open Beta 뒤에 정한다(D22 주석). 이 항목은 그때까지 열린 채 두되 **할 일은 없다** — 가시성 목표는 운영에서 사이클 길이를 직접 보며 정하면 된다.
...
**닫는 것**: `GOR-X3`(빌드 곡선 → 임계값 `N*`) · `GOR-X1`·`X4`(컷 안 깊은 사람 비율). 열한 가지 후보와
실험 설계는 `gorse.md` §12-4·§12-5. **⑵는 오너 결정이다.** 2026-09-03.

**진행 (날짜순)**

- **2026-09-03 — `D22`**(오너): 시작은 Gorse + 7 + 9, 뒤집는 조건은 `I_eligible ≥ 0.5·N*`(여유율 50%). 남은 것 둘: `N*`의 숫자(X3) · 문제 ① 쪽 조합(X1·X4).
- **2026-09-04 — `GOR-X1` → `D23`**(오너 "1 채택, 2는 X4 뒤"): 다중 라벨 + 센티넬이 M2를 막고 깊은 사람을 최상위로 올린다(10⁴ 축소 확인 포함).
- **2026-09-04 — `GOR-X3`**: 실측 5×10⁵까지. RAM ≈ 0.42 GB + 8.3 KB × item, 사이클 ≈ 0.4 ms × item(8 jobs). **`N*` 제안(분석, 오너 확정 전): 5×10⁶ item = 64 GB 박스·사이클 ~33분 → agent 환산(배수 1.88) 약 2.7×10⁶, 뒤집는 조건 agent 약 1.3×10⁶.** 오너가 박스 크기·가시성 목표(C7)를 정하면 확정.
- **2026-09-04 — `GOR-X4`**: 단독 다중 쿼리(D23)는 depth만 보면 100/100이지만 recall@GT100(depth → 희귀 하위 → 폭)은 34 — 컷이 **focus 필터**로 작동(전원 태그 6개). 후보 2 기각 근거(단독이면 depth-4 선택, 다중에 얹어도 불변) · 후보 4 무효(`cache_size` 1,000에도 38/100) · 후보 3은 통함(S=100에서 1.0, 후보 수 10⁴, 규모 비례).
- **2026-09-04 — 오너 결정**: 깊이에 희귀 하위·폭을 넣는다. 셋 중 **(i) D23 유지** — (ii) 샤딩(후보 3)은 오버헤드, (iii) 우리 인덱스(후보 5)는 지연. 이 구조 안의 개선으로 후보 11(agent × 최상위 topic item).
- **2026-09-04 — `GOR-X6` → `D24`**(오너 채택): Gorse recall 94/100(정확 99), 상위 100의 66명이 최상위 2개 이상 — 폭 중립. 비용 item ×1.88.
- **2026-09-07 — 오너**: **`N*`는 지금 정할 수 없다.** Closed Beta·Open Beta 뒤에 공개 보유 비율·item 배수의 평균이 나와야 추론할 수 있다. 목표 유저 풀은 단계적(20만 → 100만 → 1천만, 최종 1억까지 대응)이며 단계 계획은 `storage_sizing.md`가 맡는다.

**현재 남은 것은 `N*` 하나**이고 **베타 뒤로 미뤄졌다**(오너 2026-09-07). 그때까지 C11(과 C7)은 열린 채 두되 **기획 쪽 할 일은 없다** — 출시 후 X2(b) 계측과 D22 주석의 직접 신호 감시만 남는다. 실행 순서는 `gorse.md` §12-6.
...
| 항목 | 이전 성격 | 지금 | 왜 |
|---|---|---|---|
| `SURV-R4` — §7-3 형태의 실제 보유 데이터 품질 | 채택 판단용 측정 | **배포 전 게이트** | D07이 후보 생성 전체를 이 방식에 맡긴다. 근거는 합성 데이터(6명 → 5×10⁵, `gorse.md` §12-5 X4·X6)까지, 실데이터 미검증 |

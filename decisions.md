...
### C7 — catalogue projection 지연의 성격 `해소 경로 확인 — 남은 값은 가시성 목표(오너) → N*`
...
**현재 남은 것은 `N*` 하나**다 — 그것이 정해지면 C11(과 C7)을 닫는다. 실행 순서는 `gorse.md` §12-6.

...
**→ 시작과 뒤집는 조건은 `D22`로 닫혔다**(2026-09-03, 오너: 7+9로 시작, 여유율 50%). **남은 것 둘**: ⑴ `N*`의
숫자(`GOR-X3`) ⑵ ①(컷 품질) 쪽 조합 — 1은 `GOR-X1`, 2·3은 `GOR-X4`. 둘이 끝나면 C11을 닫는다. 실행 순서는
`gorse.md` §12-6.

아래는 D22 이전의 논거다(기록):
- **시작(제안)**: ⑴ — Gorse + 전용 인스턴스(7) + 쿼리 item 재생성(9). 내부 규모에서 동작이 확인된 쪽이고,
  S3이 단계 경계라 소스 교체가 S1·S2·S4–S6에 닿지 않는다(**S3을 port로 유지하는 것이 조건**).
  7·9는 문제 ②(규모·신선도)의 것이고 소스로 확인된 동작 위에 서 있어 실험 없이 간다.
- **①(컷 품질) 쪽은 실험이 먼저다 — 기각이 아니다.** 1(다중 라벨 + 센티넬)은 공식을 *읽어서* 나온 예측이라
  `GOR-X1`(한 시간)이 확인하는 즉시 시작안에 합류한다 — 코드 변경 0. 2(파생 태그)·3(샤딩)은 1만으로
  충분한지를 `GOR-X4`가 답한 뒤 조합을 정한다.
- **뒤집는 조건**: 출시 후 우리 복제본(`D10`)에서 센 `I_eligible`이 X3의 `N*`에 접근하면 ⑵로. 계측은
  topic-api 협의 없이 우리 것으로 된다(`gorse.md` §12-5 X2(b)).
- ~~**지금 정할 것**: 위 시작안을 받는가, 그리고 `N*`의 여유율~~ → D22.

---

### C12 — 출처가 `분석`인 결정의 확정 `열린 항목`

`D11`·`D16`·`D19`·`D20`·`D21`은 출처가 **분석**이다 — 이번 조사가 근거를 만들어 올렸고 오너가 직접 말한
것이 아니다(규칙 ⑤). 오너가 2026-09-03 **다시 협의하자**고 했다. 협의 전까지 다섯은 §1에 적힌 대로 문서를
묶는다(뒤집힌 것이 아니다). 협의 결과에 따라 출처를 `오너`로 바꾸거나, 뒤집어 §2로 옮기거나, 본문을 고친다.

- `D11` topic-api 최종 N 조회는 필수 — `D21`과 같은 호출이라 D21이 서면 사실상 따라온다.
- `D16` `auto_insert_*` 끄기 + 재실행 가능한 프로젝션 pass.
- `D19` 의미 미정 이벤트는 Gorse에 보내지 않는다 — `C10`과 붙어 있다.
- `D20`·`D21` 실패 규율 — C1의 해소이므로 뒤집히면 C1이 다시 열린다.

...
| **D22** | **surface 1 후보 생성은 Gorse로 시작한다 — surface 1 전용 인스턴스 + 쿼리 item 세대 재생성.** S3은 port로 유지해 소스 교체가 다른 단계에 닿지 않게 한다. **뒤집는 조건**: 출시 후 우리 복제본(`D10`)에서 센 `I_eligible`이 `GOR-X3`이 정할 임계값 `N*`의 **50%**에 닿으면 우리 인덱스(`gorse.md` §12-4의 5)로 전환을 **착수**한다 | `gorse.md` §12-4의 7·9, §12-3. 출시 전엔 `I_eligible`을 알 수 없어(X2) "어느 갈래"가 아니라 "시작과 뒤집는 조건"을 정한 것. `N*` 값 자체는 X3 뒤 여기에 숫자로 적는다. **X3 실측(2026-09-04)**: RAM ≈ 0.42 GB + 8.3 KB × N, 정상 사이클 ≈ 0.4 ms × N(8 jobs). **제안(분석) `N* = 5×10⁶` item**(64 GB 박스, 사이클 ~33분) → 뒤집는 조건 `I_eligible ≥ 2.5×10⁶` item. **`D24` 뒤 item = agent × 최상위 topic이라 agent 기준으로는 배수(합성 1.88)로 나눈다** — 5×10⁶ item ≈ agent 2.7×10⁶, 조건 ≈ agent 1.3×10⁶. **박스 크기·가시성 목표를 오너가 정하면 확정** | 오너 | 2026-09-03 |
| **D23** | **S3의 Gorse 쿼리에 topic당 다중 라벨 + 센티넬 쿼리 하나를 더한다**(`gorse.md` §12-4의 1). 확정 topic의 라벨 전부와 `__QUERY__`를 한 쿼리 item에 실어 이웃을 묻는다. 후보 2(`#deep` 파생 태그)·3(샤딩)은 **`GOR-X4` 뒤에** 정한다 — 기각이 아니다 | `gorse.md` §12-5 X1 결과: 센티넬이 M2를 막고, 10⁴ 실측에서 단일 라벨 컷은 깊은 사람 회수 0, 다중 라벨 컷은 100/100이 depth ≥ 3. 알려진 결함 — 태그가 아주 많은 깊은 사람은 컷 아래로 간다(depth 4 중 4/35, 전부 태그 11–12개); 그것을 위한 것이 후보 2다 | 실측 | 2026-09-04 |
| **D24** | **surface 1 인스턴스의 item은 `agent_id#top_topic_id`다** — agent 1명이 보유 최상위 topic마다 item 하나, 태그는 그 최상위 + 그 아래 보유 하위만(`gorse.md` §12-4의 11). S3이 id를 agent로 되돌리고 S4가 agent 기준 dedup. surface 2의 `ItemId = personal_agent_id`(D03)는 그대로 | `gorse.md` §12-5 X6: `√wsum(d)`에서 다른 주제가 빠져 recall 34 → **94**(Gorse, 25만 agent), 상위 100의 66명이 최상위 2개 이상 — 폭 중립. 비용은 item 배수(합성 1.88, 실값은 X2(b)에서 "공개 보유 유저의 평균 최상위 topic 수"). topic-api는 모든 topic을 한 최상위 아래에 보인다(`ancestor_path`) | 실측 | 2026-09-04 |
...
### C7 — catalogue projection 지연의 성격 `해소 경로 확인 — 남은 값은 X3`
...
지연 = **사이클 길이**이고, 그 값은 `GOR-X3`이 준다. X3 뒤 이 항목을 닫는다.
...
**→ `GOR-X1` 완료 → `D23`**(2026-09-04, 오너: "1 채택, 2는 X4 뒤"). 다중 라벨 + 센티넬이 M2를 막고 깊은 사람을
최상위로 올린다는 것이 실측됐다(10⁴ 축소 확인 포함). 태그가 아주 많은 깊은 사람이 컷 아래로 가는 결함은 후보 2가
맡고, 그 채택은 X4 뒤다.

**→ 시작과 뒤집는 조건은 `D22`로 닫혔다**(2026-09-03, 오너: 7+9로 시작, 여유율 50%). **남은 것 둘**: ⑴ `N*`의
숫자 — `GOR-X3` 실측 끝(2026-09-04), **제안 5×10⁶은 오너 확정 대기**(박스 크기·가시성 목표) ⑵ ①(컷 품질) 쪽 보완 — 1은 `D23`으로
끝났고, **`GOR-X4` 실측 끝(2026-09-04)**: 후보 2는 기각 근거(단독이면 depth-4 선택, 다중에 얹어도 불변), 후보 4는 무효(`cache_size` 1,000에도 38/100),
후보 3은 다중과 결합해 S=100에서 1.0(후보 10⁴, 규모 비례). 단독 다중 쿼리(D23)는 최대 depth 100/100이지만 **전원 태그 6개** — 컷이
 focus 필터로 작동. **오너 결정(2026-09-04)**: 깊이에 희귀 하위·폭을 넣되 (i) D23 유지 — 샤딩(ii)은 오버헤드, 우리 인덱스(iii)는 지연. 개선은 **후보 11**(agent를 최상위
topic별 item으로 — `√wsum(d)`에서 다른 주제가 빠짐, 정확 계산 recall 99/100, 후보 3은 쓰지 않음)이고 **`GOR-X6` 통과**(2026-09-04: Gorse recall 94/100, 정확 99/100; 상위 100의 66명이 최상위 2개 이상 — 폭 중립; 비용 item ×1.88). **→ `D24`**(2026-09-04, 오너 채택). **남은 것은 `N*` 하나**다 — 그것이 정해지면 C11을 닫는다. 실행 순서는
`gorse.md` §12-6.

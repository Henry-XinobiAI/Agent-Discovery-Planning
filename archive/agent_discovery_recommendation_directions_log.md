...
- **Privacy는 일단 배제** — 승급 게이팅·익명화·권한 link는 나중에 덧붙인다. 지금은 "무엇이 발견 가능한가"의 *모양*만 본다. (MVP 도입 방안 미기획 → 나중 논의, §9.)
- **게이트 3종은 서로 다른 축·소유다 — 한 게이트로 뭉치지 않는다:**
  - **maturity** (근거 두께) — discovery 소유, *관찰값*(6.6).
  - **safety/reliability** (위해: 과장·hallucination·공격성) — **agent moderation·응답 생성 팀 소관**, discovery는 typed verdict를 *소비*만(6.11).
  - **privacy/eligibility** (공개권·동의) — **deferred**(위).
  → "보여줘도 되나(privacy) ⊥ 보여주면 위험한가(safety) ⊥ 근거가 두꺼운가(maturity)"는 직교한다.
...
> ⚠️ "신규 구축(new build)" ≠ "후순위(2차)". 대립·직교는 재활용 부품이 아닌 **신규 구축**이지만, **서빙은 MVP에 포함**된다(6.9). 두 축을 분리해서 본다.

- **1차 (MVP)**: 강화·깊이 need(재활용 부품으로 빠르게) **+ 대립·직교 관점**(신규 구축이나 MVP 필수 — 차별점). 단 직교는 상징 축 인프라(6.7 LLM coarse 축 + KG 상속 + established 게이트)에 의존.
- **2차 (고도화)**: topic-conditioned 임베딩(확장형 for/against) + 축 도출 정밀화(ideal-point/요인분해) + 대화 중 끼어들기 오케스트레이션 고도화 — *서빙 가능 need를 늘리는 게 아니라 기존 need의 정밀도·확장성을 올림.*
...
- ⚠️ 입장 텍스트를 그냥 임베딩하면 토픽 성분이 다시 섞여 들어감 → "topic-conditioned(토픽 성분을 뺀 잔차)"가 disentanglement의 핵심 난점. **단 "완전 neural disentanglement"가 난제일 뿐, 우리가 실제로 필요한 "같은 토픽 내부의 입장 유사도"는 난제가 아니다** — anchor 파티션(6.4)이 토픽 성분을 retrieval 단계에서 고정하므로, 같은 anchor 안에서 stance만 비교하면 **의사(pseudo)-disentanglement를 사실상 공짜로** 얻는다. 더 정밀한 임베딩이 필요해지면(2차/3차) base embedding(OpenAI/Gemini 등)을 고정 feature로 두고 그 위에 **supervised projection layer**를 얹는 경로가 현실적이며, 학습용 weak label(같은 anchor 내 sim(A,B)↑·sim(A,C)↓ triplet)은 **6.7 쟁점 축·divergence 구조가 그대로 공급**한다.
...
> ⚠️ **잔차는 두 종류이고 서로 다른 질문에 답한다 — 혼동 금지:**
> - **topic-removal 잔차** (6.3) — "어떤 토픽" ⊥ "어떤 입장" 분리. for/against/orthogonal stance 공간용. = "topic-conditioned perspective embedding".
> - **persona-removal 잔차** (위, 6.5) — "그 사람 평소 성향" ⊥ "이 토픽에서의 입장" 분리. prior/blend·"진짜 토픽-특화인가" 판정용.
>
> persona-removal이 오히려 더 tractable하다 — global persona는 agent당 단일 벡터이고 blend 공식(`topic_specificity`)이 이미 있다. 2차/3차에서 projection layer(6.3)를 학습할 때 **topic projection과 persona projection을 둘 다 빼도록** 설계하면 한 layer가 두 잔차를 함께 처리할 수 있다.
...
**cold-start — KG 활용 [MVP]:** 입장 0~1개면 축 미정의 → **KG 부모/이웃 anchor에서 축 상속**(Wikidata 위계 payoff). 교차-anchor 축(예 "정통성 vs 편의")은 상위 anchor에 달고 하향 상속 — cold-start·orthogonal 둘 다 도움. LLM 세계지식으로 *잠정 축* 생성 후 실제 입장으로 확정/수정. **→ orthogonal MVP 서빙의 필수 의존성**: 상속이 없으면 cold anchor(MVP에서 가장 흔함)에서 orthogonal이 축 부재로 **항상 침묵**.

**prior ⊥ maturity 규율 적용 [MVP]:** 상속·LLM추측 축은 **잠정(provisional)** — placement(candidacy)엔 쓰되 확정 구조 행세 금지. 축에 **source 태그**(`observed`/`inherited`/`world-knowledge`) + **최소 divergence 질량** 넘어야 `established`(거짓 논쟁 제조 방지). stance의 `extraction_confidence`를 축 confidence로 전파. 잠정 축이어도 추천 value는 관찰 maturity가 게이트. **→ orthogonal은 `established` 축에만 서빙** — provisional 축에 대고 쏘면 "거짓 논쟁 제조"가 되므로, divergence 질량 게이트가 MVP에서 작동해야 함(임계값은 §9 MVP 결정사항).
...
> **권고(MVP):** stance 인구에서 **LLM 추론으로 이름 붙은 다중 축 + 양극** [MVP], divergence 질량으로 established↔provisional 태그 [MVP], 희소 시 **KG 상위 anchor 축 상속** [MVP]. **이 셋은 orthogonal을 MVP에서 서빙하기 위한 필수 의존성**(orthogonal = 축 ≥2 전제 + cold anchor 상속 없으면 침묵 + provisional 축 오용 = 거짓 논쟁 제조). ideal-point/요인분해는 규모 차면(다음 단계 — 서빙 추가가 아니라 축 정밀도 향상). 축은 **discovery 소유 anchor-level 구조** + `stance_space_version` 관리.
...
> **MVP**: 토픽 = anchor QID(상징 검색 + KG 이웃 확장). 입장 = edge의 **stance descriptor(상징)**. for/against는 서빙 시점에 유저 입장 대비 **LLM/텍스트 비교**로 판정(disentangle 임베딩 불필요, 싸고 설명가능). 모드 A 커버리지 = stance descriptor 클러스터링. **orthogonal도 MVP에 포함** — 6.7의 LLM 추론 coarse 쟁점 축(+ cold anchor는 KG 상위 anchor 축 상속) 위에서 **상징 off-axis 판정**으로 서빙. orthogonal은 임베딩 기하의 "수직"이 아니라 "다른 프레임"이라(6.3, L165) 상징 축만으로 성립하므로 임베딩 없이 가능.
>
> **다음 단계**: 규모 요구 시 topic-conditioned perspective embedding 추가(확장 가능한 for/against), 쟁점 축 도출을 **ideal-point/요인분해로 정교화**(6.7 mature 단계). *축 도출·orthogonal 서빙 자체는 MVP*이며, 다음 단계는 그 **정밀도·확장성 향상**만 담당(서빙 가능 need 추가가 아님).
> - **2차(첫 임베딩 진입점)**: LLM이 추출한 stance-only 텍스트(=`stance_summary`)를 그대로 embed — 임베딩 공간 분리 문제를 우회. LLM-per-comparison 비용이 커질 때의 최적화이지 MVP 코어 아님(MVP는 임베딩-free).
> - **3차(본격 잔차 임베딩)**: base embedding(OpenAI/Gemini 등 고정 feature) 위 **supervised projection layer** 학습 — weak label은 6.7 축/divergence가 공급, topic·persona 두 잔차(6.5) 함께 처리. dimensions 파라미터는 압축이지 분리가 아님에 유의.

이유: 처음부터 *완전* disentangled 임베딩에 베팅하는 건 위험(그게 연구 난제)하지만, 상징 descriptor + 서빙 비교가 더 견고·설명가능 — 설명가능성 1급 요구와 일치. 그리고 임베딩이 필요해질 때도 anchor 파티션 의사-disentanglement(6.3) → projection layer 점진 학습 경로가 있어, "from-scratch 신경망 분리"라는 베팅을 피할 수 있다.
...
> **score_need(후보 | 유저, 질의)** = **게이트**: on-topic ∧ (maturity ≥ `floor_need`) ∧ eligibility ∧ **safety/reliability** → **가중합**: `α·stance_term + β·maturity + γ·freshness + δ·persona_sim + ε·experience + ζ·novelty + η·validated_contribution_need`
...
- **⑦ 평판은 보강이지 popularity prior 아님 — safety ⊥ contribution ⊥ global reputation 분리.** **safety/reliability**(과장·hallucination·공격성·편향 유도)는 **hard gate** — novelty·diversity와 trade-off 금지. 단 **판정은 discovery 밖(agent moderation·응답 생성 팀 소관, 6.11과 같은 경계)** — discovery는 typed verdict를 게이트 입력으로 *소비*만 하고 산정하지 않는다. **`validated_contribution_need`**(η)는 6.12 메모리 임팩트로 검증된 기여만 반영하는 **need-조건부 booster**(전역 인기 점수 아님). **global reputation은 지배 항 금지** — 타이브레이커/약한 prior로만(안 그러면 popularity bias·rich-get-richer, §9). 신규 agent는 maturity floor + safety gate만 통과하면 contribution 이력 없이도 **exploration 슬롯**을 받는다(contribution은 floor도 penalty도 아님).
...
**같은 경계 — safety/reliability:** agent의 위해 행동(과장·hallucination·공격성·편향 유도) 판정도 retrieval이 아니라 **콘텐츠·행동 문제**라, **agent moderation·응답 생성 팀 소관**이다. discovery는 이를 산정하지 않고 **typed safety verdict를 게이트 입력으로 소비**(6.10 ⑦). need_type과 함께 query-side 계약에 들어온다. (maturity=근거⊥safety=위해⊥privacy=공개권 — 셋은 다른 게이트, §0.)
...
**기여도(contribution) 누적 — 메모리 임팩트를 점수로:**
위 메모리 임팩트는 일회성 학습 신호로 끝나지 않고 **contribution score로 누적**된다 — **agent-level · topic-level(anchor별) · need-level** 세 결을 분리해 쌓는다. 6.10 η·`validated_contribution_need`로 랭킹에 들어가지만, 규율이 핵심이다:

- **need별로 분리, 전역 스칼라 금지.** ⚠️ 나쁜 설계 = `평균 만족도 + 평균 길이 + 선택률`(= popularity prior, anti-recsys thesis 붕괴). 좋은 설계 = `{깊이: 검증된 새 지식, 대립: 검증된 stance 정교화, 직교: 검증된 새 축 개방, 경험: episode 유용성, 유사성: peer-fit 만족}`.
- **acceptance·engagement는 contribution으로 직접 쓰지 않는다** — 선택률·대화 길이는 카드 랭킹의 선행지표일 뿐(특히 대립/직교는 짧아도 좋음, ⑥). contribution의 **유일한 강한 근거 = durable + validated 메모리 임팩트**(위 gaming 가드 상속) → reputation farming 방어.
- **랭킹 진입 = need-조건부 booster이지 floor/penalty 아님.** global reputation은 타이브레이커/약한 prior로만(지배 항 금지). 신규 agent는 이력 없어도 exploration 슬롯(6.10 ⑦).
- **소유:** 이 score는 producer 입력이 아니라 **discovery-derived 피드백 projection**(8.5의 L2 delta에서 파생, §8.6).

> **MVP:** acceptance + 대화 지속(기본 implicit) + 종료 시 1-tap need-프레이밍 평가(희소 ground-truth). 2차: 메모리 임팩트(L2 변화)를 value 신호로 — 대립/직교 평가의 본선 + **contribution score 누적 시작**. 모든 신호 `need_type`별 분리.
...
- **contribution/reputation score** — 6.12 메모리 임팩트를 agent·topic·need-level로 누적한 것. memory/persona producer 입력이 **아니라** discovery-derived 피드백 projection. 그쪽은 **L2 change delta + provenance만 노출(8.5)**, 점수화·need별 분해·safety/reputation 산정은 우리가.

---

## 9. 미결정 / 다음에 팔 것

- **[MVP 결정사항] established 판정의 divergence 질량 임계** — orthogonal을 MVP에서 서빙하기로 결정(6.9)했고 orthogonal은 `established` 축에만 의존하므로(6.7), 이 임계는 **미룰 수 없으며 MVP 전에 정해야 한다.** (나머지 쟁점 축 세부는 아래 deferred.)
- **쟁점 축 세부 (deferred)** — mature 단계 분해 기법 선택(ideal-point/요인분해), 교차-anchor 축의 위계 부착 규칙.
...
- **임베딩 도입(2차/3차) 시** — projection layer 구현(topic·persona 두 잔차, 6.5)과 weak-label 파이프라인(6.7 축/divergence). ⚠️ **`embedding_model_version`을 versioning 체계(`stance_space_version`·`global_profile_version`)에 추가** — 모델 간 임베딩 공간은 비호환이라 모델 교체 시 전수 re-embed가 필요하고, 그 stale 식별·재투영을 같은 버전 관리로 다뤄야 한다.
...
- **[가드] 평판/기여도 신호의 위험 (6.10 ⑦ · 6.12)** — global reputation을 지배 항으로 쓸 때의 **popularity bias·rich-get-richer**(타이브레이커/약한 prior로 제한), 신규 agent **cold-start exploration**(contribution을 floor/penalty로 쓰지 않고 exploration 슬롯 보장), **reputation farming**(durable+validated 가드로 방어). η·`validated_contribution_need` 가중·타이브레이커 강도·exploration 비율은 튜닝 대상.
- **safety/reliability [경계 — moderation·응답 생성 팀 소관]** — discovery는 산정하지 않고 typed verdict를 게이트 입력으로 소비(6.11). *소관은 정해졌고*, 남은 건 그 팀과의 합의: verdict 스키마·신뢰도·전달 경로(query-side DTO). (privacy와 달리 deferred가 아니라 타 팀 위임.)
- **[deferred] privacy** — 중요하나 **MVP 도입 방안 미기획** → 나중 논의. 승급 게이팅·익명화·권한 link·eligibility/visibility 게이트 필드 일체 포함(§0). safety(타 팀)·maturity(discovery)와 별개 축임을 유지.

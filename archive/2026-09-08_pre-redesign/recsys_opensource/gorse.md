...
>
> **§1–§10은 `문서` 근거, §11은 `실측`, §12는 `소스` 근거의 규모 분석과 개선 후보다(rev 3).** §12의 열한
> 가지 중 **1·7·9·11은 채택**(`D23`·`D22`·`D24`), 2 기각·3 미사용·4 무효(오너 2026-09-04), 나머지는 후보다 — 실험은 §12-5의 `GOR-X1`–`X6`(전부 완료). 남은 결정은 `N*`(`decisions.md` C11).
...
1. **`SURV-R4` — 실제 보유 데이터에서 §11-2 경로 C의 품질.** 지금 근거는 합성 데이터(6명 → 10⁴ → 5×10⁵, §12-5)까지고
   **실데이터는 미검증**인데 `D07`이 후보 생성 전체를 여기 맡긴다. **이것이 최우선이다.**
...
**경로 C — 라벨별 개별 쿼리 + 가중 합산** (기존 S3 형태 — `D07`; 다중 라벨 쿼리는 위 B + 센티넬이 `D23`으로 추가됨)
...
| **focus** | "그 사람 보유 전체 중 이 subtree 비중"을 주지 않는다. 하위 강조 시 generalist가 narrow를 앞지르는 원인. **surface 1 인스턴스는 `D24`(item = agent × 최상위 topic)로 이 편향을 우회한다** |
...
이 실험은 2,000개, 상위 라벨당 보유자 250명까지만 갔다 — **그 너머는 `GOR-X3`이 5×10⁵까지 실측했다**(아래·§12-5).
...
> **이 절은 후보안이다** — 단 7·9는 `D22`(2026-09-03, 오너), 1은 `D23`(2026-09-04, 실측), 11은 `D24`(같은 날, 실측)로 채택됐다. 나머지의 채택은 `decisions.md`가 하고,
> 지금은 C11(열린 항목)이다. 앞 절과 달리 근거가
> 실측이 아니라 **v0.5.11 소스**다 — 그래서 12-1을 먼저 적고, 그 위에서 문제 둘과 개선 후보 열한 가지를 세운
...
전 게이트인 이유가 이것이고, 6명 fixture로는 보이지 않으며 **10⁴ 이상 합성에서 그대로 재현된다**(X1·X4).
...
아니라 **72시간 동안 아예 안 보인다**(`decisions.md` C7의 실체 — 해소 경로는 9(`D22`), X5로 확인).

규모(X3 실측 회귀): 사이클 ≈ 0.4 ms × N(8 jobs) → 10⁷ item에 ~67분, 10⁸에 ~11시간; RAM ≈ 0.42 GB + 8.3 KB × N → 10⁷에 83 GB, **10⁸에 830 GB — 한 박스에 안 든다**. "끝나지 않는다"는 아니지만 **매 사이클 그 비용을 내고, 사이클 길이가 곧 새 agent의 가시성 지연**이다.

### 12-4 개선 후보 열한 가지 (채택: 1 `D23` · 7·9 `D22` · 11 `D24` / 오너 2026-09-04: 2 기각 · 3 미사용 · 4 무효 / 5·6·8은 후보)
...
| 2 | **파생 태그 `T#deep`** — 자손 k개 이상 공개한 사람에게 프로젝션이 붙임 | 희소 태그 → IDF 높음 → 분자 큼. 모집단이 작아 컷 안에 다 들어옴. `D13`(파생은 우리 것) 정합 | (agent, 상위 topic)당 태그 1개. 임계값 k 튜닝 축 | 태그가 늘어 `√wsum(d)`가 커짐 — 1과 같이 쓰면 상쇄 | X4 → **X4: 기각 근거** — 단독이면 depth-4를 고르고(`√wsum`), 다중에 얹어도 순서 불변 |
| 3 | **뜨거운 라벨만 쿼리 item 샤딩** — agent에 `coffee~s`(s=hash mod S), 쿼리 item S개 | 쿼리 item은 우리가 만드는 것. S개 병렬 조회 → S×100 후보. S는 복제본의 보유자 수에서 자동 | 뜨거운 라벨에 한해 agent당 태그 1개, 쿼리 item S개(hidden). 조회 S개 병렬 | 샤드 안 100은 여전히 `√wsum(d)` 순 — 1·2와 조합 필요 | X4 → **X4: 통함, 미채택** — 다중 라벨과 결합해 S=50에서 0.87(Gorse), S=100에서 1.0(정확 계산). 필요한 슬롯(S×100) ≈ 최대-depth 보유자 수 × 2.7, 규모에 비례. 오버헤드로 쓰지 않는다(오너 2026-09-04) |
| 4 | `cache_size` 상향 | S2 | **전역**이라 전 item 캐시가 커짐 | 단독으로는 불가. 7과 묶이면 가능 | X3: 캐시는 item당 ~80건·~210 B(sqlite) — 상향은 이 두 곱을 키운다. **X4: 무효** — `cache_size` 1,000에도 정답 38/100 |
...
| **7 `D22`** | **surface 1 전용 Gorse 인스턴스** — tags item-to-item만, user·feedback 없음 | `D09`(store는 파생물)가 프로젝션 둘을 허용. surface 2의 유저별 캐시(560 GB)와 분리되어 4가 가능해짐(단 4는 X4에서 무효) | 배포 둘 (운영 ~1.3배) | 빌드 자체 | X3 |
...
| **11 `D24`** | **agent를 최상위 topic별 item으로 나눈다** — surface 1 인스턴스의 item = `agent_id#top_topic_id`, 태그는 그 최상위 + 그 아래 보유 하위만 | S3: `√wsum(d)`에서 **다른 주제의 태그가 빠진다.** 점수가 `√commonSum × commonCount` 꼴이 되어 depth·희귀 하위 순, 폭은 중립. X6(25만 agent): 정확 계산 99/100, Gorse 94/100 | item 수 × (agent당 보유 최상위 수; 합성 1.88, 실값은 복제본 D10에서) → RAM·사이클·캐시 같은 배수, `N*`를 그만큼 나눔. S3이 id를 agent로 되돌림, S4가 agent 기준 dedup. Gorse 변경 0, 쿼리 수 그대로 | 하위 조합이 같은 사람끼리의 동점(S5의 일). 최대-depth가 100명 넘을 때의 컷 자체 | **X6** → **통과**(Gorse 94/100, 정확 99/100; 25만 agent) |
| 10 | (한계) | 9까지 해도 **사이클 시간 ≥ 전면 HNSW 빌드**. 6·7·8이 줄이고 **5만이 없앤다** | | | |

`판단`(D22 이전 논거 — 기록) 순서는 **결정이 필요 없는 것부터**: 1(실험 한 시간) · 6(측정) · 8(설정값). Gorse에 남는다면 7 + 9 +
...
문제 ②가 아니라 문제 ①(컷 품질)의 것이다. 그 뒤 실측: 1은 X1 통과로 `D23`, 2는 X4에서 기각 근거, 3은 통하지만
오버헤드로 미사용, 대신 11이 `D24`(오너 2026-09-04).

### 12-5 실험 설계 — `GOR-X1`–`X6`
...
센티넬 `__QUERY__` 포함. 조회는 `GET /api/item/{query-id}/neighbors/?n=100`(server API — 첫 item-to-item 설정을 서빙한다). 재계산 강제는
...
**결과 (2026-09-04)** — 통과 기준 넷 중 셋 충족, 하나는 기준 자체가 틀렸다. **→ `D23`**(같은 날, 오너: "1 채택, 2는 X4 뒤").
...
- 다중 쿼리에서 빠진 depth 4 네 명은 **모두 태그 11–12개**(generalist형). 그 자리를 태그 4–6개의 depth 3이 채웠다.
  이것이 위 ✗ 항목의 규모판이다. → 오너 결정(2026-09-04, X4 결과): 넓은 사람도 깊으면 들어와야 한다. 이 네 명을
  회수하는 것은 후보 2(`#deep`, X4에서 기각)가 아니라 **후보 11(`D24`, agent × 최상위 item)**이다.
...
- **(a) 지금 — 시나리오.** `I_eligible = U × r`로 두고 `U = 10⁸`(목표), `r ∈ {1%, 5%, 20%}` → 10⁶ · 5×10⁶ ·
  2×10⁷. 태그 수 분포는 §11 fixture의 1–12를 Zipf로 잡는다. **이 세 점은 X3의 회귀식에 대입하는 점이고(실측 N은
  10⁵·3×10⁵·5×10⁵), X3이 돌려주는 것은 임계값 `N*`** — Gorse 사이클·RAM이 한계를 넘기 시작하는 item 수. 그러면 C11의 질문은 "I_eligible이 얼마인가"에서
  **"우리 목표 규모가 `N*`보다 큰가"**로 바뀌고, 그건 표본 없이 기획이 답할 수 있다.
- **(b) 출시 후 — 계측.** 공개 보유 복제본(`D10`)이 우리 것이므로 **topic-api 협의 없이** 센다: 공개 topic ≥1인
  user 수, 그 분포(p50·p95·max), 주간 증가, **공개 보유 유저의 평균 최상위 topic 수**(`D24`의 item 배수 — 합성 1.88). 이 값(× item 배수)이 `0.5·N*`에 닿으면 **새 C 항목**을 연다(§12-6 "출시 후").
...
- **합성 데이터**: N ∈ {10⁵, 10⁶, 10⁷} (실측은 10⁵·3×10⁵·5×10⁵ — 아래 결과). 라벨 3천 개(카탈로그 규모), 라벨 인기 Zipf(s≈1 — `coffee`류 뜨거운
...
확인 구간은 Redis에 달림). `N*`는 **item** 수다 — `D24` 뒤 item = agent × 최상위 topic이므로 뒤집는 조건은 **`I_eligible(agent) × item 배수 ≥ 0.5·N*`**, 합성 배수 1.88이면
agent 약 1.3×10⁶. 오너가 ⑴ 박스 크기 ⑵ 가시성 목표(C7)를 정하면 이 식에 넣어 `D22`에 숫자로 적는다. `SURV-R3`은 "5×10⁵까지 실측, 그 위는 회귀"로 갱신했다.
...
#### X4 — 컷 안의 깊은 사람 비율 (후보 1·2·3 비교; SURV-R4의 핵심) `X3의 5×10⁵ 세트 재사용`
...
**결과 (2026-09-04)** — **단독 쿼리로는 어느 조건도 0.9에 못 미친다. 샤딩 × 다중 라벨은 S=100에서 1.0.** → 오너 결정 2026-09-04(아래).

환경: X3의 5×10⁵ 세트(X3의 10⁶ 세트는 VM 한계로 없음), n_jobs 8, cache store MongoDB 7(저장이 빨라서), `fit_period = "4m"`
(적재 중 사이클이 시작되지 않게). **coffee 보유자 135,553명**, 하위 9개 중 보유 수(depth) 분포 0:40,936 · 1:38,122 · 2:27,535 ·
3:16,183 · 4:9,067 · **5:3,710**(최대 — 생성기가 하위를 최대 5개까지 준다).

**정답(GT100)**: 설계서대로 depth(하위 보유 수) 내림차순. 그런데 depth 5가 3,710명이라 상위 100은 동점 규칙이 정한다 —
**희귀 하위 IDF 합 내림차순 → 총 태그 적은 순**을 썼다(분석의 선택; 실서비스의 "깊이"가 정해지면 바꾼다). GT100은 전원 depth 5,
태그 수 6–12개(6개 34명, 7개 이상 66명).

| 조건 | 쿼리 수 | 후보 | **recall@GT100** (정확 / Gorse) | 후보의 depth | 비고 |
|---|---|---|---|---|---|
| (a) 단일 라벨 coffee + S | 1 | 100 | **0 / 0** | 전부 0 | 태그 하나짜리 40,936명 동점 |
| (b) 다중(coffee + 하위 9) + S — `D23` | 1 | 100 | **34 / 34** | **전부 5** | 100명 전원 **태그 6개**(coffee + 하위 5, 다른 주제 없음) |
| (c) `#deep`(하위 ≥ 4) 단독 + S — 후보 2 | 1 | 100 | **0 / 0** | **전부 4** | 태그 적은 depth-4가 depth-5를 이김 |
| (f) 다중 + `#deep` + S | 1 | 100 | 35 / 34 | 전부 5 | (b)와 같음 — 파생 태그가 순서를 못 바꿈 |
| (d) 샤딩 S=50 × 단일 | 50 | 4,401 | 0 / 0 | 0·1 | 동점 수천 명이라 HNSW가 100 미만을 돌려준 샤드 있음 |
| (e) 샤딩 S=50 × 다중 — 후보 3 | 50 | 5,000 | **89 / 87** | 4·5만 | 슬롯 50배 |
| (e) S=100 × 다중 (정확 계산만) | 100 | 10,000 | **100** | 3·4·5 | depth-5 3,686/3,710 포함 |
| (e) S=200 × 다중 (정확 계산만) | 200 | 20,000 | 100 | 2–5 | depth-5 전원 |

Gorse와 정확 공식은 recall에서 ±2 안에서 일치한다. (a)·(c)·(d)의 "HNSW≈exact" 겹침이 낮은 것은 근사 오차가 아니라 **동점 수천 명 안의
순서**가 다른 것이다(depth 분포는 같다). 이웃 조회 지연은 MongoDB에서 4–36 ms(sqlite·§11-2의 1.1 ms와 다름 — store 값).

**(b)의 정답 순위 분포**(정확 계산, 원본 세트 DS0): GT100의 (b) 순위는 최소 1 · 중앙값 1,491 · 90퍼센타일 5,443 · 최대 7,324.
`cache_size`를 올리면 — 200: 35 · 500: 36 · **1,000: 38** · 2,000: 65 · 5,000: 82. **후보 4(`cache_size` 상향)는 이 문제의 답이 아니다.**

**읽히는 것**:

- **다중 라벨 쿼리의 컷은 depth 필터가 아니라 focus 필터다.** 공식 `commonSum·commonCount/√wsum(d)`에서 depth가 포화하면(다들 하위 5개)
  남는 변수는 `√wsum(d)` — 다른 주제가 없는 사람이 위다. (b)의 100명이 전원 태그 6개인 이유. "깊이가 같으면 누가 위인가"는
  본래 **우리 S5의 질문**인데, Gorse의 컷이 "태그 적은 사람"으로 미리 답해 버린다.
- **후보 2(`#deep`)는 기각 근거가 나왔다.** 단독이면 `√wsum`이 depth-4를 고르고, 다중에 얹으면 IDF 한 항이 추가될 뿐 순서가 안 바뀐다.
  파생 태그는 recall 장치지 랭킹 장치가 아니다.
- **후보 3(샤딩)은 슬롯을 곱하는 것이라 통한다.** 필요한 슬롯(S×100) ≈ 최대-depth 보유자 수 × ~2.7(`√wsum`이 그들을 순위에 흩어 놓아서).
  여기선 3,710명 → S=100(10,000 슬롯)에서 1.0. **규모에 비례한다** — I_eligible 5×10⁶에서 coffee의 depth-5는 ~37,000명이라
  S≈1,000, topic당 쿼리 1,000개, 후보 10⁵를 S4·S5가 받아야 한다. 뜨거운 라벨에만 켜는 전제(12-4 행 3)가 그래서 붙는다.
- 정답 정의를 **depth만**으로 두면 (b)는 100/100이다(전원 최대 depth). 0.9 미달은 "최대 depth 안에서 희귀 하위·폭을 어떻게 볼 것인가"라는
  동점 규칙에서 나온다. 그 규칙을 정하는 것이 곧 이 실험의 판정이다.

**정할 것이었던 것 → 오너 결정(2026-09-04)**: 깊이에 희귀 하위·폭을 넣는다. 셋 중 **(i) `D23` 유지**를 택했다 — (ii) 샤딩은 얻는 것에 비해
오버헤드가 크고, (iii) 우리 인덱스는 지연 문제가 생긴다는 판단. 단 **이 구조 안에서의 개선**을 찾았고, 그것이 후보 11(agent를 최상위
topic별 item으로)이다 — X6에서 정확 계산 99/100, Gorse 94/100(25만 agent) → `D24`. 후보 2는 기각, 후보 4는 무효, 후보 3은 쓰지 않는다.

<details><summary>재현: 하네스 요지</summary>

```text
DS0 = 5×10⁵ agent 원본 + 쿼리 qa(coffee,S) · qb(coffee + 하위 9, S)   (DS = 데이터셋 표지, 결정 D와 무관)
DS1 = 같은 agent에 coffee 보유자마다 coffee~<md5 mod 50>, 하위 ≥ 4면 coffee#deep 추가(12,777명)
     + 쿼리 qa qb qc(coffee#deep,S) qf(coffee+하위9+coffee#deep,S) qd0..49(coffee~s,S) qe0..49(coffee~s+하위9,S)
각각 별도 컨테이너: 적재 ~80 s → fit_period 4m 뒤 첫 사이클(Push ~1 min + 저장 ~9 min, MongoDB) → 쿼리별 GET n=100
정확 계산: 12-1 S3 공식을 파이썬으로(IDF 풀 = agent + 쿼리 item, hidden 포함). S=100·200은 정확 계산만.
주의: fit_period 1m이면 적재 중 사이클이 시작돼 부분 데이터셋으로 계산되고, sqlite data store와 겹치면 POST가 분 단위로 막힌다.
```
...
#### X6 — agent × 최상위 topic item (후보 11; X4의 후속) `15분`

- **왜**: X4가 보인 focus 편향은 `√wsum(d)`에 agent의 모든 태그가 들어가서다. 나누는 단위는 **쿼리가 떨어지는 단위** — 우리 트리는
  하위 ⇒ 상위이고 topic-api는 모든 topic을 하나의 최상위 아래에 보인다(`ancestor_path`가 다중 부모에서도 한 경로를 고름 —
  `topic/catalog/flatten.py`, README "Tree depth"). 라벨마다 나누면 item 태그가 1개가 되어 `commonCount` 신호가 죽고(item 수 ×4.6 — agent당 평균 태그 수),
  최상위보다 굵게 묶으면 묶음 안에서 편향이 돌아온다. 최상위가 오버헤드 최소이면서 편향이 사라지는 유일한 단위다.
- **fixture**: X3의 5×10⁵ 세트를 (agent, 최상위)로 쪼갠 것 — item ≈ 938,547개(×1.88). 쿼리 qa·qb는 X4와 같다.
- **측정**: qb의 상위 100 — recall@GT100(X4의 정답, depth → 희귀 하위 → 폭), 상위 100의 태그 수·보유 최상위 수 분포, Gorse vs
  정확 공식 겹침. 부수: X3 하네스 값(Push·저장·RSS)이 item 수 배수대로 늘어나는지.
- **통과**: recall ≥ 0.9(X4 기준 그대로) ∧ 상위 100에 보유 최상위 2개 이상인 agent가 있음(폭이 벌점이 아니라는 증거).
- **실패의 뜻**: 정확 계산(99/100)과 Gorse가 다르다는 것 → HNSW가 ×1.88 item에서 달라졌는지 확인. (통과했고 → `D24`. item 형태는
  `D24`가 정한다 — `D23`은 쿼리만 정한다.)

**결과 (2026-09-04)** — **통과.** recall 94/100(기준 0.9), 상위 100 중 66명이 최상위 topic 2개 이상 보유. **→ `D24`**(같은 날, 오너 채택).

환경: X4와 같은 하네스·MongoDB cache store(`--wiredTigerCacheSizeGB 1`), `fit_period 4m`. **첫 시도는 5×10⁵ agent 전체(item 938,547)로
했고 Gorse가 OOM(exit 137)으로 죽었다** — X3의 기울기 `0.42 GB + 8.3 KB × N`이 주는 8.2 GB가 VM 7.7 GB를 넘는다는 예측 그대로.
그래서 **앞 25만 agent**(coffee 보유자 67,783명, depth 5가 1,899명, item **468,838개 = 1.88/agent**)로 돌리고, 기준선(D23 현재 형태)도
같은 부분집합에서 정확 계산으로 다시 뽑았다.

| 25만 agent 부분집합 | recall@GT100 (depth → 희귀 → 폭) | 폭 무시 | 상위 100의 depth | 상위 100의 태그 수 | 상위 100의 보유 최상위 수 |
|---|---|---|---|---|---|
| D23 현재 (agent = item 1개), 정확 계산 | **34** | — | 전부 5 | 전부 6개 | 전부 1개 |
| 후보 11, 정확 계산 | 99 | 100 | 전부 5 | 6개 33 · 7–12개 67 | 1개 33 · 2개 38 · 3개 20 · 4개 9 |
| **후보 11, Gorse** | **94** | 94 | 전부 5 | 6개 34 · 7–12개 66 | 1개 34 · 2개 38 · 3개 20 · 4개 8 |

- Gorse와 정확 계산의 상위 100 겹침은 94/100. 차이 6은 **컷 경계의 동점**(하위 조합이 같은 item은 점수가 같다 — X4에서 100·101위가
  동점이었다)에서 나온 순서 차이이고, 분포(depth·태그 수·최상위 수)는 같다. 근사 오차가 아니다.
- **폭이 벌점이 아니라는 증거**: 상위 100의 66명이 최상위 topic을 2개 이상 갖고 있다. D23 현재 형태에서는 0명이었다.
- 단일 라벨 쿼리(qa)는 여전히 쓸모없다(전부 depth 0). 후보 11이 고치는 것은 다중 라벨 쿼리의 컷 구성이다.
- 비용 실측: item 468,838개의 첫 사이클 = 6분 16초(Push + MongoDB 저장), X3 단가와 일치. 즉 **비용은 정확히 item 배수(1.88)**다.
- 같은 agent가 한 쿼리에 두 번 나온 경우 0 — 합성 데이터에 다중 부모가 없어서다. 실데이터에서는 topic-api의 `ancestor_path`가 한
  경로를 고르므로 최상위가 하나로 정해지고, S4의 agent 기준 dedup은 안전장치다.

**채택으로 바뀐 것 → `D24`**: surface 1 인스턴스의 item은 `agent_id#top_topic_id`, 태그는 그 최상위 + 그 아래 보유 하위. S3이 id를
agent로 되돌리고 S4가 agent 기준 dedup. `N*`(item 수)를 agent로 환산할 때 item 배수로 나눈다(합성 1.88 — 실값은 X2(b)에서 "공개 보유
유저의 평균 최상위 topic 수"로 센다). 반영 완료: `decisions.md` D24·D03 주석 · 기준 문서 §S3(rev 5.16) · `recsys_opensource/README.md` §3 ·
코드 레포 `docs/recsys-intent.md`. `inbound_event_contract.md`는 수신 계약이라 해당 없다(프로젝션 모양은 D24와 §S3이 갖는다).


### 12-6 실행 순서와 결정 지점 — **compact 뒤에는 여기서 시작한다**

**현재 상태 (2026-09-07)**: 채택 `D22`(7+9) · `D23`(1) · `D24`(11). 기각·미사용 2·3·4. 실험 X1–X6 전부 완료. **열린 것은 `N*`의 숫자 하나이고, 오너가 2026-09-07에 Closed·Open Beta 뒤로 미뤘다**(공개 보유 비율·item 배수의 평균이 실측돼야 추론 가능). 그때까지 이 절의 할 일은 없다 — D22의 뒤집는 조건은 `N*` 대신 master RSS·사이클 길이를 직접 본다(D22 주석, 제안). C7은 `N*`와 같이 닫는다. C12(분석 출처 결정 재협의)는 별건.

**2026-09-03 시점 상태**(기록): `D22` 채택(7+9로 시작, 여유율 50%). 열린 것은 C11의 잔여 둘 — `N*`의 숫자(X3), 문제 ①
...
`N*` 제안 5×10⁶(64 GB 박스)은 **오너 확정 대기**(X3 결과의 계산법에 박스·가시성 목표를 넣으면 숫자). **X4 완료**(같은 날, 5×10⁵ 세트) — 단독 쿼리는 전부 0.9
미달(다중 34/100, 전원 최대 depth지만 전원 태그 6개), 샤딩×다중은 S=50에서 0.87, S=100에서 1.0. 후보 2는 기각 근거, 후보 4는 무효.
오너 결정(같은 날): 깊이에 희귀 하위·폭을 넣되 **(i) D23 유지**, 개선은 **후보 11**(agent × 최상위 item)로 — **X6 통과**(Gorse recall 94/100, 폭 중립 확인, 비용은 item 배수 1.88). **→ `D24`**(같은 날). 남은 것은 `N*` 하나.
...
|---|---|---|---|---|
| 1 | **X1** 경로 B + 센티넬 — **완료 2026-09-04** | 없음 (한 시간) | 12-4 행 1 → **`D23`** | §11-2 "B + 센티넬" 블록 · `decisions.md` `D23`(출처 실측) · 기준 문서 §S3 rev 5.15 — 완료 |
| 2 | **X5** 쿼리 item 세대 재생성 — **완료 2026-09-04** | 없음 | 9의 두 확인(삭제 시 캐시 정리 · 세대 전환 원자성) | 12-4 행 9에 확인 결과 · 캐시 정리 절차가 필요하면 §12-5 X5 아래 · C7에 "해소 경로 확인" |
| 3 | **X3** 대규모 빌드 — **실측 완료 2026-09-04(5×10⁵까지), `N*` 제안 → 오너 확정 대기** | 합성 데이터 생성기 — 완료(`gen3.py`, 실측 N = 10⁵·3×10⁵·5×10⁵) | **`N*`** | `storage_sizing.md` S4 단가 · `SURV-R3` 갱신 — 완료. **`D22`에 `N*` 숫자 — 오너 대기** |
| 4 | **X4** 컷 안 깊은 사람 비율 — **완료 2026-09-04**(5×10⁵ 세트) | X3의 5×10⁵ 세트 | 12-4 행 2·3 (통과 기준 recall ≥ 0.9) — 단독 쿼리 전부 미달, 샤딩×다중 S=100에서 1.0. **오너 결정: 깊이에 희귀 하위·폭 포함, (i) D23 유지 + 후보 11** | 2 기각·3 미사용·4 무효를 12-4 행에 · `SURV-R4` 갱신 — 완료 |
| 4-1 | **X6** agent × 최상위 item (후보 11) — **통과 → `D24`** 2026-09-04 | X4 | Gorse 94/100(정확 99) → 후보 11 → **`D24`** | 12-4 행 11 · `decisions.md` D24·D03 · 기준 문서 §S3 rev 5.16 · X2(b) 계측 추가 — 완료 |
| 5 | — 결정 (`N*`) — **Closed·Open Beta 뒤(오너 2026-09-07)** | 1–4-1 + 베타 실측 | **C11·C7 닫기** | `D22`에 `N*` 숫자 · `decisions.md` C11·C7에 "해소" · 기준 문서 §S3에 최종 형태 한 줄 |

**출시 후**(실험이 아니라 운영): 우리 복제본에서 `I_eligible`을 주간으로 센다(X2(b)). `N*`가 없는 동안은 master RSS(박스의 50%)와
정상 사이클 길이(가시성 목표의 50%)를 직접 보고, `N*`가 정해진 뒤에는 `0.5·N*`로 읽는다 — 같은 조건의 두 표기다. 닿으면 `D22`의
...
**하지 말 것**: 2·3·4를 채택으로 쓰지 않는다(2 기각·3 미사용·4 무효 — 오너 2026-09-04). `N*`는 오너 확정 전이니 D22의 조건을 확정 숫자로 쓰지 않는다(제안값은 제안이라고 적는다).
결과를 이 문서 밖(대화·scratchpad)에만 남기지 않는다.

### 12-7 이 절이 하지 않는 것

- **1·7·9·11 외에는 채택하지 않았다**(`D23`·`D22`·`D24`; 2 기각·3 미사용·4 무효). C11의 잔여는 `N*`.
- **지연 예산을 정하지 않았다**(`decisions.md` C9). X3의 판정 기준 "목표 가시성 지연"은 C7이 정할 값이다(C9는 `/recommend` 응답 지연 예산으로 별개).
- **저장 규모를 다시 계산하지 않았다.** surface 1 전용 인스턴스(7)의 이웃 캐시 단가(item당 ~80건, X3)는
  `storage_sizing.md` S4에 넣었고, 단계별 총량(상한)은 그 문서 §8이 `D26`의 단계로 적었다 — 실값은 X2(b)의 `I_eligible`·item 배수가
  나오면 바뀐다. Redis에서의 건당 크기는 미측정(`STO-S10`).

...
| `archive/Agent_Discovery_Recommendation_Roadmap.md` (보관됨) | Alpha / Open Beta / Post-Open-Beta 단계 — 현재는 `impl/11-forward-roadmap.md` |
| **이 문서** | 구현 접근 / 서비스 구성 / memory-api 경계 / build vs reuse / 후보 기술 |

구현 서비스명은 **`bourbon-agent-recommendation-api`**다. 설계·범위는 위 두 문서를 참조하고 여기서 재진술하지 않는다. 단계 정의(Alpha/Open Beta/Post)와 내부 단계(Discovery/Recommendation)는 directions §0·§5, `impl/11-forward-roadmap.md`(구 Roadmap §1, 보관됨)를 따른다.
...
| 5 | **Serving** | Recommendation | 후보(`agent_id`)·이유·evidence_refs payload, push silence 판정 | — (build) |
...
request(topic_text, need_type, user_stance_ref?)
  ...
  ├─④ need ranking       need별 ordering            PersonaProvider.get_prior() [mock] + lexicographic ordering [build, agent ordering LLM 없음] + stance 분류 [build/LLM]
  ├─⑤ serving            payload(agent_id·evidence_refs) + silence 판정                          [build]
...
**현행 경계는 5개 Protocol이며 정본은 [impl/02](impl/02-provider-boundary.md)다.** 아래는 초기 4개 seam의 **언어중립 sketch**(확정 시그니처가 아니다)에 현재 `KnowledgeEntityProvider` 계약을 반영한 것이고, 이후 추가된 다섯 번째 `StanceEvidenceSearchProvider`(④a stance 단계 — recall-only, 실패는 503이 아니라 **사유 붙은 침묵**)는 여기 복제하지 않고 impl/02를 따른다. mock과 real은 같은 인터페이스를 구현한다(§7.1).
...
```
search_candidates(text, limit?)   -> [EntitySummary{ qid, label, importance, pageview, … }]                # POST /knowledge/entities/search (Page→.items)
search_entities(queries[], instance_of?, fanout?, limit?) -> [EntitySummary...]                          # 〃 멀티-쿼리(ANY-match) + 클래스 필터
get(qid)                          -> Entity{ qid, label, labels, aliases, linked_qids, … }               # GET /knowledge/entities/{qid} (bare)
expand_connections(qid, limit)    -> EntityConnections{ center, broader, narrower, links_out, links_in }  # GET /knowledge/entities/{qid}/connections (wire는 그룹별 NeighborPage → provider가 투영)
```

`KnowledgeEntityProvider`는 full entity linker가 아니라 **memory-api entity substrate adapter**다 — entity 검색(`search_candidates` 단일 / `search_entities` 멀티)·단건 조회·connections 호출을 감쌀 뿐 disambiguation을 결정하지 않는다. Query-side linker(모듈 1)가 candidate generation을 `search_candidates`(search-only)로 모으고, 필요 시 별도 entity linker(§4.1)를 조합해 최종 QID를 정한다(구현 계획에서는 `EntityCandidate`로 정규화 — build_plan §3.1). `suggest`는 **양쪽에서 제거 완료** — memory-api가 #87로 라우트+struct를 삭제했고, Discovery의 `suggest()`/`EntitySuggestion`도 뒤이어 제거됐다(원래도 linker recall 미사용 D2라 런타임 무영향). **어떤 메서드도 `lang`을 노출하지 않는다** — entity 검색은 body `queries[]`(1–16, 각 ≤200자, ANY-match) + `fanout`(0–50) + `limit`(1–100, 기본 20) + `instance_of`(QID ANY-match)만 받고 `lang`이 없다(계약 drift 방지). 원래 `lang`을 가졌던 `search_articles`는 표면 전체가 **제거됐다**(코드 `32d8cc6`): 소비자가 0개였고 memory-api PR #92가 그 라우트를 `POST /knowledge/articles/search`로 바꿔 이미 404였다.

`expand_connections(qid, limit)`의 `limit`은 **그룹마다 따로** 적용된다(memory-api `le=100`). 과거의 "associative links만 제한하고 taxonomy는 내부 cap"이라는 의미론은 **memory-api PR #92에서 폐기됐다.** 같은 PR이 응답 wire도 바꿨다 — 그룹마다 page 봉투를 두고, 그 안의 item을 typed edge로 감싸고, 다섯째 그룹 `typed_out`을 더 싣는다:
...
Discovery는 이 세 겹을 provider-private wire 모델로 검증하고 `EntityConnections`(center + 4그룹 `list[EntitySummary]`)로 **투영**한다. 그래서 도메인 타입에는 `limit`/`truncated`가 **없다** — 그룹별 봉투에서 request 단위 cap을 합성하면 응답이 더는 담지 않는 사실을 진술하게 되기 때문이다. `typed_out`은 검증만 하고 투영하지 않는다(소비자 없음). 상세 = [impl/02](impl/02-provider-boundary.md).
...
get_edges(anchor_id) -> [AgentTopicEdge{ agent_id, anchor_id,
                          ...
                          source_owner }]   # routing_target 제거(계약): agent_id만 반환·dispatch는 bourbon-api 런타임
```
...
|---|---|---|---|
| `KnowledgeEntityProvider` (topic→QID 검색, entity 단건, connections) | `bourbon-memory-api` `/knowledge/entities*` | real / pinned `anchors.json` | **real** |
...
| API | 용도 | Discovery 적용 |
|---|---|---|
| `POST /knowledge/entities/search` → `Page[EntitySummary]` | body `queries[]` ANY-match full-text 검색, `importance` 정렬 | topic → QID 후보 생성 |
| ~~`GET /knowledge/entities/suggest`~~ → ~~`Page[EntitySuggestion]`~~ | ~~autocomplete~~ | **양쪽 제거 완료** — memory-api #87이 라우트/struct를 삭제, Discovery의 `suggest()`/`EntitySuggestion`도 제거됨. grounding은 search-only라 무영향 |
...
- **`POST /knowledge/entities/search` → `Page[EntitySummary]`** (`.items` 언랩; body = `queries[]` + `fanout`/`limit`/`instance_of`); 각 `EntitySummary`: `qid`, `source`, `label`, `description`, `importance`, `pageview`, `pagerank`, `sitelink_count`, `categories`, `instance_of`, `abstract`.
- **~~`GET /knowledge/entities/suggest`~~ → ~~`Page[EntitySuggestion]`~~** — **양쪽 제거 완료**: memory-api #87이 라우트와 struct를 삭제했고, Discovery의 `EntitySuggestion`/`suggest()`도 providers/base·entity_http·structs/entity에서 제거됐다. grounding은 search-only라 런타임 무영향.
...
- **`GET /knowledge/entities/{qid}/connections`** — wire는 `{center, broader, narrower, links_out, links_in, typed_out}`이고 **center를 뺀 다섯 그룹이 각각 `NeighborPage{items, limit, truncated, total}`**, 그 `items`는 `NeighborEdge{relation, entity}`다(`entity`가 `EntitySummary`). top-level `limit`/`truncated`는 **없다**(PR #92에서 그룹별 봉투로 이동). Discovery의 도메인 타입 `EntityConnections`는 이걸 투영한 `center` + 4그룹 `list[EntitySummary]`뿐이다.
...
|---|---|---|
| `anchor_id ↔ owner_id` raw recommendation signals (→ `agent_id`는 edge assembly에서 파생) | 없음 | `MockMemoryEdgeProvider` fixture로 선대체 |
...
| ~~`routing_target`~~ | 계약에서 제거 | recommendation은 `agent_id`만 반환·dispatch는 bourbon-api 런타임 해석. Current code still carries routing_target on AgentTopicEdge / RecommendationItem; remove in the implementation pass. |
...
| 영역 | 후보 | 용도 | 현재 판단 | 주의점 |
|---|---|---|---|---|
| **rule-based filtering + lexicographic ordering + symbolic/text feature extraction** | (직접) | need(depth/experience/for/against/coverage)별 **filter + ordering key 산출**(build_plan §4.2 ordering contract) — coarse score·aggregate score 아님 | **Alpha 핵심** | symbolic descriptor 중심. **agent ordering엔 LLM 미개입 + aggregate/weighted serving score 금지**(build_plan §4.2 불변식). LLM은 **stance classification(§4.3)·linker rerank(build_plan §4①)·B2 judge(§8)** 에만 위치하고 **agent rerank/scoring에는 쓰지 않는다** |
...
| 단계 | 가져올 것 | 미룰 것 |
|---|---|---|
| **Alpha** | Entity linking, Wikidata/KG access, structured edge store + filter, **rule-based filter + lexicographic ordering**(agent scoring 아님 — §4.2), linker rerank·stance classification용 LLM, decision-log 설계 | RecSys framework, vector/ANN(임베딩 전), LTR, **agent rerank/scoring LLM·aggregate score** |
...
| **B. 외부에서 받음** (Memory / Persona / owner·privacy) | observed/prior stance, maturity raw signal, agent-topic edge, discoverability | **contract로 받고 mock으로 선대체**(§7). 무엇을 받는지의 source of truth는 **directions §9** — 여기서 재진술하지 않는다. (`routing_target`은 계약에서 제거; `agent_id`는 bourbon-api가 `owner_id`에서 파생) |
...
- 필드별 **source owner** (Memory / Persona / owner / privacy / safety) — 모든 필드에 `source_owner` 태그를 붙인다. `discoverability`·safety verdict는 Memory 본체가 아니라 owner/privacy/safety 소유일 수 있어, 책임 경계가 흐려지지 않게 한다. (`routing_target`은 계약에서 제거됨 — bourbon-api 런타임 해석. directions §9.3 observed=Memory / prior=Persona, §11 eligibility와 일관)
...
- **B2 silver judge 운영 원칙**(human gold 대체가 아니라 보조라서 검증이 필요하다, §8.11):
  ...
  - **leniency 보정 — 절대 점수 평균을 쓰지 않는다.** LLM judge는 후하게 주는 경향(leniency bias)이 있으므로, ranking 비교는 절대 점수 평균이 아니라 **pairwise preference**(decision-log의 순위쌍 기준 — 절대 품질 스칼라를 만들지 않는다)나 pass/fail rubric으로 받고, **B1 calibration set 대비 drift**를 함께 본다. 이는 build_plan §4.2 *불변식*(aggregate scalar은 측정용 — 서빙 score/objective로 굳히지 않는다; atomic feature를 key로 쓰는 것과는 별개)과 같은 정신이다.
...
- **coverage — unique axis / source / persona coverage, redundancy.** 결과 리스트가 덮는 고유 축/출처/페르소나 수와 중복도. → coverage need에서 비슷한 agent만 반복(same-axis crowding)하지 않고 의미 있는 범위를 덮는가. redundancy↑면 다양화 프리미티브(MMR/DPP 등) 필요 신호 — 단 이는 **Open Beta 후보**이고 **Alpha serving 로직 아님**(Alpha coverage serving은 round-robin, MMR/DPP는 §4.2에서 Open Beta로 미룸).
...
**judge 버전은 ratchet의 분모다 (B2 baseline 비교 무효화 규칙).** B2 silver judge로 측정한 값에 ratchet을 걸 때, **judge prompt·model·version을 versioned artifact로 고정**하고 eval result metadata(+ decision-log `provider_versions` 류)에 남긴다. 이 셋 중 하나라도 바뀌면 *측정 단위*가 바뀐 것이므로 **이전 B2 baseline과 직접 비교를 금지**한다 — 반드시 ① **B1 human anchor set으로 재측정**하고 ② **새 judge version 기준으로 ratchet threshold를 재설정(reset)**한다. 안 그러면 judge 업그레이드가 거짓 통과(점수가 올라간 게 품질이 아니라 judge가 후해진 것)나 거짓 회귀를 낸다. **B2는 regression detector이지 truth source가 아니다** — B2 점수 단독으로는 quality gate를 통과시키지 않는다(품질 주장은 B1만, §8.1; leniency 보정·pairwise는 §8.2). 이는 measurement validity(judge를 truth로 굳히기 전에 검증) 문헌의 실무 번역이다(§8.11). **운영상 judge=생성 단계 / metric=소비 단계로 분리한다**: B2 silver는 pinned judge version으로 **미리 생성해 persist**한다(Phase 8). Phase 7이 세운 metric layer는 phase 순서상 B2를 기다리지 않고(그 자체는 deterministic gold로 돈다), 이후 Phase 8의 persisted B2 silver 라벨을 소비할 수 있다 — **`eval run`은 기본적으로 live judge를 호출하지 않는다.** 런마다 judge를 새로 호출하면 judge가 흔들려 offline 결정론과 ratchet 비교가능성이 함께 깨지기 때문이다. 반대로 `must_not_show` false-pass·discoverability-off exposure·known-item needle top-1 같은 **hard 게이트는 judge가 아니라 결정론적 gold 라벨에서** 계산한다(확률적 judge에 안전·노출 게이트를 걸지 않는다). Phase 6 `eval run`은 judge/지표 없이 실행·커버리지 요약만 낸다(build_plan §6).
...
**for/against = perspective-aware retrieval task.** for/against는 쿼리-후보의 의미 유사도가 아니라 *관점*(axis + direction + stance)을 retrieval에 반영하는 과제다(perspective argument retrieval과 정렬, §8.11). gold/strata를 이 관점축으로 짜되, 비용이 다르므로 **milestone(Alpha-active/OB-shadow)**과 **gold 비용(human-gold/synthetic-negative)**을 함께 태깅한다 — 안 그러면 B1 사람 라벨링 부담이 조용히 몇 배가 된다:

| perspective stratum | 설명 | milestone | gold 비용 |
|---|---|---|---|
| same topic / same stance | for 정답(같은 축·같은 방향) | Alpha-active | **B1 human gold**(비쌈) |
| same topic / opposite stance | against 정답(같은 축·반대 방향) | Alpha-active | **B1 human gold** |
| semantically similar / wrong stance | 의미상 가깝지만 방향이 틀린 hard negative | Alpha-active | **synthetic negative**(`must_not_show`, 쌈) |
| high-expertise / stance-mismatched | 전문성은 높으나 stance 불일치 — expertise가 stance filter를 못 이김을 검증 | Alpha-active | **synthetic negative** |
| adjacent topic / useful orthogonal | 다른 축이지만 유용한 관점 | **OB-shadow** | 코퍼스 태그만(Alpha gold 라벨 ❌) |

for/against positive(정답) 두 strata(same-stance=for, opposite-stance=against — **둘 다 정답이지 same-stance만 B1이 아니다**)만 B1 사람 더블라벨이 필요하고, 두 negative strata는 §8.3/guard 역산으로 **합성 구성**되어 사람 비용이 없다(`must_not_show` false-pass 직격). `adjacent/orthogonal`은 orthogonal need가 Open Beta 조건부 서빙이므로 **Alpha gold에서 제외**하고 코퍼스 태그만 둔다 — **서빙하지 않는 need를 Alpha human gold로 라벨링하지 않는다**(비용 대비 근거 약함).
...
**LLM rerank prompting & perspective retrieval — §4① linker rerank·for/against strata의 *개념 정렬* 근거(구현 의존성 아님)**

이 그룹은 "이 논문을 구현한다"가 아니라 *설계 개념을 외부 용어로 정렬*하는 용도다(구현 의존성 ❌). 아래 세 편은 arXiv ID를 검증했고, 그 외(prompt-injection on rankers 등)는 본문 미독으로 see-also에 둔다.
- Pradeep et al., *RankZephyr: Effective and Robust Zero-Shot Listwise Reranking Is a Breeze!* (2023). zero-shot listwise rerank가 point/pairwise보다 안정 → **linker rerank를 listwise로 프레이밍**(단, agent ordering이 아니라 ① topic→QID 후보에만, build_plan §4①/§4.2). [arxiv.org/abs/2312.02724](https://arxiv.org/abs/2312.02724)
- Zhuang et al., *A Setwise Approach for Effective and Highly Efficient Zero-shot Ranking with LLMs* (SIGIR 2024). setwise 프롬프트로 비교 횟수·토큰비용↓ → rerank 프롬프트 설계 참고. [arxiv.org/abs/2310.09497](https://arxiv.org/abs/2310.09497)
- *Overview of PerspectiveArg2024: The First Shared Task on Perspective Argument Retrieval* (2024). 의미 유사도가 아니라 *관점*을 retrieval에 반영 → **for/against를 perspective-aware retrieval task로 정의**(§8.7 perspective strata). [arxiv.org/abs/2407.19670](https://arxiv.org/abs/2407.19670)
- (see also, 본문 미검증 — 개념 참고만) LLM ranker는 후보 텍스트에 주입된 지시에 흔들릴 수 있다(prompt injection on rankers) → §8.3 `rerank prompt injection` 가드로 방어·검증.

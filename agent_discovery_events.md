...
| `bourbon.user_deactivated` | `user_id` | 그 유저의 공개된 row·agents·사전 계산·요청자 데이터 삭제. 상호작용 로그의 actor 쪽은 익명화. (2026-09-24 개정: 탈퇴한 id 하나는 `departed_users`에 남아 그 뒤에 도착하는 이 유저에 관한 모든 이벤트의 쓰기를 거절한다 — R68) |

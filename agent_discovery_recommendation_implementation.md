...
`AnchorSummary`·`Anchor`·`AnchorConnections`·`ArticleHit`만 real 응답(§2.6의 현재 API 표면)에 묶이고, 나머지 타입은 contract로 합의해 mock으로 채운다(§7.2). 필드별 `source_owner` 규칙은 §7.2.
...
| 이웃 anchor 확장(sparse) | `GET /knowledge/anchors/{qid}/connections` | **real** |

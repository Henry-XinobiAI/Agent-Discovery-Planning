...
> **역할**: 재설계 §5-1이 A안 후보로 적은 Gorse, `implicit`, LightFM, OpenSearch와 B안의 PostgreSQL 역색인·동시 등장 CF를
...
| LightFM 1.17 | `implicit`의 대안 | Python 3.14에서 **빌드 실패**(`setup.py`가 `__LIGHTFM_SETUP__` 속성을 못 놓음). 2021년 이후 릴리스 없음 | **후보에서 뺀다** |
| OpenSearch 2.19 (타입 ①②) | topic term 색인으로 커버리지 정렬 가능 | nested + `function_score`로 커버리지 우선 정렬·friends 필터 모두 **위반 0건**. 10만 유저 p50 2.3 ms / p95 3.9 ms. 단 점수 설계에 함정 하나(§2-2), 그리고 zstd 버그(§2-3) | 동작한다. 그러나 같은 일을 PostgreSQL이 더 빠르고 단순하게 한다 |
| PostgreSQL 17 (타입 ①②, B안) | `(topic_id, tier, agent_id, 점수)` 역색인 + GROUP BY | 위반 0건. **p50 0.8 ms / p95 2.3 ms**, 36만 row 27 MB | 타입 ①②는 이것으로 충분하다 |
...
| 동시 등장 아이템 이웃 (타입 ③ B안 CF) | 첫 구현 후보(O11) | **코사인 정규화 이웃은 무작위 수준**(HR@10 0.08 %/0.02 %). 정규화 없는 원 카운트 + 인기도 tie-break는 λ=10에서 인기도와 동률, λ=3에서 ALS의 절반 | O11: 아이템 이웃을 하려면 정규화 없이. 그래도 ALS 16 factor가 더 낫고 더 싸다 |
...
**합성 데이터 자체의 발견**: 스펙 §2 기본값으로 만든 정답(`affinity × pop`)은 너무 흐리다. 정답을 전부 아는 오라클조차 held-out 대화 상대를 top-10에 넣는 비율이 **1.9 %**다. CF 품질을 recall@10으로 가르려면 §5-2의 조정이 필요하다.

## 1. 무엇을 어떻게 돌렸나

- **모집단**: `synthetic_population_spec.md` §2 기본값으로 U=100,000. 단순화: 카탈로그는 3,003개 합성 topic id를 54 그룹에 배분(실제 카탈로그 파일 대신), 유저 id는 정수(agent = 주인 index), 이벤트 열 없이 최종 상태 + 대화 로그만. 결과: topic row 1,016,411 / 열린 row(public+friends) 366,211 / discoverable agent 56,276 / 친구 간선 372만(대칭화 뒤 평균 degree 74.5 — 스펙의 median 25는 유저가 "고르는" 수라 대칭화하면 두 배가 된다) / 대화 300,082건(λ=3, 서로 다른 쌍 238,287, 대화 0건 유저 5,000) / λ=10 판은 998,591건.
- **타입 ①②**: 열린 row를 PostgreSQL 테이블 하나와 OpenSearch nested 색인에 같이 넣고, 요청자 군집 선호에서 뽑은 topic 1~3개 질의 2,000건(타입 ①)과 topic 1개 질의 1,000건(타입 ②), 가장 흔한 topic 20개 질의 200건을 둘 다에 던졌다. 정답은 numpy로 따로 계산한 (커버 수 desc, 가중 합 desc) 목록. 검사 항목은 스펙 §4의 "friends row가 친구 아닌 요청자에게 나온 건수 0", 상위 20 안 커버 수 역전 0, 보유하지 않은 agent 반환 0.
...
| 질의 | n | PostgreSQL p50 / p95 | OpenSearch p50 / p95 | 위반(둘 다) |
|---|---|---|---|---|
| 타입 ① topic 1~3개 | 2,000 | 0.8 / 2.3 ms | 2.3 / 3.9 ms | friends 누출 0 · 커버리지 역전 0 · 미보유 0 |
...
친구 목록 5,000개를 배열로 넘겨도 p95가 2.3 ms다. friends 필터는 색인 구조 그 자체라는 §5-2의 주장이 맞다.
...
`Accept-Encoding`에 `zstd`가 들어가면 OpenSearch 2.19.0의 netty `ZstdEncoder`가 `UnsupportedOperationException: Direct buffers not supported`를 던지고 **연결이 응답 없이 닫힌다**(curl로 `zstd`만 넣어도 5 s 타임아웃, `gzip`·`br`은 정상). Python 3.14는 표준 라이브러리에 `compression.zstd`가 있어서 **urllib3 2.x와 aiohttp 3.13이 기본으로 `zstd`를 광고한다** — 코드 repo의 aiohttp 3.13.3에서 `client_reqrep.py`의 `_gen_default_accept_encoding()`이 `HAS_ZSTD`면 `zstd`를 붙인다. OpenSearch를 쓴다면 클라이언트에 `Accept-Encoding: gzip`을 고정해야 한다. 이번 실측도 그렇게 해서 돌렸다. AWS OpenSearch Service의 빌드에서 같은지는 확인하지 않았다.

### 2-4. 판단 재료

타입 ①②에 OpenSearch가 주는 것은 없고(같은 정답, 3배 느림, 운영 컴포넌트 하나 추가, 점수 설계 함정, zstd 함정) PostgreSQL 역색인은 재설계 §5-2가 적은 대로 충분하다. OpenSearch가 다시 후보가 되는 지점은 §5-2 표의 "topic 벡터 kNN(필요할 때만)"이지 term 색인이 아니다.
...
**천장을 먼저 본다.** 오라클조차 HR@10이 2 %다. 정답을 다 알아도 held-out 상대를 top-10에 넣는 비율이 그 정도라는 것은, 스펙 파라미터가 만드는 대화 상대 분포가 그만큼 넓다는 뜻이다. 그래서 HR은 절대값이 아니라 **인기도 대비**로, 그리고 **오라클을 천장**으로 놓고 읽는다.
...
| B: 동시 등장 원 카운트 | 0.0012 | 0.0009 | 0.014 | 0.212 | 0.0 s |
| B: 동시 등장 + 인기도 tie-break | 0.0030 | 0.0016 | 0.080 | 0.230 | 0.0 s |
...
| B: 동시 등장 원 카운트 | 0.0076 | 0.0042 | 0.187 | 0.325 |
| B: 동시 등장 + 인기도 tie-break | 0.0102 | 0.0056 | 0.273 | 0.308 |
...
- **동시 등장 + 인기도 tie-break**: ALS 16f와 거의 같은 프로필. 라이브러리 없이도 비슷하게 갈 수 있다는 뜻인데, 인기도 tie-break를 빼면(원 카운트 행) HR이 0.0076으로 떨어진다.
...
1. **오라클의 천장이 1.9 %인 이유.** 스펙 §2의 `affinity`(대각 1.0 / 부 군집 0.5 / 나머지 0.05 + N(0, 0.1) 잡음)와 Pareto(2) 인기도가 만드는 대화 상대 분포가 그만큼 넓다. 천장을 올리는 조정은 §5-2와 스펙 §7에 있다.
...
3. **λ=10이 되면 ALS 16f와 "동시 등장 + 인기도"가 인기도와 동률**이면서 군집 비율은 두 배다(0.27~0.31 vs 0.125). 개인화가 비용 없이 붙는 시점이 그 근처다. 실서비스 대화율이 어디인지가 CF의 가치를 정한다 — 스펙 §7 "실측 없음"이 여기 직결된다.
4. **아이템 코사인 이웃은 이 밀도에서 무작위 수준이고, 데이터가 늘어도(λ=10) 그대로다.** 코사인 정규화가 인기도를 완전히 지워 "한 번 같이 나타난 희귀 쌍"이 1위가 된다. O11의 "아이템 기반 이웃"을 하려면 정규화 없는 원 카운트여야 하고, 그것도 ALS 16f보다 못하거나 같다.
...
| Load Dataset | 3 s |
| Train Collaborative Filtering Model (BPR 100 epoch) | **19 s**. 내부 평가 NDCG@10 0.127 / Recall@10 0.244 — Gorse 자체 표본 평가라 §3 표와 비교 불가 |
...
- Gorse는 MySQL에 `ALTER TABLE documents ADD INDEX idx_collection_subset_categories (collection, subset, (CAST(categories AS CHAR(255) ARRAY)))`를 만든다. MySQL의 multi-valued 인덱스는 **빈 JSON 배열에 대해 인덱스 레코드를 만들지 않으므로**, `categories`가 `[]`인 행은 이 인덱스를 타는 질의에서 존재하지 않는 것처럼 보인다.
- 서버의 질의 `SELECT … WHERE collection = 'collaborative-filtering' AND subset = '<user>' AND is_hidden = false ORDER BY score DESC LIMIT 100`은 `EXPLAIN`에서 그 인덱스를 고른다. 같은 조건에 `FORCE INDEX (PRIMARY)`를 붙이면 102행, 인덱스를 타면 **0행**. 그래서 `collaborative` recommender가 빈 결과를 내고 폴백으로 넘어갔다.
...
Gorse의 CF는 같은 데이터에서 `implicit` ALS의 1/9, 인기도의 1/15다. Gorse 내부 평가가 보고한 Recall@10 0.24는 표본 negative에 대한 값이라 이 표와 비교할 수 없다.
...
- master 한 프로세스가 데이터셋을 든다(§12-1 S9).

## 5. 이 문서가 바꾸는 것

### 5-1. 문서 갱신 제안

- **재설계 §5-1 표**: LightFM 삭제. Gorse 행의 저장소를 "MySQL/PostgreSQL data store **+ MySQL/PostgreSQL cache store**(Redis는 Redis Stack 필요, ElastiCache 불가)"로. OpenSearch 행에 "term 색인 용도로는 PostgreSQL이 더 낫다(검증 §2), kNN 용도만 남김".
...
### 5-2. 합성 스펙에 넣을 것 (§7 "열린 것")

- 정답의 선명도. 지금 파라미터로는 오라클 HR@10이 1.9 %라 방법 간 차이가 0.1 % 단위에서 갈린다. 후보: `affinity` 비대각 0.05 → 0.01, 잡음 σ 0.1 → 0.03, 또는 recall@10을 "오라클 top-10과의 겹침"으로만 재기(이 문서가 그렇게 했다). 어느 쪽이든 §4 정답 정의에 적어야 두 구현안이 같은 기준을 본다.
...
- 실제 카탈로그 파일·라벨·LLM text→topic 단계. 이 문서는 색인·정렬·CF만 잰다.

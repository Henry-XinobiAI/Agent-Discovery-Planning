...
- **production identity resolution 계약 — 부분 해소**(2026-07-28, 코드 `9740bf2`). 구 제목 "~~+ partial-unmapped 정상 제외 계약~~"은 폐기한다: 파생 resolver는 애초에 unmapped를 반환하지 않고, **누락된 catalog row의 처리 정책은 아직 미정**(아래 잔여 2)이라 제목이 미정 정책을 확정된 것처럼 읽히게 했다. "해소"로만 읽어도 안 된다:
  ...
  - ⚠️ **잔여 1 — producer-side pin 미착지**(요청 B1, `bourbon_api_discovery_open_requests.md`). bourbon-api `tests/personal_agents/test_ids.py`는 결정성·구별성만 검사하고 `AGENT_NAMESPACE`도 known vector도 pin하지 않는다. Discovery 쪽 known-vector 테스트는 **consumer-side replica pin일 뿐 upstream drift를 감지하지 못한다** — 그쪽이 namespace/포맷 문자열을 바꾸면 **양쪽의 독립 단위 CI가 모두 통과**하고 cross-service integration/preflight 또는 실제 serving에서야 드러난다.

...
- **범위**: memory-api의 per-owner competence/grounding 신호를 Discovery `AgentTopicEdge`로 투영하는 **contract slice**. **dormant ship**(default-OFF). "Phase 10 완료"·"live turn-on"·"user-facing Alpha 성립"이 **아니다** — §9 blocker가 남아 있는 한 그렇게 부를 수 없다(2026-07-27: eligibility는 blocker에서 제외됐다, §7). **2026-07-28**: lifespan 배선이 착지해 ON이 실제로 부팅되지만 **dev/진단 stage 전용**이고 production stage는 startup에서 거부된다(§7 임시 가드). identity resolver는 **부분 해소**이며 잔여 2건(producer-side pin·phantom agent)이 §9에 남아 있다.
...
- **memory-api에 agent identity가 없다.** 정체성 키는 `owner_id`(UUID) 단독. `agent_id`는 bourbon-api의 canonical `personal_agent_id` 계약에서 파생된다. **~~Discovery가 파생 규칙을 복제하면 안 된다~~ → 2026-07-28 개정**: bourbon-api에 배치 resolve 계약이 없고(`GET /users/me/personal-agent`는 호출자 본인 전용) 추천 hot path에 네트워크 홉을 추가할 수 없으므로, **현 단계에서 그 UUID5 파생을 consumer-side로 복제한다**(`DerivedOwnerIdentityResolver`). Discovery의 known-vector 테스트는 **로컬 사본만 고정하며 upstream drift는 잡지 못하므로**, producer-side pin은 요청하지 않기로 종결됐다(구 B1, 2026-08-24 오너 결정 — §9 잔여 1 참조).
...
- **eligibility는 어떤 응답에도 없다**(전부 빌드타임 내부). → 진짜 갭은 eligibility 하나로 수렴한다. **dormant contract-slice 구현의 memory-api 블로커는 0**(eligibility는 AllowAll 정책·mock-first)이지만, **production turn-on은 여러 상류 계약에 blocked**(전부 §9): ① pagination/bounded-retrieval **+ join-integrity**(dangling link skip으로 `truncated=false`가 완전성 미보장, §6 ③), ② **stale-competence lifecycle**(active-set 변경/교체 시 과거 competence 잔존·`with_statements=false`로 감지 불가, §4), ③ **freshness 입력 의미**(`last_seen`이 evidence recency 아님, §4), ④ persisted link/entity ownership fold-검증(정책 ④). ⑤ **탈퇴(비활성) owner 라이프사이클**(memory-api에 수신 경로 자체가 없음 — 요청 R1, 2026-07-28 신규). **identity resolver**는 2026-07-28 부분 해소됐고 잔여는 §9 참조. (~~real eligibility~~ — 2026-07-27 오너 결정으로 blocker에서 제외: 도입 전까지 AllowAll이 승인된 임시 정책이다, §7.)
...
**lifespan 배선 (2026-07-28 착지, 코드 `c404eda`).** lifespan이 `REAL_EDGE_ENABLED=ON`일 때 네 인자를 모두 공급한다: `HttpOwnerTopicProjectionProvider.from_settings()`(생성·소유·종료 전부 lifespan) + `DerivedOwnerIdentityResolver` + `AllowAllEligibilityProvider` + provenance `memory-api-competence@v0` / `allow-all@v0`. 스위치는 플래그가 아니라 **projection 객체의 존재**다(stance가 `stance_search is None`을 쓰는 것과 같은 패턴). OFF면 네 인자 모두 `None`이라 Guard 0을 만족한다.

**"production에서 조용히 켜지는" 경로는 여전히 없다 — 이제 근거가 Guard 1이 아니라 전용 가드다.** 배선이 열린 이상 `STAGE=prod` + `REAL_EDGE_ENABLED=1` 두 줄이면 프로덕션이 real edge로 뜨므로, **같은 커밋에** `api.main._reject_real_edge_in_production()`을 넣었다: production stage에서 플래그가 켜져 있으면 startup `ValueError`(Guard 0/1과 같은 종류 — 셋 다 배포 설정 오류). 가드는 **어떤 client 생성보다 먼저** 실행되므로 차단 시 정리할 자원이 없다. 예외 메시지는 blocker를 열거하지 않고 **§9를 가리킨다**(열거하면 다시 stale해진다). **제거 조건: §9의 gate가 *전부* 닫힌 뒤 별도 커밋으로 함수·호출부·테스트를 삭제** — 그 커밋이 "프로덕션 turn-on 허가" 기록이다. ON은 그때까지 **dev/진단 stage 전용**이다.
...
- [x] **lifespan 실배선 테스트(2026-07-28, §7)** — ⓐ **dev/진단 stage에서 ON이 실제로 조립**된다: projection client + `DerivedOwnerIdentityResolver` + `AllowAllEligibilityProvider`가 주입되고 decorator가 lifespan이 만든 **그 client 인스턴스**를 쓰며 provenance가 `memory-api-competence@v0`/`allow-all@v0`로 verbatim 기록되고 종료 시 풀이 반납된다. ⓑ **prod stage + ON = 전용 가드로 startup `ValueError`**이며 그 시점에 **client가 하나도 생성되지 않았다**(가드가 모든 client 생성보다 앞). ⓒ **prod stage + OFF는 정상 부팅**(가드 과잉차단 방지 회귀). ⓓ **OFF면 edge projection client를 아예 만들지 않는다**(유휴 풀 0, provenance `unavailable@v0`). ⓔ 조립 실패 시 **세 client 모두** 종료된다. ⓕ `OwnerIdentityResolver` Protocol 적합성은 타입 대입으로 mypy가 증명, known vector 3쌍 pin(단, **consumer-side replica pin일 뿐 drift 미감지** — §9 잔여 1).
- [x] mypy/ruff/format clean, 전체 스위트 green, eval gate exit 0 — **2026-07-28 T2 시점 811 passed·2 skipped / mypy 189 files clean / eval gate EXIT 0 / `STAGE=prod`에서도 동일**(ambient STAGE 비종속). (2026-07-27 개정 시점: 797 passed / mypy 187 files.)

**구현 기록(코드 repo)**: 초기 슬라이스 = PR #24(main `f02c03b`). 2026-07-27 개정(브랜치 `chore/code-quality-cleanup`): `05f7f75` = §2.3 owner-space routing(`memory_owner_id` + `_evidence_key` 3곳 대칭 + 직렬화 경계 테스트, eval 코퍼스 `edges.json` 재생성, baseline md5 불변) / `0778d44` = §7 정책 개정(Guard 2·`stance_active` 제거, AllowAll docstring 재작성, assembler가 `StanceEvidenceSearchProvider` 포트에 의존). **2026-07-28 turn-on Step 1**(브랜치 `feat/phase-10-turn-on-step-1`): `9740bf2` = `DerivedOwnerIdentityResolver`(bourbon-api UUID5 파생의 consumer-side 복제 + namespace/known-vector pin) / `c404eda` = lifespan real-edge 배선 + **client 생성 이전에 실행되는 production stage 가드** + 배선 상태를 반영한 stale docstring 10곳 정정.
...
- **production identity resolution 계약 — 부분 해소**(2026-07-28, 코드 `9740bf2`). 구 제목 "~~+ partial-unmapped 정상 제외 계약~~"은 폐기한다: 파생 resolver는 애초에 unmapped를 반환하지 않고, **누락된 catalog row의 처리 정책은 아직 미정**(아래 잔여 2)이라 제목이 미정 정책을 확정된 것처럼 읽히게 했다. "해소"로만 읽어도 안 된다:
  - ✅ **해소** — `DerivedOwnerIdentityResolver`가 bourbon-api의 UUID5 파생을 복제해 owner_id→agent_id를 계산한다. 파생이 순수 함수라 **요청 집합에 대한 total mapping**이며(resolver mapping의 key 누락은 발생 불가), 그럼에도 decorator의 exact-mapping/collision 가드는 **유지**한다(Protocol이 다른 resolver도 받고, UUID5 collision은 기대하지 않을 뿐 불가능하지는 않다). canonical mapping을 반환하며 `owner_user_id` 소유 Agent를 열거하지 않는다 — (2)가 요구한 형태.
  - ~~⚠️ 잔여 1 — producer-side pin 미착지(요청 B1)~~ → **종결(2026-08-24, 오너 결정)**: pin 요청은 발신하지 않는다 — 파생은 bourbon-api가 지정한 **고정 계약**이며(기존 버전도 같은 방식으로 구현), 명문화(namespace 상수 `43d67b39-…`·known vector·세 repo 재검증)는 `recommendation_pipeline_design.md` S6-0, 레지스터는 `archive/bourbon_api_discovery_open_requests.md`로 보관. 원 문제의식(namespace/포맷 문자열 변경 시 양쪽 단위 CI가 모두 통과)은 사실로 남지만 — Discovery known-vector 테스트는 consumer-side replica pin일 뿐 upstream drift를 감지하지 못한다 — 계약이 고정으로 선언된 이상 turn-on blocker가 아니다.
  - ⚠️ **잔여 2 — phantom agent(실제 row 미검증)**. canonical agent ID의 *계산*은 끝났지만 **bourbon-api catalog에 그 row가 실제로 존재하는지는 아무도 확인하지 않는다.** 정상 등록 owner에게 row가 있다는 것은 bourbon-api lifecycle 계약의 *기대*이지 resolver의 보장이 아니다 — legacy 누락·데이터 손상·부분 이관이면 **존재하지 않는 agent를 가리키는 ID가 조용히 추천된다.** memory-api R1의 *비활성* 문제와는 **별개 경로**다(R1은 row가 있고 비활성, 이건 row 자체가 없음). 검증 위치 = **real-memory CLI preflight + 최종 turn-on acceptance**. 함께 확정할 것: **row 누락 시 처리 정책이 fail-loud인지 후보 조용히 제외인지**(미정 — turn-on 전 결정).
- **memory-api turn-on 요청 R1–R6** — 전문은 `memory_api_discovery_open_requests.md`(요청 레지스터). 아래 기존 항목과의 매핑:
  - **R1 탈퇴(비활성) owner 라이프사이클** — **신규 blocker**(2026-07-28 발견). bourbon-api는 탈퇴 시 `disable_for_user`로 agent를 비활성화하지만 memory-api에는 **수신 경로가 없다**(이벤트 리스너 0개·owner purge/tombstone 라우트 없음·owner 상태 필드 없음). 따라서 탈퇴자의 personal knowledge가 cross-owner 조회에 계속 노출되고 **대화 불가능한 agent가 추천된다**. Discovery는 파생이 비활성 여부를 모르므로 자체 해결 불가. 옵션 A(purge)/B(tombstone+기본 제외, 권장)/C(상태 노출+필터) 중 memory 팀 선택.
  - **R2** = attribution 필터 테스트 green(`STANCE_JUDGE_ENABLED` release gate). **R3** = 아래 pagination/join-integrity 항목. **R4** = 아래 freshness 입력 의미 항목. **R5** = 아래 persisted link/entity ownership 검증 항목. **R6** = 아래 stale-competence 항목.
- **requester self-exclusion 계약** — **production turn-on blocker**. 상태(2026-08-05) = **design resolved; implementation/acceptance pending**. 설계 정본은 [`2026-08-05-requester-self-exclusion-design.md`](2026-08-05-requester-self-exclusion-design.md)이며 **아래 (1)~(4)와 어긋나면 그쪽이 맞다**. gate는 아직 **닫히지 않았다** — ⓐ Discovery 구현 ⓑ 호출자 적용 ⓒ 비프로덕션 acceptance 셋 다 미완이고, 셋이 증명된 뒤 **별도 커밋**으로 닫는다(promotion 가드 제거와 같은 규율). 현재 파이프라인은 요청자 본인을 후보에서 제외하지 않는다(검증: `Query`에 requester identity 필드 없음, `verify_token`은 토큰 유효성만 검사하고 identity 미반환, Retriever의 축소(`_one_hit_per_direct_agent`·`_preempted_by_direct`)는 `agent_id` 기준으로만 접고·requester 미비교, Gate는 `eligibility_context`를 self-exclusion에 미사용). real edge를 켜면 요청자 본인이 해당 QID의 competence 조건을 만족할 때 자기 자신이 추천 후보로 나온다. turn-on 전에 확정할 계약:
  - **(1) requester identity 소스 = 신뢰된 내부 호출자의 on_behalf_of claim** (2026-08-05 개정 — 이전 서술 *"신뢰된 upstream만 설정 가능한 identity context"*를 대체). `/recommend`는 **외부 클라이언트에 직접 노출되지 않는 내부 서비스 API**다(공개 ingress 없음, Alpha·Open Beta 확정). 인증된 moderator/agent runtime이 자신이 **이미 인증한** 요청자의 owner_id를 `Query.requester_owner_id: UUID | None`(top-level typed 필드, `eligibility_context` dict 아님)로 전달하며 이 API는 그 claim을 **신뢰한다**. ~~현 shared bearer(`verify_token` → `API_TOKENS`)는 **내부 호출자 집합의 구성원임만** 증명하고~~ **정정(2026-08-11)**: 그 bearer는 **부착돼 있지 않다**(코드 `a693af3`, 오너 결정 — 형제 서비스도 같은 형태). 정본 정정은 [`2026-08-05-requester-self-exclusion-design.md`](2026-08-05-requester-self-exclusion-design.md) §3에 있고, 어긋나면 그쪽이 맞다. ⓐ 어느 컴포넌트인지 ⓑ 그 컴포넌트가 특정 owner를 대리할 권한이 있는지 ⓒ claim이 실제 세션 actor와 일치하는지는 bearer가 있던 시절에도 **증명하지 않았다** — 그 셋은 그대로 유효하며, 이 API가 쓰는 신원은 처음부터 요청 본문의 claim이므로 **self-exclusion은 무영향**이다. 따라서 안전성 근거는 여전히 **2층**이지만 ①이 얇아졌다: ① ~~네트워크 경계 + shared secret~~ → **네트워크 경계 단독**(매니페스트 가드로 검사 — `tests/test_k8s_manifests.py`) ② *"호출자는 외부 요청 body의 owner ID를 그대로 중계하지 않고, 자신이 인증·인가한 session/event actor에서 파생한다"*는 upstream 계약. **"유효하지만 틀린 owner ID"는 이 API가 판별할 수 없다** — (4)는 그 위의 completeness guard다.
  - **(2) 제외 입도 & 비교 방식** — 요청에 **requester owner_id**(신뢰된 upstream identity context)가 들어오고, candidate와 **동일한 production resolver**로 requester owner_id → requester agent_id를 resolve한 뒤 agent space에서 `edge.agent_id == requester_agent_id`로 제외한다. 양쪽이 같은 agent 네임스페이스이므로 **self-exclusion 목적으로 `owner_id`를 `AgentTopicEdge`/`Candidate`에 실을 필요가 없다**(새 중간 타입·side mapping 불필요; 추천 대상이 agent라는 점과도 정합). **주의(2026-07-27)**: edge에 `memory_owner_id`가 생겼지만 이는 **evidence routing 전용**이며 self-exclusion은 위 계약대로 agent space 비교를 유지한다 — 두 용도를 섞지 않는다(§2.3). **현행 bourbon-api canonical identity 계약은 `owner_id → personal_agent_id(owner_id)`의 결정적 1:1 매핑**(UUID5 기반 deterministic 함수·같은 owner는 항상 같은 ID·personal-agent lifecycle "active user마다 personal_agent 하나"·bootstrap이 그 canonical ID를 Agent PK로 사용)이므로 **1:many 분기는 현재 turn-on gate가 아니라 미래 계약 변경**이다 — 향후 owner당 복수 personal agent를 지원한다면 resolver 출력과 self-exclusion 입도를 함께 재설계해야 하지만, 이는 현재 분기가 아니다. 단 DB의 `Agent.owner_user_id`에는 unique constraint가 **없으므로** "DB가 owner당 Agent 하나를 강제한다"고 주장하지 않는다 — 1:1 근거는 deterministic `personal_agent_id` + personal-agent lifecycle 계약 + canonical ID bootstrap이며, **production resolver도 임의의 `owner_user_id` 소유 Agent를 열거하지 말고 canonical personal-agent mapping을 반환**해야 한다(2026-07-28 `DerivedOwnerIdentityResolver`가 그렇게 구현됐다). **다만 "서로 다른 owner는 반드시 다른 ID"라고 단정하지 않는다** — 정상 lifecycle에서 owner당 canonical personal agent 하나를 *기대*할 뿐이고, UUID5 collision은 기대하지 않을 뿐 수학적으로 불가능하지는 않다. 따라서 비정상 resolver 결과까지 포함해 **decorator의 collision guard는 유지**한다(§9 identity 항목과 동일한 서술). requester resolution 실패는 (4) fail-loud를 따른다. 어느 쪽이든 **순수 translator/`OwnerTopicProjection`에서 제거한다는 뜻이 아니다** — 현 `MemoryEdgeProvider.get_edges(anchor_id)`에는 requester context가 없으므로, 제거는 requester context를 받을 수 있는 **별도 orchestration/filter 경계**에서 일어나야 한다. **구체화(2026-08-05)**: 위 내용은 전부 유지되고, "동일한 production resolver"는 **composition root가 하나의 resolver를 만들어 edge projection·self-exclusion·e2e preflight 셋에 주입**하고 exact-resolution 검증(missing/extra/invalid/collision·count-only 로깅·식별자 비노출)을 **공유 helper로 고정**하는 것으로 구체화된다. 오늘은 `build_edge_wiring()`이 `DerivedOwnerIdentityResolver()`를 **자기 안에서 생성**하고(`api/depends/pipeline.py:101`) e2e preflight는 `personal_agent_id()`를 **직접 호출**하므로(`cli/e2e/preflight.py:206`) 값이 우연히 같을 뿐 공유가 아니다.
  - **(3) drop locus + audit 표현** — 반드시 **stance search 이전**에 제외한다(자기 자신의 statement를 불필요하게 조회하지 않도록). **정정(2026-08-05)**: 이전 서술 *"현재 Candidate-기반 drop 계약으로는 기록할 수 없다"*는 **층을 혼동했다**. 정확히는 — eligibility 이전 drop은 현재 **Candidate 기반 `DecisionLog.record()` 매퍼 API**로는 표현할 수 없지만, **직렬화 스키마인 `PoolEntry`(agent_id·anchor_id·via·via_qid)와 `DropEntry`(agent_id·anchor_id·reason)는 이미 `EdgeHit`만으로 구성할 수 있다**. 따라서 *"별도 audit 경로 vs `GateResult` 계약 변경"*은 실제 선택지가 아니었다: **`GateResult`를 변경하지 않고 pre-gate edge disposition 입력을 `DecisionLog`에 추가**한다. Gate 내부 phase-0 필터는 **기각**한다(Gate가 identity resolver·requester context를 알게 되고 eligibility 없는 drop을 `list[Candidate]`에 억지로 넣어야 한다) — 제외는 **파이프라인 ②retrieve ↔ ③gate 사이**에서 일어난다. 최종 audit reason은 `requester_self`로 구분한다.
  - **(4) fail-loud 규칙** — requester identity **누락은 no-op이 아니라 fail-loud**여야 한다(no-op은 identity 전달 누락을 조용히 허용해 turn-on 안전 규칙으로 부적합). **범위 한정(2026-08-05)**: 이것은 (1)의 2층 신뢰 위에 얹히는 **completeness guard**이며 *유일한* 안전장치가 아니다 — 막는 것은 **누락**이지 "유효하지만 틀린 값"이 아니다. 또한 판정 축은 stage가 아니라 **`REAL_EDGE_ENABLED`**다(dev/진단 real run도 같은 정책을 실행해야 e2e가 self-exclusion을 검증할 수 있다).
  - ~~optional requester ID를 지금 배관하면 위 (1)~(3)을 성급하게 고정하고…~~ → **해소(2026-08-05)**. 지연 근거였던 "클라이언트 입력이 인증 identity처럼 오용될 위험"은 (1)의 배포 구조 확인으로 사라졌다. 설계는 [`2026-08-05-requester-self-exclusion-design.md`](2026-08-05-requester-self-exclusion-design.md)에 있고, **turn-on gate 1은 "미지의 외부 설계 대기"에서 "호출자들이 필드를 채우기로 합의"로 축소**됐다 — 축소이지 해소가 아니다(위 ⓐⓑⓒ).

...
- **requester self-exclusion 계약** — **production turn-on blocker**. 상태(2026-08-05) = **design resolved; implementation/acceptance pending**. 설계 정본은 [`2026-08-05-requester-self-exclusion-design.md`](2026-08-05-requester-self-exclusion-design.md)이며 **아래 (1)~(4)와 어긋나면 그쪽이 맞다**. gate는 아직 **닫히지 않았다** — ⓐ Discovery 구현 ⓑ 호출자 적용 ⓒ 비프로덕션 acceptance 셋 다 미완이고, 셋이 증명된 뒤 **별도 커밋**으로 닫는다(promotion 가드 제거와 같은 규율). 현재 파이프라인은 요청자 본인을 후보에서 제외하지 않는다(검증: `Query`에 requester identity 필드 없음, `verify_token`은 토큰 유효성만 검사하고 identity 미반환, Retriever의 축소(`_one_hit_per_direct_agent`·`_preempted_by_direct`)는 `agent_id` 기준으로만 접고·requester 미비교, Gate는 `eligibility_context`를 self-exclusion에 미사용). real edge를 켜면 요청자 본인이 해당 QID의 competence 조건을 만족할 때 자기 자신이 추천 후보로 나온다. turn-on 전에 확정할 계약:
  - **(1) requester identity 소스 = 신뢰된 내부 호출자의 on_behalf_of claim** (2026-08-05 개정 — 이전 서술 *"신뢰된 upstream만 설정 가능한 identity context"*를 대체). `/recommend`는 **외부 클라이언트에 직접 노출되지 않는 내부 서비스 API**다(공개 ingress 없음, Alpha·Open Beta 확정). 인증된 moderator/agent runtime이 자신이 **이미 인증한** 요청자의 owner_id를 `Query.requester_owner_id: UUID | None`(top-level typed 필드, `eligibility_context` dict 아님)로 전달하며 이 API는 그 claim을 **신뢰한다**. 현 shared bearer(`verify_token` → `API_TOKENS`)는 **내부 호출자 집합의 구성원임만** 증명하고 ⓐ 어느 컴포넌트인지 ⓑ 그 컴포넌트가 특정 owner를 대리할 권한이 있는지 ⓒ claim이 실제 세션 actor와 일치하는지는 **증명하지 않는다**. 따라서 안전성 근거는 **2층**이다: ① 네트워크 경계 + shared secret ② *"호출자는 외부 요청 body의 owner ID를 그대로 중계하지 않고, 자신이 인증·인가한 session/event actor에서 파생한다"*는 upstream 계약. **"유효하지만 틀린 owner ID"는 이 API가 판별할 수 없다** — (4)는 그 위의 completeness guard다.

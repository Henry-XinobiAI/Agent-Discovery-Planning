...
- **requester self-exclusion 계약** — **production turn-on blocker**. 현재 파이프라인은 요청자 본인을 후보에서 제외하지 않는다(검증: `Query`에 requester identity 필드 없음, `verify_token`은 토큰 유효성만 검사하고 identity 미반환, Retriever의 축소(`_one_hit_per_direct_agent`·`_preempted_by_direct`)는 `agent_id` 기준으로만 접고·requester 미비교, Gate는 `eligibility_context`를 self-exclusion에 미사용). real edge를 켜면 요청자 본인이 해당 QID의 competence 조건을 만족할 때 자기 자신이 추천 후보로 나온다. turn-on 전에 확정할 계약:

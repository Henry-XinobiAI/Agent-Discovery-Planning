...
- **★ K = 전체 recall 상한이며, 그 단위는 "distinct agent"다 (안전장치 필요)**: Tier 1.5는 shortlist `owner_ids` 안에서만 검색하므로 `sort=confidence&limit=K`에서 탈락한 owner는 이후 **복구 불가**. **grounding confidence ≠ stance relevance**이고 `has_opinion`도 experiential/procedural에 담긴 입장을 놓쳐 **hard filter로 부적절 → prior로만**. 대응(§5 확정): **충분히 overfetch 후 Discovery에서 축소 + eval에서 Tier 1 owner recall@K 측정·K ratchet**. (eligible-pagination은 현 memory-api `Page`가 cursor/offset 없이 `truncated`만 주므로 불가 — `api/routers/base.py:202` — Alpha에서 memory-api 변경 1건 유지 위해 배제.)
  - **★ 단위 (LOCK · 2026-08-04 `24ebc2d`)**: K가 묶는 두 비용(evidence 검색 fan-out `ceil(K/20)`, 후보별 judge 콜)은 **agent당** 발생한다. 그런데 Discovery 쪽 shortlist의 입력은 **owner가 아니라 candidate edge**다 — 한 agent가 여러 앵커 facet으로 pool에 들어올 수 있으므로(retrieval one-hop 확장). 따라서 `Ranker.stance_shortlist(limit=K)`는 **distinct agent를 K개까지** 세고, edge를 K개 세지 않는다. edge를 세면 한 agent의 형제 edge 두 개가 K 슬롯 두 개를 먹으면서 **judge 콜은 한 번만** 나가 recall이 조용히 줄어든다. (memory-api 엔드포인트 쪽 K는 owner당 topic entity가 0/1개라[아래 카디널리티 항] owner 단위 = agent 단위로 이미 일치한다 — 단위가 갈리는 건 Discovery pool 쪽뿐이다.)
  - **후보의 3-상태 처분 (관측 계약)**: ① 아직 대표가 없는 agent의 첫 edge이고 K가 남아 있으면 **kept** · ② 이미 kept된 agent의 이후 edge는 `duplicate_agent_edge`(형제가 judge됐다) · ③ K가 소진된 뒤 처음 나타난 agent와 **그 agent의 모든 edge**는 `stance_shortlist_limit`(그 agent는 아무것도 judge되지 않았다). ②와 ③은 다른 사실을 말하므로 사유를 합치지 않는다 — ②는 "그 agent는 판정됐다", ③은 "그 agent는 K에 밀려 판정 자체가 없었다".
  - 어느 edge가 그 agent를 대표하는지는 **judge 전이라 stance를 알 수 없으므로** stance-무관 competence 키(`maturity_band → evidence → freshness`)로 정한다. 세 키가 **완전 동점**이면(형제끼리는 마지막 `agent_id` 항이 같아 tie를 못 깬다) stable sort가 남긴 **retrieval traversal 순서**가 대표를 정한다 — 그 need 기준으로 두 edge가 동등하므로 어느 쪽이든 랭킹은 같고, tie를 깨자고 `anchor_id` 같은 새 key를 넣지 않는다는 것이 규칙이다(다만 로그의 대표 `anchor_id`가 입력 순서에서 나온다는 사실은 명시한다). 이는 최종 stance 랭킹의 **무손실 prefix가 아니다**(§D4 `_stance_key`는 competence 3키 뒤에 `stance_confidence`를 넣는다) — 경계 동점에서 잘린 agent가 나중에 더 높은 confidence를 받을 수 있다는 **agent-level recall 손실은 K의 수용된 trade-off**이고, 전용 stance-evidence recall eval이 재는 대상이 **아니다**(그건 judge 입력까지의 evidence 도달을 잰다).
...
- **K / M / per_owner_limit / batch shape + K recall 안전장치**: Tier 1 shortlist 크기(K), inline preview 상한(M), 검색 owner당 상한, per-candidate vs 소배치. **K는 전체 recall 상한이고 그 단위는 distinct agent다(§D2)** — **overfetch-후-축소 + recall@K ratchet**으로 고정(eligible-pagination은 `Page`에 cursor/offset 없어 배제).

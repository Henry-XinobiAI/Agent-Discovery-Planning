...
- **범위 밖**: `API_TOKENS`의 `name:secret` 매핑(호출자별 토큰·로그 귀속)은 **별도 backlog**다. 이 슬라이스에
  섞지 않는다 — 섞으면 self-exclusion이 인증 리팩터링에 인질로 잡힌다. **두 번째 정책의 provider·집행 규칙도
...
claim을 **신뢰한다**. 현 shared bearer 인증(`verify_token` → `API_TOKENS`)은 **내부 호출자 집합의 구성원임만**
검증하고, claim과 사용자 세션의 결합은 검증하지 않는다.

**bearer token이 증명하는 것**: 허용된 내부 호출자 집합 중 하나가 요청했다. **그뿐이다.**

**증명하지 못하는 것**: 어느 컴포넌트가 호출했는가 · 그 컴포넌트가 특정 owner를 대리할 권한이 있는가 ·
`requester_owner_id`가 실제 대화의 요청자와 일치하는가.
...
1. 네트워크 경계 + shared secret으로 호출자를 내부 서비스로 제한한다.
...
§3의 2층 신뢰 모델에서 **2층 전체가 이 문장에 걸려 있다**.
...
- **호출자 귀속** — `API_TOKENS`가 `name:secret` 매핑이 아니어서 로그가 "어느 컴포넌트가 호출했는지"를
  남기지 못한다. **별도 backlog**이며 이 슬라이스에 섞지 않는다.

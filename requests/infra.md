...
**요청**: RDS PostgreSQL 17, dev·prod 각 1 인스턴스(가장 작은 크기로 시작, 예 `db.t4g.small`), DB `agent_discovery`, 앱 계정 1개(DDL 권한 포함 — 스키마 마이그레이션은 앱이 alembic으로 배포 시 적용), 서비스 네임스페이스에서만 접근. 접속 문자열은 Secret `DATABASE_URL`로.

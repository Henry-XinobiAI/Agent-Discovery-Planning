...
topic-api의 `/api/svc/topic`과 같은 형태로 `/api/svc/agent-discovery` 아래를 호출한다. edge-auth가 `x-user-id`를 채우므로 클라이언트가 유저 id를 보내지 않는다.
...
- **`lang`을 모든 요청에 보내 주기를 요청한다.** 허용값 `ko`, `en`, `ja`. 없으면 `en`으로 답하고, 그 외 값은 422. 응답의 `label`·`owner_note`는 요청 언어의 문자열 하나로 온다(언어별 맵이 아니다). 요청 언어가 없으면 en → 있는 것 → null 순으로 대체된다(R40).
- 응답 envelope마다 **`recommendation_id`**가 있다. 페이지를 이어 가면 `cursor`가 이 값을 실어 같은 목록으로 이어진다.
- `degraded: []`가 비어 있지 않으면 일부 신호가 빠진 응답이다(예 `friends_unavailable`). 상태는 200이고 표시를 바꿀 필요는 없다.
- 응답에 "가려진 개수"나 순위 공백은 없다. 없는 것은 없는 것이다.
...
추천 카드에서 대화를 시작할 때, bourbon-api의 대화 시작 요청(`POST /rooms/dm/{user_id}/agent` — 파라미터 위치는 bourbon-api가 정한다)에 두 값을 넣는다.
...
값을 만들거나 바꾸지 않는다. 받은 것을 그대로 넘긴다. 카드 meta에 값이 없으면(구버전 카드) `entry`만 `recommend_explicit`으로 보낸다.

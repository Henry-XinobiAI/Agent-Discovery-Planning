...
- **`section_topic_id`·`position`은 새로 생긴 선택 필드입니다(R66).** 카드의 값을 그대로 넘겨 주시면 되고, 안 보내셔도 지금처럼 받습니다. 필요한 이유는 §3과 같습니다 — 탐색 서브탭에서는 **한 사람이 한 화면의 두 선반에 동시에 설 수 있어서**("음식" 선반과 "요리 문화" 선반) `owner_user_id`만으로는 어느 카드를 누르셨는지 알 수 없고, position은 선반마다 1부터 다시 세므로 둘이 짝이어야 카드 하나를 가리킵니다. 탐색 메인과 타입 ①은 `section_topic_id`가 `null`입니다.
...
{ "recommendation_id": "<uuid>",
  "cards": [ { "owner_user_id": "<uuid>", "position": 1, "section_topic_id": "topic_food" },
             { "owner_user_id": "<uuid>", "position": 2, "section_topic_id": "topic_food" } ] }
→ 204
```

- **세 값 모두 카드에서 그대로 읽어 주시면 됩니다.** `section_topic_id`는 그 카드가 선 **선반의 topic_id**이고, 탐색 메인(`/discover/for-you`)과 타입 ①에서는 `null`입니다.
...
- **한 번에 200장을 넘기지는 말아 주세요.** 넘으면 422입니다. 한 요청의 크기 제한일 뿐이니 길어지면 나눠 보내시면 됩니다 — 어차피 나눠 보내는 것이 기본입니다(아래).
...
- 우리가 낸 적 없는 `recommendation_id`나 다른 사람의 목록이면 **조용히 버려지고 그때도 204**입니다. 클라이언트가 분기하실 것은 없습니다.

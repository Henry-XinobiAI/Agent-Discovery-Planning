...
탐색 서브탭(타입 ②)에서는 이렇게 됩니다 — 두 필드가 생긴 이유가 이 화면입니다:

```
{ "recommendation_id": "<uuid>", "owner_user_id": "<uuid>", "entry": "discover_by_topic",
  "section_topic_id": "<sections[].topic_id>", "position": 2 }
```

- **`section_topic_id`·`position`은 새로 생긴 선택 필드입니다(R66).** ⚠️ **`section_topic_id`는 카드가 아니라 카드를 감싼 섹션**(`sections[].topic_id`, 섹션 더 보기 화면에서는 경로의 `{topic_id}`)**에서 읽어 주세요** — 자세한 이유는 §3과 같습니다. `position`은 카드의 값 그대로입니다. 둘 다 안 보내셔도 지금처럼 받습니다. 필요한 이유는 §3과 같습니다 — 탐색 서브탭에서는 **한 사람이 한 화면의 두 선반에 동시에 설 수 있어서**("음식" 선반과 "요리 문화" 선반) `owner_user_id`만으로는 어느 카드를 누르셨는지 알 수 없고, position은 선반마다 1부터 다시 세므로 둘이 짝이어야 카드 하나를 가리킵니다. 탐색 메인과 타입 ①은 `section_topic_id`가 `null`입니다. **§3과 같은 주의**: 선반 topic은 카드가 아니라 **섹션**(`sections[].topic_id`)에서 읽어 주세요.
...
- **한 번에 50장을 넘기지는 말아 주세요. 넘으면 422입니다.** 한 요청의 크기 제한일 뿐이니 **50장씩 나눠 보내시면 됩니다** — 어차피 나눠 보내는 것이 기본입니다(아래).
  ⚠️ **"렌더 시점"을 기준으로 잡으실 거면 여기 걸리실 수 있습니다**: 탐색 서브탭 한 화면이 최대 `sections`(10) × `per_section`(20) = **200장**입니다. 통째로 보고하실 계획이면 반드시 나눠 보내 주세요.
- **`items`가 비어 있으면 422입니다.** 그릴 것이 없으면 호출하지 않으시면 됩니다.
...
- **`entry`는 보내지 말아 주세요.** §2의 본문을 복사하시면 `entry`가 남기 쉬운데, 이 route는 그 키를 받지 않습니다(어느 화면인지는 우리가 이미 알고 있습니다). **두 route 모두 선언하지 않은 키를 422로 거절합니다** — §2도 마찬가지이니 양쪽 다 필드를 임의로 더하지 말아 주세요. fire-and-forget이라 422가 나도 눈에 띄지 않고, 그러면 노출이 하나도 안 쌓입니다.

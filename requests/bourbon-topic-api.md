...
**요청**: `write_settings` 성공 뒤 아래를 best-effort로 발행해 주시면 된다. 브로커가 없어도 patch는 성공해야 한다(발행 실패는 경고만).
...
- 새 visibility **값은 싣지 않는다.** 우리는 어차피 재조회한다. 값이 있으면 소비자가 재조회를 생략할 여지가 생긴다.

...
1. ~~**이 호출량이 괜찮은지.** 탐색 화면 요청 1건당 두 route에 각 1회, id는 최대 50개입니다.~~ → **괜찮다**(오너, 2026-09-22 구두). 레이트 리밋이나 권장 상한은 받지 않았으므로 우리 쪽에서 상한을 만들지 않는다 — 한 페이지당 두 호출이라는 모양 자체가 상한이다(`for_you.limit_max`).

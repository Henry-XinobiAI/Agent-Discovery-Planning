...
|---|---|---|
| `GET /api/internal/users?ids=` | `name`, `picture`, `handle` | 한 페이지의 소유자 수, 최대 50 (그쪽 상한 100 아래) |
| `GET /api/internal/agents?ids=` | `name`, `picture` | 같음 |
...
1. **이 호출량이 괜찮은지.** 탐색 화면 요청 1건당 두 route에 각 1회, id는 최대 50개입니다. 배치 상한 100은 문서에 있는데 **호출 빈도에 대한 안내는 못 찾았습니다.** 레이트 리밋이나 권장 상한이 있으면 알려 주십시오.
2. **`GET /api/internal/agents`를 요청 경로에서 써도 되는지.** 그쪽 모듈 docstring은 카탈로그를 "이벤트 소비자 밑에 까는 주기 스윕"용으로 설명합니다. 우리는 `ids` 셀렉터만, 요청 경로에서 씁니다. 그 용도를 의도하지 않으셨다면 알려 주십시오 — 소유자 id에서 personal agent id를 유도하는 uuid5 공식은 우리가 이미 재계산하고 있어서, 필요하면 users 쪽만으로 좁힐 수 있습니다(대신 agent의 `name`·`picture`를 잃습니다).
3. **네트워크가 닿는지.** 우리 네임스페이스에서 `http://bourbon-api`로 나가는 호출이 처음입니다. NetworkPolicy나 메시 규칙에 더할 것이 있으면 알려 주십시오.

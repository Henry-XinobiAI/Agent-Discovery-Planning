...
| `cf_score` | 행렬 분해(ALS) 점수, 소스 안에서 정규화. **학습 입력의 confidence는 "대화 시작 1회"가 아니라 `1 + α·log(1 + turns)`** — 배우는 대화(긴 대화)가 한 번 시작하고 끝난 대화보다 강한 신호다(오너 2026-09-08, 결정 레지스터 R16). α는 설정. (2026-09-24 개정: R16의 `β·reopen_count` 항은 R46(2026-09-10)이 `reopened`를 받지 않기로 하면서 없어졌다 — 다시 찾았는지를 알려주는 이벤트가 없어 곱할 수가 없다.) 가중은 자동 게이트가 0에서 올린다(R35) | 0~1 | cf_engine — `implicit` ALS(R33) (입력: `interactions` + turn 카운터) | ③ |

...
| `content_similarity` | 요청자 topic 집합과 소유자 topic 집합의 가중 겹침. R25: IDF 가중 × 카탈로그 계층 감쇠(양방향 최단 hop, 0.6/hop·최대 3 hop, 경로당 최구체 일치 하나, drawer는 통과 노드), 임베딩 없음. 0~1 정규화와 IDF의 분자(그 topic을 가진 소유자 수)는 그 요청의 쿼리 안에서 나오고, **분모는 주기 스냅샷에서 읽는다**(R59) — **답에 나올 수 있는 소유자 수**(공개된 row가 하나라도 있는 사람, R58)를 `population_stats`에서 한 줄로, 읽을 것이 없으면 그 자리에서 세어서. 정규화는 **R28로 빼기 전** 최대값으로 한다(누구와 이미 대화했나에 따라 스케일이 흔들리지 않게, walkthrough §5-5) | 0~1 | 요청자 topic(R27) × visible_topic_rows + 카탈로그 edge 테이블(R26) | ③ |

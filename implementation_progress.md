...
- [ ] **15. 1-10 배포 준비** 🟢
  ...
  **새로 드러난 것 다섯**:
  ...
  - **`GROUNDING_DISAMBIGUATION_ENABLED`가 dev와 prod에서 다르고, 그 차이가 이제 숫자다**(2026-09-15, 16-D). dev 오버레이는 `true`, prod는 `false`다. 16-D가 2만 모집단 200건으로 잰 결과, **꺼진 쪽에서 질문의 26%(52/200)가 422 `ambiguous`로 끝난다** — 카탈로그에 후보가 있었는데 규칙이 못 골랐다는 뜻이고, topic 하나짜리 질문에 몰린다(116 중 40). 코드가 이 간극을 설계 시점에 알고 있었다(`stages/expansion_prompt.py`: plain-name 규칙은 이 플래그를 필요로 한다). **그래서 dev에서 보는 타입 ①과 prod에서 볼 타입 ①이 다르고, go-live 전에 둘 중 하나를 고르는 판단이 필요하다** — 켜면 못 고른 그룹당 completion 하나가 더 들고, 끄면 저 26%가 그대로 간다. 근거는 `validation_results.md` §11-4.
  PR: #37 (부분) · 기획 #86

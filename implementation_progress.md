...
- [x] **14. 1-9 검증** 🔴
  ...
  **1-9 범위인데 남은 것 다섯** — 항목 16으로 뺀다:
  ...
  - **타입 ①** 지연 시간·정확도 — e3llm-api를 통과시키는 200건(스펙 §3-6).
  - **타입 ② 순서 일치율** — 지표 함수는 있고 채점기에 연결하지 않았다.
...
- [ ] **16. 1-9 잔여 측정** 🟡
  다섯 중 **셋 완료**. 게이트 임계의 다음 질문("어느 파라미터가 결과를 지배하나")에는 16-B가 답했다 — `opener_rate`다. 16-C는 답이 아니라 **빈자리**를 찾았다: 정확도를 깎는 장애 둘이 `degraded[]`에도 `basis`에도 아무것도 남기지 않는다.
  ...
  - [ ] **16-D. 타입 ① 지연·정확도, 타입 ② 순서 일치율.** **다음 작업.** 항목 16에서 **외부 의존이 붙는 유일한 것**이다(e3llm-api를 통과시키는 200건, 스펙 §3-6).

...
- [ ] **16. 1-9 잔여 측정** 🟡
  다섯 중 **하나 완료**. 게이트 임계를 오너가 정하려면 최소한 민감도 표가 있어야 한다 — 지금 결론이 "생성기 파라미터에 달렸다"이므로, 어느 파라미터가 결과를 지배하는지가 다음 질문이다.
  ...
  - [ ] **16-B. §6-4 민감도 표** — `opener_rate`·`affinity` 노이즈·`off_cluster_topic_rate` ±50%. **다음 작업.**
  - [ ] **16-C. 장애 주입 3종** — CF 워커 중단 / `cf_candidates` 비어 있음 / 이벤트 유입 중단.
...
- **§6-4 민감도 표는 아직 없다** — 1-6도 1-9도 만들지 않았다. 평가기는 항목 14가 만들었고 `--params` 파일 변형으로 돌아가므로, 남은 것은 돌리는 일이다(항목 16). 항목 14의 결론이 "게이트 수치가 생성기 파라미터에 달렸다"이므로 **이 표가 그 다음 질문에 답한다**.

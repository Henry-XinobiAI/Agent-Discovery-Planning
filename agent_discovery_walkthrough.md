...
임계값(샘플이 작으면 기본값 유지)과 주기는 R37 — 설정 `calibration.*`이고 값은 코드에 있다(R54). λ(유저당 대화 시작 수)는 `entry=direct`인 시작만으로 재고, 추천을 거친 시작은 `conversation_rate_via_recommend`로 따로 둔다 — 추천이 만든 대화로 추천 파라미터를 보정하면 순환이 된다.

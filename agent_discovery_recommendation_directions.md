...
**Favorite / user preference (Alpha 미소비; positive favorite은 Post-Open-Beta).**

- favorite은 personalization 신호이지 expertise 근거가 아니다.
- score term으로 쓰지 않고, Post-Open-Beta에도 **bounded tie-break**로만 쓴다.
- **Post-Open-Beta 허용 Need**: 깊이/경험/강화. **대립/직교에서는 0**(favorite은 유저와 stance가 정렬될 확률이 높아 두 Need의 objective를 훼손).
- 어떤 gate(topic/maturity/safety/discoverability)도 우회하지 못한다.
- negative preference(hidden/muted/dismissed)는 **Open Beta 후보 항목**(serving suppression)이며 positive favorite보다 먼저 다룬다.
- source/ownership은 §9.7.
...
Discovery는 source of truth가 아니다. 다른 컴포넌트(Memory, Persona, Moderation/Runtime)가 raw signal을 제공하고, Discovery가 retrieval/ranking용 projection을 만든다. (user preference/favorite은 §9.7 — Alpha 미사용, positive favorite 소비는 Post-Open-Beta.)
...
유저는 다른 유저의 agent를 **즐겨찾기(favorite)** 할 수 있다(follow/friend 같은 사회관계가 아닌 단순 즐겨찾기). 이는 **user→agent 선호 관계**로, observed stance(Memory)도 prior(Persona)도 아닌 **독립 입력 소스**다.

| 데이터 | 출처 | 이유 |
|---|---|---|
| `favorite_agent_ids` | **`bourbon-api`(메인 앱 API, 예상)** | personalization tie-break (§5.3) |
| `muted` / `hidden` | bourbon-api | serving hard filter (do-not-show) |
| `dismissed_recently` | bourbon-api | Push fatigue / silence 보조 신호 |

소유가 Memory/Persona와 다르다 — 같은 곳에 접으면 serving policy와 경계가 흐려진다. 통합 시 anchor/edge(Memory)·prior(Persona)와 **별개 provider**(`UserPreferenceProvider`, bourbon-api 래퍼)로 받는다. **Alpha는 이 소스를 소비하지 않는다** — favorite은 expertise 신호가 아니라 Alpha 평가축을 흐리므로, **design 레벨로만 slot을 예약**(런타임 Protocol·no-op 미생성)한다. 소비 단계가 다르다: **positive favorite tie-break은 Post-Open-Beta**, **negative preference(hidden/muted=do-not-show, dismissed_recently=Push fatigue)는 Open Beta 후보 항목**(bourbon-api 신호 존재 시 serving suppression). negative는 플랫폼 safety/privacy gate가 **아니라** 유저 개인의 **preference-based serving suppression**(maturity/safety/privacy 3종과 별개 축)이며 positive favorite보다 먼저 다룬다. ranking 거버넌스와 Need별 on/off는 §5.3, gate 우회는 절대 금지.
...
- **stance content의 observed/prior source 소유 확정** (Alpha 선결, §9.3) — for/against 성립 전제 · **[해결됨 2026-07-02: Memory=`observed_stance` 소유, Discovery=`axis_positions`·need 분류 파생]**
- granularity 선택 기준: "충분한 maturity density"의 임계값
- `floor_need` 기본값과 Need별 maturity floor · **[seed 확정: `MATURITY_MIN`/band cutoff provisional·ratchet 대상, build_plan §4.2]**
- request text에서 topic·need_type·user_stance를 뽑는 추출 경로 · **[구현됨: `normalize.py` + linker grounding, build_plan §3.1]**
...
- negative preference serving suppression 도입 여부 — `hidden`/`muted`(do-not-show), `dismissed_recently`(Push fatigue/silence). bourbon-api 신호 존재 시 candidate, positive favorite은 Post-Open-Beta(§9.7·§5.3)
...
- **positive favorite** 반영 (출처: bourbon-api, §9.7) — bounded tie-break(깊이/경험/강화, 대립/직교 제외, §5.3). negative preference는 그보다 앞선 Open Beta 후보 항목(위)
...
> 위 질문들의 구현 측면 — build vs reuse, 단계별 컴포넌트 후보, 선결 의존성(anchor vocabulary 정렬·Memory edge readiness) — 은 `agent_discovery_recommendation_implementation.md`(working)에서 다룬다. 확정 기술 선택은 그 문서가 가리키는 별도 `technology_selection`으로 미룬다.

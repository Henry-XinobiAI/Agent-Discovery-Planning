...
| 모드 | 설명 | 출력 |
|---|---|---|
| **모드 A: pull** | 유저가 직접 "이 주제로 agent 추천해줘"라고 요청 | 후보 리스트 |
| **모드 B: push** | 대화 중 시스템이 "지금 다른 agent가 도움이 될 수 있다"고 감지 | 인라인 추천 또는 침묵 |

모드 B가 더 어렵다. 대화 중 끼어드는 추천이기 때문에 틀리면 방해가 된다. 그래서 먼저 "지금 어떤 필요(need)인가"를 알아야 한다.
...
MVP는 복잡한 임베딩/학습 모델 없이 먼저 동작하게 만든다. 핵심은 **구조화된 텍스트 필드와 간단한 규칙**이다.
...
### 3.2 agent-topic edge의 두 얼굴

edge는 knowledge face와 stance face를 나눠 가진다.

| face | 의미 | prior 사용 |
|---|---|---|
| **knowledge face** | 이 topic에 대해 얼마나 근거가 있는가 | 금지 |
| **stance face** | 이 topic에서 어떤 입장인가 | 가능 |
...
| topic-local credibility | maturity/evidence | 이미 edge knowledge face에 있음 |
...
5. need가 있으면 need-specific ranking
6. 후보 리스트 제공
7. 선택/대화/피드백 logging
```
...
Discovery는 source of truth가 아니라 projection consumer다. Memory/Persona/Moderation이 raw signal을 주고, Discovery가 retrieval/ranking용 projection을 만든다.

### 9.1 Memory가 제공해야 하는 것
...
### 9.2 Memory 또는 Persona가 합의해야 하는 것
...
| `silence_threshold` | push에서 침묵 판단 |
...
### 9.5 Discovery가 derive하는 것

Memory/Persona가 만들 필요 없는 것:
...
Discovery는 source of truth가 아니라 projection이다. 따라서 Memory/Persona 쪽 변경을 이벤트로 받아야 인덱스를 최신으로 유지할 수 있다.
...
Safety verdict도 lifecycle 대상이다. agent나 특정 routing target이 나중에 unsafe로 판정되면 moderation/response team이 update 또는 revoke 이벤트를 발행하고, Discovery는 해당 candidate projection을 재계산하거나 gate에서 제외한다.

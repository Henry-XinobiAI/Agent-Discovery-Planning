...
- need=depth/experience/coverage에서 `user_stance_ref`가 와도 **reject하지 않고 ignore + decision-log에 기록(log-only)** — 클라이언트가 공통 payload를 보낼 수 있으므로 불필요한 stance에 422를 내지 않는다(API를 까다롭게 만들지 않음).

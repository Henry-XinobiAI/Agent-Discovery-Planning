...
세 레포를 직접 확인한 결과 (base: 2026-06-23, memory-api 계약: **2026-07-02 재확인 — main `d66b2c1`(#47, 이전 스냅샷 `4106366` 대비 24커밋)에서도 public `/knowledge/entities*` 계약 완전 불변**(5라우트·6모델·`Page[T]` diff 0). entity rename은 `8ffcec8`에서 도입. `4106366`→`d66b2c1`은 LongMemEval 평가 튜닝·**신규 conversation artifacts/conversations 라우트(대화메모리 도메인 = Discovery 범위 밖)**·personal KG 강화(statement→public-KG-by-QID 매핑·entity salience·context-band)·shared-utils 위주로 **public entity 계약은 동일**. salience/statement-edge는 이제 main personal KG에 있으나 Discovery edge 계약(`AgentTopicEdge`) 대체물이 아니다 — §4②·§11 메모):

...
> 가지 중 **7·9는 `D22`, 1은 `D23`, 11은 `D24`로 채택**됐고 나머지는 후보다 — §12-5의 실험 `GOR-X1`–`X5`가 닫는다(`decisions.md` C11).
...
> **이 절은 후보안이다** — 단 7·9는 `D22`(2026-09-03, 오너), 1은 `D23`, 11은 `D24`(2026-09-04, 실측)로 채택됐다. 나머지의 채택은 `decisions.md`가 하고,
...
### 12-4 개선 후보 열한 가지 (7·9는 `D22`, 1은 `D23`, 11은 `D24`, 나머지는 결정 아님)
...
| 3 | **뜨거운 라벨만 쿼리 item 샤딩** — agent에 `coffee~s`(s=hash mod S), 쿼리 item S개 | 쿼리 item은 우리가 만드는 것. S개 병렬 조회 → S×100 후보. S는 복제본의 보유자 수에서 자동 | 뜨거운 라벨에 한해 agent당 태그 1개, 쿼리 item S개(hidden). 조회 S개 병렬 | 샤드 안 100은 여전히 `√wsum(d)` 순 — 1·2와 조합 필요 | X4 → **X4: 통함** — 다중 라벨과 결합해 S=50에서 0.87, S=100에서 1.0. 필요한 S ≈ 최대-depth 보유자 수 / ~40, 규모에 비례 |
...
- **(b) 출시 후 — 계측.** 공개 보유 복제본(`D10`)이 우리 것이므로 **topic-api 협의 없이** 센다: 공개 topic ≥1인
  user 수, 그 분포(p50·p95·max), 주간 증가, **공개 보유 유저의 평균 최상위 topic 수**(`D24`의 item 배수 — 합성 1.88). 이 값이 `N*`에 접근하면 C11을 다시 연다(아래 "뒤집는 조건").
...
**결과 (2026-09-04)** — **단독 쿼리로는 어느 조건도 0.9에 못 미친다. 샤딩 × 다중 라벨은 S=100에서 1.0.** 채택은 오너 결정(아래 "정할 것").
...
**(b)의 정답 순위 분포**(정확 계산, D0): GT100의 (b) 순위는 최소 1 · 중앙값 1,491 · 90퍼센타일 5,443 · 최대 7,324.
...
- **후보 3(샤딩)은 슬롯을 곱하는 것이라 통한다.** 필요한 슬롯 ≈ 최대-depth 보유자 수 × ~2.7(`√wsum`이 그들을 순위에 흩어 놓아서).
...
topic별 item으로)이다 — 정확 계산으로 recall 99/100, Gorse 확인은 **X6**. 후보 2는 기각, 후보 4는 무효, 후보 3은 쓰지 않는다.

<details><summary>재현: 하네스 요지</summary>

```text
D0 = 5×10⁵ agent 원본 + 쿼리 qa(coffee,S) · qb(coffee + 하위 9, S)
D1 = 같은 agent에 coffee 보유자마다 coffee~<md5 mod 50>, 하위 ≥ 4면 coffee#deep 추가(12,777명)
...
- **왜**: X4가 보인 focus 편향은 `√wsum(d)`에 agent의 모든 태그가 들어가서다. 나누는 단위는 **쿼리가 떨어지는 단위** — 우리 트리는
  ...
  `topic/catalog/flatten.py`, README "Tree depth"). 라벨마다 나누면 item 태그가 1개가 되어 `commonCount` 신호가 죽고(×4.6),
...
- **실패의 뜻**: 정확 계산(99/100)과 Gorse가 다르다는 것 → HNSW가 ×1.88 item에서 달라졌는지 확인. 통과하면 후보 11을 `Dnn`으로
  올리고 `D23`의 item 형태를 고쳐 쓴다.
...
**채택 시 바뀌는 것**: `D23` — surface 1 인스턴스의 item은 `agent_id#top_topic_id`, 태그는 그 최상위 + 그 아래 보유 하위. S3이 id를
agent로 되돌리고 S4가 agent 기준 dedup. `N*`는 agent 수 기준으로 item 배수만큼 나눈다(합성 1.88 — 실값은 X2(b)에서 "공개 보유 유저의
평균 최상위 topic 수"로 센다). 기준 문서 §S3, `inbound_event_contract.md`의 프로젝션 모양, `recsys-intent.md`의 `ItemId = personal_agent_id`
(surface 2에만 남음)를 고친다.
...
| 4 | **X4** 컷 안 깊은 사람 비율 — **완료 2026-09-04**(5×10⁵ 세트) | X3의 10⁶ 세트 | 12-4 행 2·3 조합 (통과 기준 recall ≥ 0.9, 오너 확정) — **단독 쿼리 전부 미달, 샤딩×다중 S=100에서 1.0. 깊이 정의부터 오너 결정** | 채택 조합 → `Dnn` · 기준 문서 §S3 · `SURV-R4`(배포 전 게이트) 판정 |
| 4-1 | **X6** agent × 최상위 item (후보 11) — **통과 → `D24`** 2026-09-04 | X4 | recall이 정확 계산(99/100)대로 나오면 후보 11 → `Dnn`, `D23`의 item 형태 수정 | 12-4 행 11 · `D23` 근거 · 기준 문서 §S3 item 형태 · X2(b)에 "agent당 보유 최상위 수" 계측 추가 |
| 5 | — 결정 | 1–4-1 | **C11 닫기** | `decisions.md` C11에 "해소" · 기준 문서 §S3에 최종 형태(전용 인스턴스 · 세대 · 조합) · `recsys_opensource/README.md` §2 표 갱신 |
...
**하지 말 것**: 실험 전에 2·3을 채택으로 쓰지 않는다(규칙 ③). `N*` 없이 D22의 조건을 숫자로 쓰지 않는다.

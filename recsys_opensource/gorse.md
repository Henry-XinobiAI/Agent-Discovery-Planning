...
> 가지 중 **7·9는 `D22`, 1은 `D23`으로 채택**됐고 나머지는 후보다 — §12-5의 실험 `GOR-X1`–`X5`가 닫는다(`decisions.md` C11).
...
> **이 절은 후보안이다** — 단 7·9는 `D22`(2026-09-03, 오너), 1은 `D23`(2026-09-04, 실측)으로 채택됐다. 나머지의 채택은 `decisions.md`가 하고,
...
### 12-4 개선 후보 열 가지 (7·9는 `D22`, 1은 `D23`, 나머지는 결정 아님)
...
**결과 (2026-09-04)** — **통과.** recall 94/100(기준 0.9), 상위 100 중 66명이 최상위 topic 2개 이상 보유. 채택(`Dnn`)은 오너 확인 대기.
...
오너 결정(같은 날): 깊이에 희귀 하위·폭을 넣되 **(i) D23 유지**, 개선은 **후보 11**(agent × 최상위 item)로 — **X6 통과**(Gorse recall 94/100, 폭 중립 확인, 비용은 item 배수 1.88). **채택(`Dnn`)은 오너 확인 대기.** 이것과 `N*`가 남았다.
...
| 4-1 | **X6** agent × 최상위 item (후보 11) — **통과 2026-09-04**(recall 94/100, 25만 agent) | X4 | recall이 정확 계산(99/100)대로 나오면 후보 11 → `Dnn`, `D23`의 item 형태 수정 | 12-4 행 11 · `D23` 근거 · 기준 문서 §S3 item 형태 · X2(b)에 "agent당 보유 최상위 수" 계측 추가 |

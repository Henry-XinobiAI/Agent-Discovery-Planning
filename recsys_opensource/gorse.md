...
> 가지는 **후보안이고 결정이 아니다** — 어느 것을 쓸지는 §12-5의 실험 `GOR-X1`–`X5`가 닫는다(`decisions.md`
> C11). rev 2(2026-09-03)에서 **§11이 뒤집은 자리는 앞 절
...
> **이 절은 후보안이다.** 채택은 `decisions.md`가 하고, 지금은 C11(열린 항목)이다. 앞 절과 달리 근거가
...
| **6** | **item 집합을 `I_eligible`로** — 공개 topic ≥1 · recommendable인 agent만 | `D16`이 이미 프로젝션을 우리 것으로 뒀다. 5%면 빌드·RAM·캐시 전부 1/20 | 측정 하나 | — | **X2** |
| 7 | **surface 1 전용 Gorse 인스턴스** — tags item-to-item만, user·feedback 없음 | `D09`(store는 파생물)가 프로젝션 둘을 허용. surface 2의 유저별 캐시(560 GB)와 분리되어 4가 가능해짐 | 배포 둘 (운영 ~1.3배) | 빌드 자체 | X3 |
| 8 | `n_jobs` 상향 + master 사이징 | S8·S9 | 박스 하나 크게 | 단일 프로세스 한계 | X3 |
| **9** | **쿼리 item을 사이클마다 새 id로 재생성** (`q:<label>:<gen>`), `cache_expire`는 길게 | S7: 새 item = 캐시 없음 = 무조건 계산. agent들의 목록(아무도 안 읽음)은 한 번 쓰고 끝 → Redis 왕복 3억 회 절약. **새 agent가 1 사이클 안에 보인다** | 사이클마다 hidden item ~3천 개 생성·이전 세대 삭제 | 빌드 자체(S6) | **X5** |
...
#### X2 — `I_eligible` 측정 (후보 6, C11의 임계값) `topic-api 협의`

- **질문**: 공개 tier topic이 1개 이상인 user 수, 그 user들의 공개 topic 수 분포(p50·p95·max), 월별 증가.
  `recommendable` opt-in은 아직 없으므로(`EVT-E4`) **상한 추정**이다.
- **방법**: topic-api 테이블의 DynamoDB export → Parquet 집계. 또는 topic-api 팀에 집계 endpoint 요청. 데이터
  소유가 그쪽이라 **협의 대상**이다.
- **산출**: 현재 `I_eligible`, 1억 유저 시점 외삽값, 태그 수 분포(→ X3의 합성 분포 근거, `storage_sizing.md`
  400 KB 항목 한계의 입력). `SURV-R8`·`STO-S1`이 같은 질문이다.
- **판정**: 없음(측정). 이 값이 12-4의 5 vs 7+9를 가른다.
...
- **판정**: 사이클 ≤ 목표 가시성 지연(C7이 정할 값) ∧ RAM ≤ 한 박스. 넘으면 12-4의 5.
...
- **통과(제안)**: 채택 조합의 recall ≥ 0.9. 값은 오너가 정한다.

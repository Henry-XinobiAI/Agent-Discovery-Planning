...
**정할 것(오너)**: ⑴ 깊이의 정의 — depth만인가, 희귀 하위(IDF)·폭을 넣는가. depth만이면 `D23`으로 끝이고 후보 2·3은 필요 없다.
⑵ 넣는다면 셋 중 하나 — (i) `D23` 그대로 두고 focus 편향을 받아들인다 (ii) 후보 3을 뜨거운 라벨에 켠다(S는 최대-depth 보유자 수 / ~40,
운영 지표로 조정; 후보 10⁴–10⁵를 S4·S5가 받는 비용) (iii) 후보 5(우리 인덱스)로 간다 — 컷과 순서가 우리 것이 된다. 어느 쪽이든
후보 2는 기각, 후보 4는 무효로 적는다.
...
**남은 결정은 오너 몫**: 깊이의 정의(depth만인가) → 그에 따라 D23 유지 / 후보 3 / 후보 5. X4 결과 소절 "정할 것".

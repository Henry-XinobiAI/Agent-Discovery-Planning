...
★ 마지막 행이 [`gorse.md` §11-2 경로 C](gorse.md)(라벨별 개별 질의 + 가중 합산)다.
**표면 1의 topic 검색은 feedback을 전혀 읽지 않으므로 버킷을 어떻게 바꾸든 결과가 움직이지 않는다.**
feedback 의미론은 전적으로 **표면 2의 문제**다.

`판단` 이 사실은 순서에도 영향을 준다 — 표면 1을 먼저 세우는 동안 feedback 정의를 미뤄도 표면 1의
품질이 볼모가 되지 않는다.

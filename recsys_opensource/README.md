...
## 1. 이 조사가 답하는 질문 — surface가 둘이다

제품의 추천 surface는 둘이고, **recsys 관점에서 서로 다른 문제**다. 이 구분이 이 문서 전체의 축이다.

```text
surface 1   topic / context 쿼리 있음   →  그 주제를 잘 아는 agent
         쿼리가 있다. 검색에 가깝다.

surface 2   쿼리 없음                   →  이 사용자의 활동 기반으로 agent 여럿
         쿼리가 없다. 이것이 추천 시스템의 정확한 정의다.
```

`판단` **surface 2는 추천 라이브러리의 메인 API 그 자체다.** `recommend(user_id, N)` — Gorse의 제품
전체가 이것이고, implicit·Cornac의 기본 호출이 이것이다.

`판단` **surface 1은 표준 추천 호출이 아니다.** 추천 라이브러리의 기본 함수는 요청 시점 쿼리를 받지
않는다. 다만 Gorse에는 **다른 경로가 있다** — 더미 topic item의 이웃을 라벨별로 묻는 방식으로
호출당 약 1.1 ms에 쿼리형 후보 검색이 된다(§7-3 · `D07`). "기본 호출이 아니다"와 "불가능하다"는
다르고, 이전 판은 그 둘을 섞었다.

두 방향으로 틀릴 수 있다. surface 2를 surface 1의 요구(요청별 권한 필터·임의 allowlist·쿼리 처리)에 맞추면
기성 도구를 못 쓰고, surface 1을 surface 2에 맞추면 쿼리가 무시된다. **이전 판의 주된 오류가 전자였다** —
surface 1의 요구를 두 surface 모두의 요구로 다뤘다.

---

## 2. surface × 층 — 도구 배치

비교 축은 "도구"가 아니라 **surface × 층**이다. 같은 도구가 한 surface에서는 엔진이고 다른 surface에서는
오답이다.

| 층 | surface 1 (쿼리 있음) | surface 2 (쿼리 없음) |
|---|---|---|
| **grounding** | 우리 (자연어 → topic + 하위 라벨) | 해당 없음 |
| **후보 생성** | Gorse 전용 인스턴스(`D22`) · 라벨별 쿼리(§7-3) + 다중 라벨 쿼리(`D23`) · item = agent × 최상위 topic(`D24`). 우리 인덱스(§7-5)는 D22의 뒤집는 조건에서 | **Gorse** `recommend(user)` |
| **자격 gate** | 우리 (§9) | 우리 (§9) |
...
`판단` **surface 2에서 Gorse는 통째로 쓰고, surface 1에서는 후보 생성기까지만 쓴다.** 근거는 §6·§7.
...
`열린 항목` **R1 — surface 2가 `friends` 보유까지 근거로 삼는가.** `public`만이면 요청자별 변동이 없어
...
ephemeral query intent  요청 한 건, 짧은 수명            surface 1의 후보 제약
observed behavior      실제 사건으로 append              별도 projection 후 학습
```

`판단` **이 분리가 surface 둘을 가르는 것과 같은 선이다.** surface 1은 ephemeral query intent를 읽고,
surface 2는 declared profile + observed behavior를 읽는다.
...
`판단` **가장 강한 보호는 받지 않는 것이다.** 우리 인덱스·store에 공개 프로젝션만 들어오면 "공개 → 비공개
...
`판단` **surface 2는 Gorse가 하는 일 그 자체이므로 직접 만들 이유가 없다.**

```text
이벤트 → Gorse 에 user/item/feedback 적재
surface 2 → GET /api/recommend/{user}?n=20 → 본인·차단 후처리 제거 → hydrate → 응답(D02: 형태가 다름)
```
...
영구히 제외**하므로, `enable_replacement = true`와 ranker가 **먼저** 설정되어야 surface 2가 우리 요구를
만족한다(`D18` · [`feedback_semantics.md` §2](feedback_semantics.md)). 쿨다운·시간 감쇠는 Gorse가 주지
않으므로 우리 rerank가 소유한다.

### 6-1 이전 판의 기각 사유 재평가

이전 판은 Gorse를 사실상 기각했다. 사유 셋 중 둘은 **surface 2에서 성립하지 않는다.**

| 이전 기각 사유 | 재평가 |
|---|---|
| 요청자별 공개 범위를 표현 못 함 | **surface 1의 요구였다.** surface 2 근거를 `public`으로 한정하면 요청자별 변동이 없다. 물어보지도 않은 사용자에게 friends 공개 전문성을 근거로 대는 것은 surface 2에서 오히려 부적절할 수 있다 → **R1의 제품 결정 문제** |
...
topic × agent 행렬을 만들어 `implicit`의 BM25 item-item에 쿼리한 결과. 보유자: `bob`{coffee,espresso}
`carol`{coffee,roasting,drip} `dave`{drip} `erin`{hiking,photo} `generalist`{전부}.

| 쿼리 | implicit item-item | 올바른 정식화 (BM25 문서검색) |
|---|---|---|
...
**원인**: item-item은 **동시 출현 유사도**를 계산한다. 쿼리가 실제로 묻는 것은 "이 주제 보유자들과
*비슷한* 사람"이고, `generalist`는 누구와도 비슷하므로 항상 이긴다. 교과서적 인기도 편향이다.

우리 질문은 "이 주제를 **대표하는** 사람"이고 그것은 **검색**이다 — 문서=agent(보유 주제 묶음),
단어=topic_id, 쿼리=확정 주제들. 그러면 미보유자 0점 · IDF가 희소 주제 전문가를 올림 · 길이
...
| 경로 | 결과 |
|---|---|
| `?category=<topic>` | 후보 집합은 **정확히** 좁혀진다. 그러나 `gen`(6주제·피드백 12건)이 `drip` 쿼리에서 `dave`(drip 전담·피드백 0건)를 이긴다 |
| 결합 태그 쿼리 (더미 topic item에 하위 라벨 전부) | `generalist` 1등, **찾으려던 `broad`(상위+하위 전부 보유)는 결과에서 누락** |

후자의 누락은 **쿼리와 태그 집합이 동일한 item이 이웃에서 제외되기** 때문이다. 쿼리 태그를 늘릴수록
정답이 제외될 확률이 올라간다.

`판단` 두 경로 모두 잠근 원칙("첫날 항으로 인기도를 쓰지 않는다")과 걸린다.

### 7-3 `실측` 성립하는 형태 — 라벨별 개별 쿼리 + 가중 합산

> 규모에서의 형태는 여기에 둘이 더해진다 — topic당 다중 라벨 + 센티넬 쿼리(`D23`)와 item = agent × 최상위 topic(`D24`).
> `gorse.md` §12-4의 1·11, §12-5 X1·X4·X6.

전제 둘: **하위 라벨 보유가 상위 라벨 보유를 함의한다**(제품 사실) · 쿼리 1회 오버헤드가 작다.

```text
grounding(우리) → 확정 topic + 하위 라벨들
   → 라벨마다 Gorse item-to-item 쿼리            실측 ~1.1 ms/호출
...
센티넬은 어떤 agent도 갖지 않는 태그(`__QUERY__`)를 쿼리 item에 하나 넣는 것이다. 어떤 후보와도
...
보유자가 1등**이 된다. 상위 쿼리는 후보를 긁어오는 데 쓰고 랭킹 가중은 하위에 준다.

좁은 쿼리에서 좁고 깊은 전문가가 1등이 되는 것(마지막 행)이 **쿼리 구체성이 보존된다**는 증거다.
확장은 아래로만, 쿼리가 일반적인 만큼만 한다.
...
topic을 필수로 요구하므로 쿼리 topic을 보유하지 않은 후보는 **응답을 만들 수 없다.** 따라서 이 필터는
옵션이 아니라 **surface 1에서 Gorse가 후보를 추가할 수 있게 하는 전제**이고, 판정은 Gorse 점수가 아니라
우리 store의 `public_topics` 목록으로 한다. 상세는 [gorse.md §11-3](gorse.md).

### 7-5 `열린 항목` retrieval·검색 인덱스 — 조사 빈칸

7-1이 보인 올바른 정식화는 **역색인 + BM25**다. 7-3은 그것을 Gorse의 더미 item 트릭으로 얻는 경로이고,
직접 인덱스를 갖는 경로는 조사된 바 없다.
...
| # | 문서 근거 판단 | 실측 |
|---|---|---|
| 1 | 요청별 임의 후보 제한은 표준 endpoint에 없음 | **더미 topic item + 이웃 쿼리로 가능.** 숨긴 item(`IsHidden=true`)도 이웃이 계산·서빙된다 |
| 2 | (서술 없음) | **쿼리와 태그 집합이 같은 item은 이웃에서 제외된다.** 사양서로 예측 불가이고 이 용도에서 치명적이었다 |
...
`판단` surface가 둘이 되면서 이 표에 축이 하나 붙는다. **두 surface의 피드백이 한 곳으로 모여야 한다** —
surface마다 다른 엔진을 쓰면 학습 루프가 갈라진다. 원본은 우리 스트림 하나이고 Gorse에는 프로젝션만 보낸다.
...
`판단` 후자는 정보 유출 문제가 아니다 — §5의 원칙(공개 프로젝션만 받는다)이 그것을 이미 닫는다.
...
```text
0. 제품 결정         R1 (surface 2 의 friends 포함 여부) 을 먼저 닫는다
                     이것이 §6 의 전제다

1. 노출 계약          exposure/decision 스키마를 고정하고 기록을 시작한다
                     off_policy.md §3 이 소유.  나중에 만들 수 없는 유일한 항목

2. surface 2            Gorse 적재 + 클라이언트 + 후처리 필터 + 재추천 정책
                     기성 기능이 대부분이지만 그대로는 안 된다 —
                     enable_replacement 와 ranker 가 먼저다 (D18)

3. surface 1            grounding + 라벨별 쿼리 + 가중 합산 (§7-3) + 다중 라벨 쿼리(D23), 전용 인스턴스(D22), item = agent × 최상위(D24)
                     rerank 는 규칙으로 시작
...
6. 재검토            retrieval 인덱스 직접 소유 (§7-5)
                     D22의 뒤집는 조건이 발동하면 — 출시 후 I_eligible × item 배수 ≥ 0.5·N*
```

`판단` **1을 2·3보다 먼저 두는 이유는 소급 불가이기 때문이다.** 모델도 인덱스도 다시 만들 수 있지만
...
| # | 항목 | 성격 | 상태 |
|---|---|---|---|
| **R1** | surface 2가 `friends` 보유까지 근거로 삼는가 | 제품 결정 | **미정. §6의 전제** |
| R2 | surface 1이 `friends` 보유까지 포함하는가 | 제품 결정 | 미정. `public`만이면 배치 인덱스로 충분 |
| R3 | 목표 규모에서 재계산 시간 (§11) | 측정 | **5×10⁵까지 실측**(`gorse.md` §12-5 X3): Push 0.1 ms/item(8 jobs) 선형, RAM 8.3 KB/item. 그 위는 회귀 |
| **R4** | 실제 보유 데이터에서 §7-3 형태의 품질 | 측정 | **배포 전 게이트로 승격**(`decisions.md` §4) — `D07`이 후보 생성 전체를 이 형태에 맡긴다. **합성 5×10⁵ 실측(`gorse.md` §12-5 X4)**: 다중 라벨 컷 100은 전원 최대 depth지만 전원 태그 6개(focus 필터); 희귀 하위·폭을 넣은 정답 기준으론 34/100 → `D24`(agent × 최상위 item) 뒤 94/100(X6, 25만 agent). 실데이터 게이트는 여전히 남는다 |
...
**R3·R4·R8을 실제로 재는 실험은 [`gorse.md` §12-5](gorse.md)의 `GOR-X1`–`X6`다** — R3은 X3, R4는 X1·X4·X6,
R8은 X2 — 단 R8은 **출시 전에는 측정 불가**라 X2가 시나리오(X3의 입력)와 출시 후 계측으로 나뉜다.
여기 표는 질문을 갖고 그쪽이 절차를 갖는다.

---

## 14. 이 문서가 하지 않은 것

- **이 문서는 도구를 도입하지 않는다.** §6·§7은 판단이고 선택이 아니다. 채택·기각은 `decisions.md`가 소유한다(`D06`·`D07`·`D22`–`D24`).
- **실제 데이터로 재보지 않았다.** §7의 실측은 합성 fixture(6~19 item), `gorse.md` §12-5는 합성 5×10⁵까지이고 **메커니즘 대조**까지만
  유효하다. 품질 주장이 아니다.

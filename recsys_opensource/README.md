...
| **후보 생성** | Gorse 라벨별 쿼리(§7-3) 또는 검색 인덱스(§7-5, 빈칸) | **Gorse** `recommend(user)` |
| **자격 gate** | 우리 (§9) | 우리 (§9) |
...
3. surface 1            grounding + 라벨별 쿼리 + 가중 합산 (§7-3)
                     rerank 는 규칙으로 시작
...
| **R4** | 실제 보유 데이터에서 §7-3 형태의 품질 | 측정 | **배포 전 게이트로 승격**(`decisions.md` §4) — `D07`이 후보 생성 전체를 이 형태에 맡긴다. **합성 5×10⁵ 실측(`gorse.md` §12-5 X4)**: 다중 라벨 컷 100은 전원 최대 depth지만 전원 태그 6개(focus 필터); 희귀 하위·폭을 넣은 정답 기준으론 34/100. 실데이터 게이트는 여전히 남는다 |

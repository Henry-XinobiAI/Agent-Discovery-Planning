# facets 소유권 분리 — 논의 기록 (rev 6, 2026-08-28)

> **문서 지위: 결정이 아니다.** 2026-08-26 대화에서 나온 구조 아이디어와, 그 판단에 필요한
> **검증된 사실**을 임시로 모아 둔 것이다. 어떤 항목도 확정이 아니고 어떤 코드도 이 문서를 근거로
> ⚠ **`D07` 이후 이 논의의 S3 서술은 fallback 경로의 것이다.** 후보 생성은 Gorse가 하고
> `/topics/{id}/users`는 Gorse 불능일 때만 탄다 — 아래의 rollup 수용·S3 캐시·S3 재시도 1회는
> 그 경로에 대한 이야기로 읽는다. 논의 자체는 그대로 유효하다(비용 축이 줄어들 뿐이다).
>
> 바뀌지 않았다. **설계 기준 문서는 `recommendation_pipeline_design.md`**이며, 이 문서가 기준 문서와
> 어긋나면 기준 문서가 맞다. 논의 후 결정이 생기면 그때 기준 문서 §S3·§S4·§9(열린 결정)로 올리고 이 문서는
> 그 결정을 가리키는 기록으로 남긴다.
>
> **표기**: `사실`(상류 소스에서 `file:line`으로 확인) · `판단`(이 문서의 의견) · `열린 항목`(협의·측정 대기).
> 사실과 판단을 섞어 쓰지 않는다 — 이 문서의 유일한 지속 가치는 앞의 것이다.
>
> **자매 문서**: `service_boundary_discussion.md` — 같은 경계의 다른 절반이다. 이 문서는
...
>
> - `archive/recsys_serving_discussion.md` §6-2 — 피드백 루프 기반 추천의 작업 기록. **그 절이 이 논의의 답을
>   선결 조건으로 지목한다**: topic-api를 토픽 조회로 좁히면 holder 인덱스 소유가 이쪽으로 오므로,
>   §7-1의 착수 자격(측정값 셋)이 그 그림에서는 더 이상 독립적으로 미룰 수 있는 항목이 아니다.
...
- 기준 문서(`recommendation_pipeline_design.md`) 변경 **없음**. §6-2의 계약 변경은 결정 후에 올린다.
...
- **2026-08-27 rev 3** — 자매 문서 `recsys_serving_discussion.md`(rev 1) 역참조 추가. 내용 변경은
  없다. rev 2에서 링크를 한 방향만 두면 경계 전체를 소유하는 것처럼 읽힌다고 적은 것과 같은 이유인데,
  이번에는 방향이 반대다 — 그 문서가 이 문서의 답을 **선결 조건으로** 삼으므로, 링크가 없으면 이쪽
  §7-1의 착수 자격이 여전히 독립적으로 미룰 수 있는 항목처럼 읽힌다.
- **2026-08-27 rev 4** — `recsys_serving_discussion.md` 참조를 rev 3으로 갱신. 논지 변경은 없다. 그
  문서가 같은 날 여러 rev로 움직이는 중이라 참조만 새로 고친 것이고, §7-1(착수 자격)에 대한 그쪽의
  지목은 rev 1 이후 바뀌지 않았다.
- **2026-08-27 rev 5** — `recsys_serving_discussion.md` 참조에서 **rev 고정을 제거**했다(`(rev 3)` → `(현행)`).
  논지 변경은 없다. 이유는 그 문서가 하루에 네 번 rev를 올리는 중이고, pin을 유지하려면 그때마다 이
  문서도 올려야 하는데 그 갱신이 담는 정보가 0이기 때문이다. 가리키는 절(§6-2)은 rev 1 이후 바뀌지 않았다.
- **2026-08-28 rev 6** — `recsys_serving_discussion.md` 참조에서 **버전 주장을 완전히 제거**하고 절 번호로
  바꿨다(`(현행)` → ` §6-2`). 그 문서가 `recommendation_scoring_design.md`로 승계되면서 "현행"이 틀린
  말이 됐는데, 가리키는 절은 그대로 거기 남아 있기 때문이다. 절 포인터는 rev가 올라가도 참이므로 이
  참조는 다시 stale해지지 않는다. 같은 정리로 **자매 문서 참조의 rev pin도 전부 제거**했다(README에
  규약으로 기록 — 살아 있는 문서는 pin하지 않고, `archive/`로 얼린 문서만 pin한다). 논지 변경 없음.

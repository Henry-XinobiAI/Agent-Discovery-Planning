...
| **`serving_surface_design.md`** | **설계 rev 1 (2026-08-24)** | 노출 표면: internal-only(`/api/internal/svc/<name>/` 단독) + edge-auth 관례 채택 + topic-api 레퍼런스 이디엄 차용 + 신규 template 구현 결정. x-user-id 금지·신원=body·label 유지·surface boundary 테스트 | 표면 계약 / template 델타 |

...
> **논의 기록 (결정 아님)** — `facets_ownership_split_discussion.md` (rev 5, 2026-08-27):
...
> **논의 기록 (결정 아님)** — `service_boundary_discussion.md` (rev 4, 2026-08-27):
...
> **논의 기록 (결정 아님)** — `recsys_serving_discussion.md` (rev 4, 2026-08-27):
> topic-api를 토픽 조회로 좁히고 대화 이벤트를 피드백으로 받아 학습하는 그림의 **사실·판단·열린
> 항목(R1–R13)**, **오픈소스 recsys 도입 평가**(§5), **GPU가 필요한 지점**(§5-6 — 어디에도 없다),
> **도입 형태**(§5-7 — 여섯 단계 중 S5 하나를 바꾸는 라이브러리이고 재작성이 아니다).
> 계약은 이 그림의 제약이 아니라는 전제로 rev 2에서 논거를 재도출했고(§3-4가 무엇이 계약에서 나왔고
> 무엇이 측정·산수·성질에서 나왔는지 가른다), 방법은 facets 선례를 따른다 — **이음매는 만들고 정책은
> 만들지 않는다**(§3-0). 시급한 항목은 R1 하나뿐이다: 후보별 노출·propensity 이벤트는 **서빙 시작
> 전에** 정해야 나중에 보정이 가능하다(§3-0-2 — facets는 기다려도 되지만 피드백은 기다리면 사라진다).
> **rev 4**는 bourbon-agent의 persona 생산·저장 경로를 예제까지 정리했다(§2-11) — 마크다운 헤딩이 곧
> 토픽이고, `sharable`/`private`에 상류 fail-closed 정책이 이미 있고, `doc`이 AES-GCM으로 암호화되어
> **테이블 조인 경로가 없다**(수신은 API 하나뿐, R14).

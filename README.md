...
| 2 | [`decisions.md`](decisions.md) | 오너가 정한 것(R01–R45)과 정해지지 않은 것(O5·O6·O15·O20 — 나머지 O1~O19·O21은 닫힘). **본문이 이 표와 다르면 표가 맞다** | 유효 |
...
| 9 | [`implementation_plan.md`](implementation_plan.md) | 구현 단계와 완료 조건(R43): 갭 정리 → 로컬 스택 → 설정·계약 → 저장·적재 → 타입 ①② → 생성기·적재 스크립트·CLI → 타입 ③ → CF → 검증 → 배포 준비. 순서의 이유와 열린 구현 판단 | 계획 |
| 10 | [`requests/`](requests/README.md) | 받는 곳별 요청서(인프라·bourbon-api·클라이언트·bourbon-agent·topic-api). 이벤트 정의서 §6의 묶음을 붙일 수 있는 문장으로. 보내는 시점은 1차 배포 즈음 | 초안 |

...
| 문서 | 레이어 | 다루는 것 | 핵심 단위 |
|---|---|---|---|
| **[`decisions.md`](decisions.md)** | **결정 레지스터** | **현재 유효한 결정 전부(D01–D26 중 25건, D17은 회수) · 뒤집힌 결정 · 결정이 없어서 문서가 갈라진 자리 · 열린 항목 레지스터 지도.** 설계 문서와 어긋나면 이쪽이 맞다 | 결정 |
| [`agent_discovery_redesign.md`](agent_discovery_redesign.md) | **재설계 출발점 (2026-09-08)** — 제품 모델(세 타입·visibility·friend·성숙도), 두 구현안(오픈소스 중심 / 직접 구현)과 혼합, 10만 합성 유저 비교 프로토콜 | **이전 결정·제약을 전제하지 않는다.** 오너 설명 2026-09-08 기준. 결정 아님 — §8 열린 질문에 답이 붙고 §6 비교가 끝나면 결정 레지스터를 다시 쓴다 | 출발점 |
| [`agent_discovery_contract.md`](agent_discovery_contract.md) | 공통 계약 — 타입별 route·요청·응답, 후보 소스/필터/랭커 인터페이스, 불변식 8개, 열린 row 저장 모델, feature 표, 결정 로그, 받을 이벤트 | 재설계 §9-2. 두 구현안이 똑같이 지키는 것만. 초안 2026-09-08 | 계약 초안 |
| [`agent_discovery_events.md`](agent_discovery_events.md) | 이벤트 정의서 — 있는 것(`topics_updated`, `message_created`, `friendship_changed`, `user_deactivated`)을 어떻게 소비하고, 없는 것(`user_topic_settings_updated`, `agent_dm_opened`, agent 공개 여부)을 우리가 먼저 정의해 시험한 뒤 요청하는지; 발행 지점마다 필드 존재를 검증 | 재설계 §9-3. rev 2(2026-09-08, pull된 최신 코드). 오너 확인 5건. 미발신 | 정의서 초안 |
| [`synthetic_population_spec.md`](synthetic_population_spec.md) | 합성 모집단 생성기 — 카탈로그 기반 군집·topic·friend·대화 로그·이벤트 열·정답, 파라미터 표, 두 주입 경로, 검증 | 재설계 §9-4. 두 구현안 비교의 정답 데이터. 초안 2026-09-08 | 스펙 초안 |
| [`recsys_build_vs_adopt.md`](recsys_build_vs_adopt.md) | 후보 생성 — 기존 오픈소스 도입 vs 우리 인덱스 | **논의 정리(2026-09-07, 결정 아님).** 다른 문서 없이 읽히도록 제약을 글로 풀었고 이전 결정을 근거로 쓰지 않는다. CF·content·검색 엔진·임베딩·우리 인덱스의 장단점과 채택하기 힘든 측면, 혼합 패턴, 저장소 후보의 도쿄 단가, 지연 추정 | 논의 |

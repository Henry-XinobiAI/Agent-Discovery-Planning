...
| **`recommendation_pipeline_design.md`** | **설계 정본 rev 5.11 (2026-08-27, 외부 리뷰 5회 반영 + 응답 wire 확장: 4값 + `owner_notes`)** | topic-api 소비 기반 추천 파이프라인 S1–S6: 단계별 동작·topic-api endpoint·산출물·실패 3분기. `persona_topic_search_design.md`(rev 14)를 대체(2026-08-24 archive) — 열린 결정은 §9 | 파이프라인 단계 산출물 |

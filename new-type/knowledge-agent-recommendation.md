...
| 규모 | 사용자 10만 × 사용자당 기록 수백 = 수천만 행까지 무리 없다(추론, 슬라이스 0에서 기록 수를 잰다) | 더 큰 규모에 유리 |
...
**다시 볼 조건**: 슬라이스 0·2에서 요약문 검색의 품질이 부족하면(§13-2) — 예를 들어 영어 번역이 제품명·지명을 흔들어 매칭이 떨어지면 — 검색만 OpenSearch로 옮기고 기록의 기준은 PostgreSQL에 둔다. 새 서비스가 memory-api로 옮겨 가는 날에도 같은 판단을 다시 한다.
...
슬라이스 1은 discovery의 T1로 시작하고, 지연 시간이 문제가 되면 이 대안을 검토한다. 두 방식을 같은 질문 세트로 비교할 수 있게, 요청이 need 목록을 받으면 T1을 건너뛰는 입구를 처음부터 열어 둘지는 열린 항목 18이다.
...
**빼 둔 결정이 이 기능의 가치를 좌우한다.** 경험 기록이 가장 많이 나올 곳은 아마 사용자가 자기 agent와 나눈 대화(`agent_dm`)일 것이다 — "나 어제 이거 마셨는데" 같은 말을 사람보다 자기 agent에게 더 편하게 할 것이라고 본다(추론). 그리고 그곳이 공개 범위 문제가 가장 큰 곳이다. 슬라이스 0에서 기록 수를 방 종류별로 나눠 재면(§13-2) 이 결정이 얼마나 무거운지 미리 보인다.
...
**6과 7이 슬라이스 2를 진행할지 정하는 기준이다.** 이 두 숫자가 낮으면 경험 소스를 만들어도 대부분의 추천이 `prior_only`(관심 소스)로 떨어져 체감 품질이 오르지 않는다. 그때는 기록이 쌓일 원천(예: `agent_dm`의 범위 결정, §12)을 먼저 정하거나 슬라이스 2를 미룬다. 기준값은 슬라이스 0의 결과를 보고 정한다.
...
|---|---|---|---|
| **0. 추출 spike** | 새 서비스 골격, dev 메시지 읽기 | — (측정만) | 사전 필터·추출·레지스트리를 dev 샘플에 돌려 §13-2의 일곱 가지를 잰다. 추출 모델과 저장소(§7-6)를 정한다. **6·7번으로 슬라이스 2의 진행 여부를 정한다** |
| **1. 관심 소스 + 한 명 추천** | 없음 | 이 분야에 관심을 드러낸 사람(`prior_only`) | route·envelope, T1 형제 프롬프트(새 필드를 전부 추출해 decision log에), `knowledge` facet 미러, deterministic 순위, `people_needed` 판정 |
| **1-b. 두 사람 조합** | 없음 | 두 need를 두 사람이 나눠 채우는 경우 | 조합 탐색, 최대 2명 |
| **2. 경험 소스** | 새 서비스(이벤트 소비, 추출, 저장, 조회 API) | 이것을 겪었다·좋아한다·해 봤다·안다고 말한 사람 | 네 검색 경로, 요약문 검색, 요약문 기반 이유, 합성 대화 측정 |
| **3. `evidence_ref`** | bourbon-agent 실행 경로(+ 원 메시지가 필요하면 memory-api `messages/lookup`) | 추천받은 agent가 근거에서 바로 답을 시작하는가 | 발급·조회, 소유자 확인, calibration을 `evidence_ref` 조회 여부로 나눠 재기 |
| **4. 트리거** | bourbon-agent 변경 | 실제로 불리는가 | tool 설명 변경 또는 두 번째 tool, 호출률 |
| **5. 실행·종합** | bourbon-agent | 실제 답변 | 이 문서 범위 밖 |
| **서비스화** | 공개 범위·동의 결정(§12), 내부 호출 인증(§10-3) | — | 남겨 둔 자리에 필터를 붙이고, production에 적재 |

- 슬라이스 0이 먼저다. 추출 정밀도, 물량, 그리고 기록이 추천에 쓸 만큼 쌓이는지를 모르면 슬라이스 2의 가치와 비용을 말할 수 없다.
- 슬라이스 1·1-b는 새 서비스 없이 배포된다. 그 결과는 **관심 기반 추천**이지 "겪은 사람" 추천이 아니므로, 명시적인 추천 요청 경로에서만 내보낸다.

"글렌드로낙 이번 신상 마셔 본 사람" 질문이 슬라이스마다 어디까지 가는가:

| 도달한 슬라이스 | 나오는 사람 |
|---|---|
| 1 | 위스키·몰트 위스키에 관심이 큰 사람 |
| 2 | 글렌드로낙(또는 그 신상)을 마셔 봤다고 말한 사람이 엔티티·상위 엔티티 경로로 들어온다. 최근에 말한 사람이 위로, `summary_en`이 "newest release"와 맞으면 맨 위 |
| 3 | 그 사람의 agent가 대화할 때 그 경험 기록에서 답을 시작한다 |
...
18. 질문 분석을 discovery의 T1에 둘지, bourbon-agent의 tool 인자로 옮길지(§9 T1). 그리고 요청이 need 목록을 받으면 T1을 건너뛰는 입구를 슬라이스 1부터 열어 둘지.

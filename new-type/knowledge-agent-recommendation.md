...
1과 3은 lived-knowledge가 질문과 기록을 **같은 엔티티 레지스트리로** resolve해서(§7-3) 줄어든다. 2와 4는 아래 규칙으로 막는다.
...
두 흐름이 있다. 메시지가 올라올 때 lived-knowledge가 경험 기록을 쌓는 **적재 흐름**, 질문이 올 때 discovery가 추천을 만드는 **추천 흐름**이다.

### 4-1. 적재 흐름 — lived-knowledge

```text
bourbon-api ── bourbon.message_created (id, 보낸 사람, 방 종류) ──▶ lived-knowledge
                                                                   │

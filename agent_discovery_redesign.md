...
검증 결과 문서 한 편. 표로 정리된 위 측정치와, 그 위에서 정하는 **설정 초기값**(랭커 가중, CF 게이트 임계, 갱신 값). 그 문서는 근거이고, 값은 코드의 `agent_discovery/settings.py`에(R54), 뜻은 `agent_discovery_settings.md`에, 결정은 `decisions.md`에 R 행으로 적는다.

...
- **"비슷한 사람들이 좋아했던"** = CF. (요청자, 타인 agent) 상호작용이 있어야 한다. 아이템 수가 유저 수와 같고, 한 유저가 대화해 볼 agent는 많지 않으므로 행렬은 매우 희소하다. CF가 의미 있으려면 로그가 어느 정도 쌓여야 하고, 그 전까지는 다음 순서로 간다.
  ...
  2. **content 유사도**: 요청자의 topic 집합과 겹치는 topic을 가진 agent. 지금 정의는 topic id 정확 일치의 가중 합이고, O18의 제안은 카탈로그 계층을 따라 감쇠하고(topic-api 검색 설정에서 빌린 0.6/hop) 흔한 topic은 IDF로 낮추며 임베딩 kNN은 넣지 않는 것이다 — 카탈로그가 이미 의미 계층이라는 판단이고 확정은 O18이다. 요청자의 프로필은 본인 `public`·`friends`·`private` topic이고(R22), 타인 agent 방에서 한 말도 bourbon-agent의 추출 → topic-api를 거쳐 여기로 들어온다. 그래서 O18 제안은 "대화한 agent의 topic을 프로필에 더하는" 별도 경로를 넣지 않는다.
...
- **`private`, `hidden`은 후보 저장소에 들어오지 않는다.** 타인의 것은 그 두 tier를 읽지 않는다. 미러에 없으니 셀 수도 없다. 요청자 **자신의** topic 목록(타입 ②③의 입력)은 본인의 `public`·`friends`·`private`을 읽고 `hidden`은 읽지 않는다(R22). 어디서 읽는지만 미정이다(결정 레지스터 O2).
...
| content 유사 유저 | 요청자 topic 집합과의 가중 겹침 — 지금은 정확 일치의 `score` 가중 합, O18 제안은 IDF 가중 + 카탈로그 계층 감쇠 0.6/hop. 벡터 kNN은 O18이 다시 볼 조건(grounding 실패 비율)이 나쁠 때만 | PostgreSQL (같은 인덱스 + 카탈로그 edge 테이블). 벡터 kNN은 pgvector / OpenSearch |
...
| 저장 용량, 월 비용(도쿄 단가) | |
| 컴포넌트 수, 장애 시 동작(엔진 다운, `cf_candidates` 비어 있음, 이벤트 유입 중단 — `friendship_changed`·`topics_updated` 지연) | |
...
| 운영 | 컴포넌트 수, 장애 주입 3종(엔진 / `cf_candidates` 비어 있음 / 이벤트 유입 중단)의 응답 |
...
8. A안의 엔진을 Gorse(서비스형)로 하나 `implicit`(라이브러리형. LightFM은 Python 3.14 빌드 실패로 제외)으로 하나. 둘 다 재려면 시간이 두 배다 (O8).

---

## 9. 다음 할 일

1. 오너 답 받기 — 2026-09-08에 넷(타입 ② 모양, 상한, friends, 호출 API) 완료. 남은 것은 결정 레지스터 O1~O19(O10은 닫힘).

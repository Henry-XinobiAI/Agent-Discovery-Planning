...
> **마지막 조건은 1-8에서 충족되지 않았고, 코드 문제가 아니다**(2026-09-12). 두 가지가 겹친다. (1) §3-1의 수치는 **그 스파이크의 생성기**로 잰 것이고, 1-6 이후의 `synthetic/`으로 다시 재면 λ=10에서 ALS가 인기도의 0.45배다(§3-1에 붙인 주석 참고) — `gate.min_hr_ratio` 1.0을 넘기지 못한다. 게다가 §3-1 표 자체로도 0.0096/0.0112 = **0.86**이라 1.0을 넘지 않는다("동률"은 산문이고 임계는 부등호다). (2) "같은 군집"은 실서비스에 입력이 없어 대리 지표를 쓰는데(계약 §11), 그 눈금에서는 인기도가 더 높다. **임계를 넘길 때까지 조정하지 않았다** — 그것이 R35가 막으려는 바로 그 일이다. 1-8은 게이트를 계약대로 구현하고, λ=3·λ=10 양쪽에서 닫힌다는 것과 λ가 커질수록 CF가 상대적으로 나아진다는 방향을 라이브 테스트로 고정했다. 어느 수치가 맞는지는 1-9의 측정 문제다.
...
- **IDF 분모의 비율**: 13-3이 모집단을 스냅샷으로 옮긴 뒤 이 항이 p50/p95에서 얼마를 차지하는지 확인만 한다(크기는 13-3에서 이미 쟀다).

...
> `agent_discovery_redesign.md` §9의 5·6번을 단계로 자른 것(2026-09-10). 결정은 `decisions.md`, 지켜야 할 것은 `agent_discovery_contract.md`, 값은 `agent_discovery_settings.md`다. 이 문서는 **순서와 완료 조건**만 정한다. 코드 저장소에서 단계를 시작할 때 `tasks/todo.md`에 체크 항목으로 옮기고, 끝난 단계는 여기에 날짜를 적는다.
...
- **외부 이벤트는 아직 오지 않는다.** 우리 CLI가 로컬 브로커에 발행해 왕복을 시험하고, 품질은 합성 모집단으로 본다. 요청서 다섯 개는 `requests/`에 있다.
- **산출물에 반드시 포함**: 합성 데이터를 로컬 저장소에 넣는 적재 스크립트, 세 타입을 호출하고 이벤트를 발행하는 시험 CLI.

## 1. 단계

각 단계는 "완료 조건"이 전부 참일 때 끝난다. 테스트는 단계마다 먼저 쓴다(실패하는 테스트 → 구현). `pre-commit run --all-files`는 매 커밋.

### 1-1. 갭 정리 — 지금 코드에서 무엇을 남기고 무엇을 바꾸나

코드 저장소는 재설계 이전 모양이다(코드 읽은 기준: bourbon-agent-discovery-api main `b096aa2`, 2026-09-10 — `agent_discovery/`·`api/`·`worker/`·`cli/` 7,151줄, 765 테스트). 먼저 인벤토리를 만들고 그 뒤 단계를 시작한다.
...
| `worker/` 뼈대 (app, health, redis, scheduling, budget, ingest 재시도) | deferq 호스팅·debounce·heartbeat·재시도가 계약대로다. 리스너와 task만 바뀐다 |
| `api/main.py`, `api/middleware/`, `api/structs/errors.py`, 배포 매니페스트, `tests/test_deploy_env_wiring.py` | 서비스 껍데기 |
...
| `api/routers/events/user-topics-changed` route (저장소 없이 읽고 기록만) | 재조회 task가 저장소에 쓴다. 워커가 자기 API를 HTTP로 찌르는 구조는 유지할지 1-4에서 결정(아래 "열린 구현 판단" 1) |
| `agent_discovery/composition.py`의 요청 tier `public`만 | 타인 row `public`·`friends`, 요청자 프로필 `public`·`friends`·`private`(R22) |
| `api/routers/recommend/` `POST /recommend` 단일 route | `/recommend/explicit`(내부) + `/discover/by-topic`·`/discover/for-you`(클라이언트 + 내부 미러). 기존 route는 bourbon-agent 이행까지 유지 |
...
**완료 조건**: 위 표가 `tasks/todo.md`에 옮겨져 있고, 삭제 대상의 테스트 수를 알며, 남기는 모듈의 테스트가 그대로 초록.
...
- 카탈로그(R26): `scripts/sync_catalog.py`가 topic-api 저장소의 `data/catalog_dist/catalog.json`을 우리 `data/catalog_dist/`로 복사하고 `built_at`·원본 커밋을 옆 파일에 남긴다. 복사본은 **커밋한다** — 빌드가 다른 저장소를 읽지 않게. 바뀌면 스크립트 재실행 → 커밋 → 재배포.
...
**완료 조건**: `docker compose up -d` 뒤 `alembic upgrade head`와 테이블 생성이 돌고, 목 서버가 빈 응답을 답하며, 카탈로그 edge 테이블에 2,959 edge가 적재된다.
...
**완료 조건**: 계약 §2·§3의 예시 JSON이 모델로 파싱·직렬화되고, 레지스터-클래스 일치 테스트가 초록.

### 1-4. 저장 모델과 적재 경로

PostgreSQL: `visible_topic_rows`(+ `descriptions` JSON, hydration용), `agents`, `friends`, `interactions`, `catalog_edges`, `population_stats`. 인기도는 `interactions`에서 요청 시 계산(30일 창이라 작다) — 별도 테이블은 p95가 말할 때.
DynamoDB(계약 §6-1): `USER#…/CF_CANDIDATES`, `REC#…/LOG`, `EVT#…`, `CONFIG/CF_GATE`.

리스너(전부 `worker/`에 한 흐름씩): `topics_updated`·`user_topic_settings_updated` → 같은 debounce → 재조회 task(row 통째 교체, `agents` 없으면 생성, `topic_revision` 복제 지연 재시도); `personal_agent_visibility_changed`; `agent_dm_opened`(→ `interactions`, `agents.last_active_at`); `message_created`(`room_type` 필터 → `interactions.turns`); `friendship_changed`; `user_registered`(agent id 결정론 계산, `email` 읽지 않음); `user_deactivated`. 모든 리스너가 `event_log`에 append.
...
**완료 조건**: CLI로 `topics_updated`를 세 번 발행하면 재조회 1회·row 교체 1회, `user_topic_settings_updated`로 private 전환 뒤 row가 사라지고, `agent_dm_opened` → `interactions` 1행 + `last_active_at` 갱신, 각 이벤트가 `event_log`에 1건. 불변식 1(저장소에 private·hidden 없음)을 테스트가 강제.

### 1-5. 타입 ①② — 인덱스 소스와 응답

- `topic_index` 소스: 계약 §6의 쿼리(술어 안에 친구 배열, GROUP BY owner, 커버리지 정렬). 실측 스파이크의 SQL을 옮긴다.
...
**완료 조건**: walkthrough §3·§4의 예제 세계를 적재하면 문서의 결과 순서가 그대로 나온다(문서의 예시값을 테스트 픽스처로). 위반 0건 테스트(friends row가 비친구에게 나가지 않음).

### 1-6. 합성 생성기, 적재 스크립트, 시험 CLI

- `synthetic/`(코드 저장소): 스펙 §2~§5 그대로. 출력 파일 형식은 스펙 §5. 시드 고정.
- `scripts/seed_local.py`: 출력 파일을 로컬 PostgreSQL·DynamoDB·목 서버 데이터로 적재(10만 유저 기본, `--users`로 축소). 이벤트 스트림은 CLI 발행기를 재사용해 브로커에 넣는 모드와, 저장소에 직접 넣는 빠른 모드 둘.
...
- `popularity` 소스(R29: 30일, 반감기 7일, confidence 가중, 신규 사전값, 최대로 정규화).
- `content_similarity` 소스(R25: IDF × `0.6^hop`, 양방향 최단 hop ≤ 3, 경로당 최구체 일치 하나, drawer 통과, `catalog_edges`로). 모르는 topic id → 정확 일치 + 지표.
- `UserQuery`, 이미 대화한 상대 제외와 소진 뒤 재등장(R28, `talked_before`는 로그만), 구간 섞기(R20, seed = `recommendation_id`), `cf_candidates` 없으면 콜드스타트 경로.
- `/discover/for-you` + 커서.

**완료 조건**: walkthrough §5의 예제로 §5-5 표의 점수가 그 표의 가중(pop .4 / content .4 / cf .2)과 cf 열 값을 입력으로 주면 재현되고, 섞기가 seed로 재현되며, ablation "인기도만"·"content만"이 스펙 §4의 정답으로 recall@10을 낸다.
...
- 게이트(R35, 레지스터 §4): 실로그 leave-one-out HR@10·군집 다양성, `w_cf` 조정, `CONFIG/CF_GATE`에 기록.
...
- go-live 전제 확인: bourbon-api `discoverable`·`agent_dm_opened`, 클라이언트 `recommendation_id`, topic-api api AMQP, prod topic-api 워커.

## 2. 순서의 이유

- 생성기(1-6)가 타입 ③(1-7)보다 앞인 것은 R34가 검증을 그것으로 정했기 때문이다. 타입 ①②(1-5)는 walkthrough 예제만으로 검증되므로 생성기 전에 끝낸다.
- 저장 모델(1-4)이 route(1-5)보다 앞인 것은 불변식 1·4가 저장소에서 판정되기 때문이다.
...
2. `popularity`를 요청 시 집계하나 테이블로 두나 — 요청 시로 시작, p95가 말할 때 테이블.
3. 목 서버를 우리 저장소에 두나, topic-api 저장소의 dynamodb-local compose를 그대로 띄우나 — 목 서버(우리 합성 데이터를 답해야 하므로).
...
- **topic-api prod 워커가 0 replicas인 채면** `topics_updated`가 없다. dev에서 시험하고 prod는 그 전제가 풀린 뒤.

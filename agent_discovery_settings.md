...
| `friends.tombstone_ttl_days` | 30 | 끊긴 친구관계를 이벤트 순서 비교용으로 얼마나 더 들고 있나(R50) | 올리면 뒤늦게 재발행된 옛 `accepted`도 계속 걸러낸다. 내리면 그 창 밖의 옛 이벤트가 친구관계를 되살린다. 0은 지우는 설계로 되돌아가는 것과 같다 |

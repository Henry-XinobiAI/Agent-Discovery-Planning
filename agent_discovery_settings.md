...
| `attribution.window_hours` | 클라이언트의 대화 시작 보고(계약 §2-5)와 대화의 첫 메시지를 같은 `(actor, owner)` 쌍으로 잇는 최대 간격 | 늘리면 오래된 카드에서 시작한 대화도 추천으로 잡히고, 줄이면 늦게 연 대화가 `direct`가 된다 | R47, 값은 분석 |
| `attribution.predicted_room_ttl_hours` | 추천으로 나간 카드의 예측 room id를 얼마나 들고 있나(R51) | 올리면 늦게 시작된 대화도 소유자를 찾지만 항목이 오래 남는다. 내리면 그 밖에서 시작된 대화는 기록되지 않는다 — `attribution.window_hours`보다 짧으면 보고는 있는데 소유자를 모르는 방이 생긴다 | R51, 값은 분석 |
| `attribution.max_impression_cards` | 목록 하나의 결정 로그 항목이 노출 보고(계약 §2-6)로 받는 **카드 수** | **건수가 아니라 카드 수인 이유**: 항목이 바닥나는 것은 바이트이고 보고 하나가 카드 여러 장을 싣는다 — 건수만 묶으면 항목이 400 KB를 넘고, 넘긴 항목은 같은 목록의 `pages[]`까지 못 받는다(답변 경로의 기록이다). 올리면 오래 스크롤한 화면도 끝까지 기록되고 항목이 커진다. 내리면 **아래쪽 선반의 노출부터 잃는다** — 보고가 화면 순서대로 오므로 잘리는 쪽은 언제나 덜 본 쪽이고, 그러면 "아래 선반은 안 본다"가 데이터가 아니라 이 값의 그림자가 된다. 타입 ②가 먼저 닿는다 | R66, 값은 분석 |
...
| `bourbon_api.timeout_ms` | 응답 직전 표시 필드 hydration **전체**의 상한(R63 — 두 route, 모든 chunk, 그 안의 재시도까지). **실제로 쓰는 값은 이것과 요청에 남은 시간 중 작은 쪽**이다: 페이지는 이미 정해진 뒤라, 여기서 라우트 데드라인을 넘기면 완성된 답이 503이 된다 | 내리면 `hydration_partial`이 늘고 p95가 짧아진다. **이 값은 라우트 데드라인보다 한참 짧아야 한다** — 목록의 길이도 순서도 이 호출에 달려 있지 않으므로, 답을 늦추면서까지 기다릴 이유가 없다. 초기값은 bourbon-topic-api가 같은 peer에 쓰는 2초(2000)를 따라간다(`topic/config.py`의 `BOURBON_API_TIMEOUT_SECONDS`). dev 관측에서 두 배치 조회의 실제 분포를 보고 줄인다 | R63, 초기값은 topic-api의 같은 값 |
...
| `decision_log.ttl_days` | 결정 로그 보존 | 오프라인 평가 기간보다 짧으면 정답 로그가 사라진다. 보정 창(`calibration.window_days`)의 4배로 시작. **숫자를 두 곳이 쓰는데 집행되는 곳은 하나다** — DynamoDB 쪽은 `expires_at` TTL이라 AWS가 지우고, PostgreSQL `attributions`는 같은 기간을 인용만 할 뿐 지울 잡이 없다 | 계약 §8, 값은 분석 |

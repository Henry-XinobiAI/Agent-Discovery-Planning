...
| `attribution.max_impression_reports` | 목록 하나의 결정 로그 항목에 노출 보고(계약 §2-6)를 몇 건까지 붙이나 | 올리면 오래 스크롤한 화면도 끝까지 기록되지만 항목이 커진다(DynamoDB 항목 400 KB 상한이 진짜 상한이다). 내리면 **아래쪽 선반의 노출부터 잃는다** — 보고가 화면 순서대로 오므로 잘리는 쪽은 언제나 덜 본 쪽이고, 그러면 "아래 선반은 안 본다"가 데이터가 아니라 이 값의 그림자가 된다. 타입 ②가 먼저 닿는다 | R66, 값은 분석 |

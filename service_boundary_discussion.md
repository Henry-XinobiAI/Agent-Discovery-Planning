# 서비스 경계 — topic-api ↔ agent-discovery 논의 기록 (rev 5, 2026-08-28)
...
> **설계 기준 문서는 `recommendation_pipeline_design.md`**이며, 이 문서가 기준 문서와 어긋나면 기준 문서가 맞다.
> 결정이 생기면 그때 기준 문서 §S1·§S2·§9(열린 결정)로 올리고 이 문서는 그 결정을 가리키는 기록으로 남긴다.
>
> **표기**: `사실`(상류 소스에서 `file:line`으로 확인) · `판단`(이 문서의 의견) · `열린 항목`(협의·측정 대기).
>
> **자매 문서**: `facets_ownership_split_discussion.md` — 같은 경계의 *값 소유* 쪽 절반이다.
> 이 문서는 *해석과 순위 정책* 쪽 절반을 다룬다. §4-2에서 둘이 만난다.
>
> `archive/recsys_serving_discussion.md` §6-3 — 피드백 루프 기반 추천의 작업 기록. **그 절이 §3의 경계 규칙과
...
→ **"요청 경로는 결정적 인덱스 읽기로 유지한다"는 선을 topic-api 저자들이 스스로 그어두고 있다.**
...
하고(422 매칭 없음 / 422 모호 / 503 판단 도달 못 함, 기준 문서 §S2) 검색 화면은 후보를 보여줘야 한다.
...
미결: 요청을 발신할지, 어떤 형태(인덱스 방식·계약·비용 부담)로 할지. 발신 전이다.
...
> ⚠ **`SB1`–`SB5`는 논의 기록의 항목이고 결정이 아니다**(2026-09-03 표시). 이 논의의 전제(우리가
> 저장소 0의 소비자라는 것)는 `decisions.md` `D07`·`D10`으로 바뀌었으므로, 아래를 현재 계획으로
> 읽지 않는다.
...
| SB4 | 경계 규칙을 기준 문서에 절로 올릴지 (§3의 한 줄 규칙) | 미정. 올린다면 `recommendation_pipeline_design.md` §10(구조 원칙) |
...
- 기준 문서(`recommendation_pipeline_design.md`) 변경 **없음**. §3의 규칙을 §10으로 올리는 것은 SB4다.
...
- **2026-08-27 rev 3** — 자매 문서 두 개의 참조 rev 갱신(facets rev 4, recsys rev 3). 논지 변경 없음.
  SB1의 상태도 그대로 "미발신"이다.
- **2026-08-27 rev 4** — `recsys_serving_discussion.md` 참조에서 **rev 고정을 제거**했다(`(rev 3)` → `(현행)`).
  논지 변경은 없다. 이유는 그 문서가 하루에 네 번 rev를 올리는 중이고, pin을 유지하려면 그때마다 이
  문서도 올려야 하는데 그 갱신이 담는 정보가 0이기 때문이다. 가리키는 절(§6-3)은 rev 1 이후 바뀌지 않았다.
- **2026-08-28 rev 5** — `recsys_serving_discussion.md` 참조에서 **버전 주장을 완전히 제거**하고 절 번호로
  바꿨다(`(현행)` → ` §6-3`). 그 문서가 `recommendation_scoring_design.md`로 승계되면서 "현행"이 틀린

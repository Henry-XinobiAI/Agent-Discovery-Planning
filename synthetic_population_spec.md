...
- **입력**: topic-api 카탈로그 파일(`data/catalog_dist/catalog.json`, 3190 topic, 3136 부모 간선, 라벨 ko/en/ja), 파라미터 하나의 묶음(§2), 시드.
...
| 실제 | 생성기가 따르는 것 |
|---|---|
| 카탈로그 3190 topic, 63 상위 그룹, 최대 7단계(응답은 3단계) | 카탈로그 파일을 그대로 읽는다. topic id는 카탈로그 id 그대로(현재 3190개는 모두 32 hex이지만 타입은 `[A-Za-z0-9_-]{1,64}`라 형식을 가정하지 않는다). 군집의 선호는 **상위 그룹**(root부터 2단계) 단위로 정의하고 그 아래 leaf를 샘플링한다. **이 수는 카탈로그가 다시 빌드될 때마다 움직인다** — 2026-09-18 빌드에서 3003·54에서 위 값으로 바뀌었고, 코드의 `tests/synthetic/test_catalog.py`가 그 변화를 실패로 드러낸다 |
...
> 아래 값은 **실험 설계**다 — 설정 레지스터와 달리 R54의 "값은 코드"에 해당하지 않는다. 서비스가 튜닝하는 값이 아니라 어떤 모집단을 만들어 무엇을 재는지를 정하는 값이고, λ의 세 값처럼 오너 결정(R16)인 것도 있다. 코드의 `synthetic/params.py`가 **같은 이름**으로 이 표를 담고, 한 run은 자기가 실제로 쓴 값을 `params.json`으로 쓴 뒤 그 해시를 `run_id`로 삼는다(§5). 그러므로 **어떤 모집단에서 잰 숫자인지는 그 run의 `params.json`이 말하고, 이 표는 말하지 않는다** — 검증 결과를 읽을 때 이 표를 근거로 삼지 않는다.

| 이름 | 기본 | 뜻 |
|---|---|---|
| `U` | 100,000 | 유저 수 |
| `seed` | 20260908 | 모든 난수의 시드 |
| `K` | 60 | persona 군집 수 (카탈로그 상위 그룹과 비슷한 자릿수, 그룹과 1:1은 아니다 — 정할 때 54였고 2026-09-18 빌드에서 63이 됐다. 60은 둘 다에 가까우므로 움직이지 않는다. 1:1로 짝지었다면 카탈로그가 이 값을 정하게 됐을 것이다) |
...
1. 카탈로그를 읽어 상위 그룹 `G`와 각 그룹의 leaf 집합을 만든다. **그룹은 root다**(1-6에서 파일을 재고 정했다). §1 표의 "63 상위 그룹"과 여기 "상위 2단계"가 갈리는데, 한 단계 아래(610개)에서는 **중앙값 그룹이 leaf 하나**라 군집의 주 취향이 topic 하나가 되고 커버리지 정렬이 전부 동점이 된다. root는 중앙값 14개, 평균 44개. 2026-09-18 빌드에서 다시 쟀고 판단은 그대로다 — 바뀐 것은 수이고, root를 고른 근거인 두 중앙값의 간격(14 대 1)은 그대로다.

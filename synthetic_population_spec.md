...
| 실제 | 생성기가 따르는 것 |
|---|---|
| 카탈로그 3003 topic, 54 상위 그룹, 최대 7단계(응답은 3단계) | 카탈로그 파일을 그대로 읽는다. topic id는 카탈로그 id(32 hex) 그대로. 군집의 선호는 **상위 그룹**(root부터 2단계) 단위로 정의하고 그 아래 leaf를 표본으로 뽑는다 |
| 유저 topic row: `score` 0~100 (facet 6개의 가중 평균), `visibility` 4값, `revision` | `score`는 0~100 실수, `visibility`는 `private` 기본에서 열린 것만 public/friends. `revision`은 쓰기마다 +1 |
...
| 성숙도: topic은 `score`, agent는 컴포넌트 예정 | topic은 `score` 그대로. agent 성숙도는 열린 topic 수와 총 turn 수에서 유도한 0~1 |
...
| `tier_mix_of_opened` | public 0.4 / friends 0.2 / private 0.4 | 연 유저의 topic별 tier 비율 (private = 열지 않은 것) |
...
| `revisit_rate` | 0.3 | 같은 (actor, owner) 쌍이 다시 열리는 비율 |
...
1. 유저 `u`: `user_id` = uuidv7(시드 기반 시각), 주 군집 1, 부 군집 0~2, `registered_at`(기간 안 균등, 최근일수록 topic 적게).
2. topic 수를 뽑고, `off_cluster_topic_rate`만 무작위, 나머지는 (주 군집 0.7, 부 군집 0.3)의 선호에서 Zipf 표본. 중복 제거.
...
3. 대화마다 `started_at`, `turns`, `reopened`(revisit), `entry`(처음엔 전부 `"direct"` — 우리 추천이 없던 세상), `recommendation_id=null`.
...
| 타입 | 정답 | 지표 |
|---|---|---|
| ① | 요청자에게 열린 agent를 (커버 topic 수 desc, `score` 합 desc)로 정렬한 목록 | 커버리지 위반 0건(상위 k에서 커버 수가 역전된 쌍), NDCG@10 |
| ② | 섹션 topic마다 그 topic을 요청자에게 열어 둔 agent를 `score` desc | 섹션별 recall@N, 순서 일치율 |
| ③ CF | 요청자 `u`에 대해 `affinity[cluster(u)][cluster(owner(a))] × pop[a]`가 큰 순, 열린 것만, 이미 대화한 것 제외 | recall@10, NDCG@10 |
...
| `params.json` | 1 | — |
| `clusters.parquet` | K | — |
| `users.parquet` | U | `user_id, agent_id, primary_cluster, secondary_clusters, registered_at, discoverable, deactivated_at` |
| `user_topics.parquet` | U × 평균 topic 수 ≈ 100만 | `user_id, topic_id, score, visibility, revision, updated_at` |
| `friends.parquet` | U × degree/2 ≈ 150만 | `user_low, user_high, accepted_at` |
| `conversations.parquet` | U × λ ≈ 30만 | `room_id, actor_user_id, owner_user_id, agent_id, started_at, turns, reopened` |
| `events.jsonl` | 위 전부 + turn ≈ 250만 건 | 시간순. 이벤트 이름과 payload는 `agent_discovery_events.md` 그대로 |
| `queries.parquet` | 2,000 | `query_id, requester, lang, text, topic_ids` |
| `truth_*.parquet` | 타입별 | §4 |
| `topic_api_stub/` | — | 재읽기 대역이 읽는 유저별 현재 row 스냅샷 |

100만에서는 위 행 수가 10배, `events.jsonl`은 2,500만 건 정도. 생성 시간 목표는 10만 5분 이내, 100만 1시간 이내(단일 프로세스, numpy 벡터화).
...
2. **불변식**: 저장소에 private/hidden row 없음, 탈퇴 유저 row 없음, `discoverable=false` 주인의 row 없음.

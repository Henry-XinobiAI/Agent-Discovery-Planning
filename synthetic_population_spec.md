...
| `conversation_rate` | 유저당 포아송 λ ∈ **{3, 10, 30}** (기간 90일). 비교는 세 값을 모두 돈다 | 타인 agent와 대화를 시작한 횟수. 3은 첫 초안의 추정치였고, 오너는 실제 값이 그보다 클 것으로 본다(2026-09-08: 다른 agent와 대화해 배우는 것이 핵심 feature). CF의 가치가 이 값에 가장 민감하다(`library_verification.md` §3). 실측이 오면 §8의 보정이 이 행을 갈아 끼운다 |
...
LLM 확장은 비용이 있으므로 두 구현안 비교에서는 **쿼리 200개만** 실제 LLM을 통과시켜 text → topic 정확도를 재고, 나머지 1,800개는 topic id로 직접 넣어 인덱싱·정렬 부분만 잰다. text → topic 단계는 A·B 공통이라 이렇게 갈라도 비교가 흔들리지 않는다.
...
- 샘플이 작을 때(활성 유저 1,000명 미만 또는 대화 시작 5,000건 미만)는 파일을 내지 않고 기본값을 유지한다. 임계값은 `decisions.md` O16.
